...
proc installLocalCoordinateState(
    sw: Switch, record: SignedLsmrCoordinateRecord
) =
  let encoded = record.encode().tryGet()
  sw.peerInfo.metadata[LsmrCoordinateMetadataKey] = encoded
  sw.peerStore[LsmrChainBook][record.data.peerId] = @[record]
  sw.peerStore[LsmrBook][record.data.peerId] = record
  sw.peerStore[ActiveLsmrBook][record.data.peerId] = record
