...
suite "PeerStore":
  ...
  test "PeerStore API":
    ...
    peerStore.setAddresses(peerId1, @[multiaddr1])
    peerStore.setAddresses(peerId2, @[multiaddr2])
  ...
  test "PeerStore address TTL":
    var peerStore = PeerStore.new()
    let now = Moment.now()
    peerStore.addAddressesWithTTL(peerId1, @[multiaddr1], chronos.milliseconds(10), now)

    check peerStore.getAddresses(peerId1, now = now).len == 1

    let expired = now + chronos.milliseconds(15)
    check peerStore.getAddresses(peerId1, now = expired).len == 0
    check peerId1 notin peerStore[AddressBook]
