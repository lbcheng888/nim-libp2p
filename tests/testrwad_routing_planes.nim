...
import ../libp2p/rwad/[identity, node, types]
from ../libp2p/rwad/rpc/server import RpcServer, newRpcServer, start, stop
...
proc mustNewRwadNode(cfg: RwadNodeConfig): RwadNode {.raises: [].} =
  try:
    newRwadNode(cfg)
  except Exception as exc:
    raiseAssert("newRwadNode failed: " & exc.msg)
...
proc makeConfig(
  ...
  saveGenesis(baseDir / "genesis.json", createDefaultGenesis(identity, "rwad-routing-planes"))
  RwadNodeConfig(
    dataDir: baseDir,
...
suite "RWAD routing planes":
  teardown:
    checkTrackers()

  test "dual stack legacy primary keeps legacy discovery mounted":
    let baseDir = getTempDir() / ("rwad-routing-legacy-" & $nowMillis())
    let cfg = makeConfig(
      baseDir,
      RoutingPlaneMode.dualStack,
      PrimaryRoutingPlane.legacy,
      LsmrConfig.init(networkId = "rwad-routing-legacy", minWitnessQuorum = 1),
    )
    var node: RwadNode
    {.cast(gcsafe).}:
      node = mustNewRwadNode(cfg)
    defer:
      waitFor node.stop()

    let status = node.chainStatus()
  ...
  test "dual stack lsmr primary surfaces certified coordinate counts":
    let baseDir = getTempDir() / ("rwad-routing-lsmr-" & $nowMillis())
    let networkId = "rwad-routing-lsmr"
    let cfg = makeConfig(
      baseDir,
      RoutingPlaneMode.dualStack,
    ...
    let witness = makeSignedWitness(anchorKey, remotePeer, networkId, "rwad-op", 5'u8, 8'u32)
  ...
  asyncTest "rpc chain.status returns routing status for dual stack legacy primary":
    let baseDir = getTempDir() / ("rwad-rpc-routing-legacy-" & $nowMillis())
    let cfg = makeConfig(
      baseDir,
      RoutingPlaneMode.dualStack,
      PrimaryRoutingPlane.legacy,
      LsmrConfig.init(networkId = "rwad-rpc-routing-legacy", minWitnessQuorum = 1),
    )
    var node: RwadNode
    {.cast(gcsafe).}:
      node = mustNewRwadNode(cfg)
    let port = pickPort()
    let server = newRpcServer(node, "127.0.0.1", int(port))
    try:
      await server.start()
      ...
      let rpcStatus = jsonTo(rpcResponse["result"], ChainStatusSnapshot)
      ...
      let healthStatus = jsonTo(health, ChainStatusSnapshot)
  ...
  asyncTest "rpc chain.status returns routing status for dual stack lsmr primary":
    let baseDir = getTempDir() / ("rwad-rpc-routing-lsmr-" & $nowMillis())
    let networkId = "rwad-rpc-routing-lsmr"
    let cfg = makeConfig(
      baseDir,
      RoutingPlaneMode.dualStack,
      PrimaryRoutingPlane.lsmr,
    ...
    let remoteKey = PrivateKey.random(ECDSA, rng[]).tryGet()
    let remotePeer = PeerId.init(remoteKey).tryGet()
    let witness = makeSignedWitness(anchorKey, remotePeer, networkId, "rwad-rpc-op", 5'u8, 8'u32)
    node.network.switch.peerStore[ActiveLsmrBook][remotePeer] =
      makeSignedCoordinateRecord(remoteKey, remotePeer, networkId, @[5'u8, 1'u8], @[witness])
    let port = pickPort()
    let server = newRpcServer(node, "127.0.0.1", int(port))
