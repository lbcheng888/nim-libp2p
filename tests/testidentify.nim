...
suite "Identify":
  ...
  suite "handle identify message":
    ...
    asyncTest "identify includes bandwidth metadata":
      identifyProto1 = Identify.new(
        remotePeerInfo, bandwidthManager = BandwidthManager.new()
      )
      msListen.addHandler(IdentifyCodec, identifyProto1)
      ...
      check id.metadata.len == 1
      check id.metadata.hasKey(BandwidthMetadataKey)
      check id.bandwidthAnnouncement.isSome()
  ...
  suite "handle push identify message":
    ...
    asyncSetup:
      ...
      identifyPush1 = IdentifyPush.new(
        updateStore1, bandwidthManager = switch1.bandwidthManager
      )
      identifyPush2 = IdentifyPush.new(
        updateStore2, bandwidthManager = switch2.bandwidthManager
      )
    ...
    asyncTest "wrong peer id push identify":
      ...
      check:
        ...
        switch1.peerStore.getAddresses(oldPeerId) != switch2.peerInfo.addrs
