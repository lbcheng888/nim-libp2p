...
suite "Dcutr":
  ...
  template waitForExistingConn(a, b: untyped) =
    checkUntilTimeout:
      a.connManager.connCount(b.peerInfo.peerId) > 0
      b.connManager.connCount(a.peerInfo.peerId) > 0
  ...
  asyncTest "Client returns local unsupported outcome when no hole-punchable address exists":
    let sw = newStandardSwitch()
    let addrs = @[MultiAddress.init("/ip4/198.51.100.42/udp/7890").tryGet()]

    let outcome = await DcutrClient.new().startSync(sw, sw.peerInfo.peerId, addrs)

    check outcome == dcore.DcutrLocalUnsupportedAddrs

  asyncTest "Client returns remote unsupported outcome when peer advertises no hole-punchable address":
    let behindNATSwitch = newStandardSwitch()
    let publicSwitch = newStandardSwitch()
    let unsupportedAddr = MultiAddress.init("/ip4/198.51.100.42/udp/7890").tryGet()
    let dcutrProto = LPProtocol.new(
      @[dcore.DcutrCodec],
      proc(conn: Connection, proto: string): Future[void] {.async: (raises: [CancelledError]).} =
        try:
          let connectMsg = dcore.DcutrMsg.decode(await conn.readLp(dcore.DcutrMsgMaxSize))
          connectMsg.expectMsgType(dcore.MsgType.Connect)
          await conn.send(dcore.MsgType.Connect, @[unsupportedAddr])
        except dcore.DcutrError as err:
          raiseAssert err.msg
    ...
    waitForExistingConn(behindNATSwitch, publicSwitch)

    let outcome = await DcutrClient.new().startSync(
      behindNATSwitch, publicSwitch.peerInfo.peerId, behindNATSwitch.peerInfo.addrs
    )

    check outcome == dcore.DcutrRemoteUnsupportedAddrs

    await allFutures(behindNATSwitch.stop(), publicSwitch.stop())

  asyncTest "Client rejects unexpected DCUtR response message type":
    let behindNATSwitch = newStandardSwitch()
    let publicSwitch = newStandardSwitch()
    let dcutrProto = LPProtocol.new(
      @[dcore.DcutrCodec],
      proc(conn: Connection, proto: string): Future[void] {.async: (raises: [CancelledError]).} =
        try:
          let connectMsg = dcore.DcutrMsg.decode(await conn.readLp(dcore.DcutrMsgMaxSize))
          connectMsg.expectMsgType(dcore.MsgType.Connect)
          await conn.send(dcore.MsgType.Sync, @[])
        except dcore.DcutrError as err:
          raiseAssert err.msg
        except LPStreamError as err:
          raiseAssert err.msg
    ...
    expect dcore.DcutrError:
      discard await DcutrClient.new().startSync(
        behindNATSwitch, publicSwitch.peerInfo.peerId, behindNATSwitch.peerInfo.addrs
      )
  ...
  asyncTest "Client connect timeout":
    ...
    ductrClientTest(behindNATSwitch, publicSwitch):
      try:
        ...
        discard await client.startSync(
          behindNATSwitch, publicSwitch.peerInfo.peerId, behindNATSwitch.peerInfo.addrs
        )
  ...
  test "should return valid TCP and QUIC addresses only":
    let testAddrs =
      @[
        ...
        .tryGet(),
        MultiAddress.init("/ip4/198.51.100.7/udp/4001/quic-v1").tryGet(),
        MultiAddress
        .init(
          "/dns4/quic.example/udp/4002/quic-v1/p2p/QmYyQSo1c1Ym7orWxLYvCrM2EmxFTANf8wXmmE7DWjhx5N"
        )
        .tryGet(),
        MultiAddress
        .init(
          "/ip4/203.0.113.8/udp/4003/quic/p2p/QmYyQSo1c1Ym7orWxLYvCrM2EmxFTANf8wXmmE7DWjhx5N"
        )
        .tryGet(),
    ...
    let expected =
      @[
        ...
        MultiAddress.init("/ip4/198.51.100.7/udp/4001/quic-v1").tryGet(),
        MultiAddress.init("/dns4/quic.example/udp/4002/quic-v1").tryGet(),
        MultiAddress.init("/ip4/203.0.113.8/udp/4003/quic").tryGet(),
      ]
  ...
  test "should limit hole-punchable addresses when maxAddrs is set":
    let testAddrs =
      @[
        MultiAddress.init("/ip4/192.0.2.1/tcp/1234").tryGet(),
        MultiAddress.init("/ip4/203.0.113.5/tcp/5678").tryGet(),
    ...
    let result = getHolePunchableAddrs(testAddrs, maxAddrs = 2)

    check result == testAddrs[0 .. 1]
