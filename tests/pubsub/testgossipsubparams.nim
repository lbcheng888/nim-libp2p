...
suite "GossipSubParams validation":
  ...
  test "maxMessageSize fails when zero":
    const errorMessage = "gossipsub: maxMessageSize parameter error, Must be > 0"
    var params = newDefaultValidParams()
    params.maxMessageSize = 0
    let res = params.validateParameters()
    check res.isErr()
    check res.error == errorMessage

  test "shardChunkSize fails when larger than maxMessageSize":
    const errorMessage =
      "gossipsub: shardChunkSize must be <= maxMessageSize when sharding is enabled"
    var params = newDefaultValidParams()
    params.enableShardEncoding = true
    params.maxMessageSize = 64
    params.shardChunkSize = 128
    let res = params.validateParameters()
    check res.isErr()
    check res.error == errorMessage

  test "shardMinMessageSize fails when larger than maxMessageSize":
    const errorMessage =
      "gossipsub: shardMinMessageSize must be <= maxMessageSize when sharding is enabled"
    var params = newDefaultValidParams()
    params.enableShardEncoding = true
    params.maxMessageSize = 64
    params.shardChunkSize = 32
    params.shardMinMessageSize = 128
    let res = params.validateParameters()
    check res.isErr()
    check res.error == errorMessage
