...
proc setupGossipSubWithPeers*(
  ...
  let transportType =
    when defined(libp2p_quic_support):
      ...
      TransportType.TCP
  let gossipSub = TestGossipSub.init(newStandardSwitch(transport = transportType))
...
proc generateNodes*(
    ...
    transportAddrs: seq[string] = @[],
): seq[PubSub] =
  var baseAddrs: seq[MultiAddress] = @[]
  if transportAddrs.len > 0:
    for text in transportAddrs:
      baseAddrs.add(MultiAddress.init(text).tryGet())
  else:
    case transport
    of TransportType.QUIC:
      baseAddrs.add(MultiAddress.init("/ip4/127.0.0.1/udp/0/quic-v1").tryGet())
    of TransportType.TCP:
      baseAddrs.add(MultiAddress.init("/ip4/127.0.0.1/tcp/0").tryGet())
    of TransportType.Memory:
      discard

  for i in 0 ..< num:
    var nodeAddrs = baseAddrs
...
proc setDefaultTopicParams*(nodes: seq[GossipSub], topic: string): void =
  for node in nodes:
    withMapEntry(node.topicParams, topic, TopicParams.init(), params):
      params.topicWeight = 1.0
...
proc setupNodes*(count: int): seq[PubSub] =
  ## Helper used by flood-publish tests that exercise large payloads.
  ## Increase the max message size so we don't trip the default 1MiB guard.
  generateNodes(count, gossip = true, maxMessageSize = 5 * 1024 * 1024)
