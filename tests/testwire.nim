...
suite "Wire":
  ...
  test "initTAddress returns error for a UDP WebRTCDirect address":
    let ma = MultiAddress
      .init(
        "/ip4/127.0.0.1/udp/1234/webrtc-direct/certhash/uEiDjrWq8zYtPXmOz08k4DQhPayjJYqpyR5jH5z8aaWZ2aw"
      )
      .get()
    check initTAddress(ma).isErr

  test "initTAddress returns error for a IPv6 WebRTCDirect address":
    let ma = MultiAddress
      .init(
        "/ip6/::1/udp/0/webrtc-direct/certhash/uEiAENlMVTJM43KZl38nn7rRIb5uiry9dKDpPZpnE0-WxEg"
      )
      .get()
    check initTAddress(ma).isErr
