...
const
  SuccessVectors = [
    ...
    "/ip4/192.0.2.1/udp/4001/quic/webtransport",
    "/ip4/192.0.2.1/udp/4001/quic/webtransport/certhash/uEiDjrWq8zYtPXmOz08k4DQhPayjJYqpyR5jH5z8aaWZ2aw",
  ...
  FailureVectors = [
    ...
    "/ip4/192.0.2.1/udp/4001/webtransport",
    "/ip4/192.0.2.1/udp/4001/quic/certhash/uEiDjrWq8zYtPXmOz08k4DQhPayjJYqpyR5jH5z8aaWZ2aw",
    "/ip4/192.0.2.1/udp/4001/quic/webtransport/certhash/abc",
  ]
  ...
  PatternVectors = [
    ...
    PatternVector(
      ...
      good:
        @[
          "/ip4/1.2.3.4/udp/3456/webrtc-direct/certhash/uEiDjrWq8zYtPXmOz08k4DQhPayjJYqpyR5jH5z8aaWZ2aw",
          "/ip6/::/udp/0/webrtc-direct/certhash/uEiAENlMVTJM43KZl38nn7rRIb5uiry9dKDpPZpnE0-WxEg",
        ],
      bad:
        @[
          "/ip4/0.0.0.0",
    ...
    PatternVector(
      pattern: WebTransport,
      good:
        @[
          "/ip4/1.2.3.4/udp/3456/quic/webtransport",
          "/ip4/1.2.3.4/udp/3456/quic/webtransport/certhash/uEiDjrWq8zYtPXmOz08k4DQhPayjJYqpyR5jH5z8aaWZ2aw",
          "/ip6/::/udp/0/quic-v1/webtransport/certhash/uEiAENlMVTJM43KZl38nn7rRIb5uiry9dKDpPZpnE0-WxEg",
        ],
      bad:
        @[
          "/ip4/1.2.3.4/udp/3456/webtransport",
          "/ip4/1.2.3.4/tcp/3456/quic/webtransport",
          "/ip4/1.2.3.4/udp/3456/quic",
        ],
      partial: true,
    ),
    PatternVector(
...
suite "MultiAddress test suite":
  ...
  test "MultiAddress pattern matching test vectors":
    for item in PatternVectors:
      for gitem in item.good:
        let parsedGood = MultiAddress.init(gitem)
        check parsedGood.isOk()
        let a = parsedGood.get()
        let matched =
          if item.partial:
            item.pattern.matchPartial(a)
          else:
            item.pattern.match(a)
        check matched == true
      for bitem in item.bad:
        let parsedBad = MultiAddress.init(bitem)
        if parsedBad.isErr:
          continue
        let a = parsedBad.get()
        let matched =
  ...
  test "webtransport certhash roundtrip preserves order":
    let addressStr =
      "/ip4/192.0.2.1/udp/4001/quic/webtransport/certhash/uEiDjrWq8zYtPXmOz08k4DQhPayjJYqpyR5jH5z8aaWZ2aw/certhash/uEiAENlMVTJM43KZl38nn7rRIb5uiry9dKDpPZpnE0-WxEg"
    let initRes = MultiAddress.init(addressStr)
    require initRes.isOk()
    let ma = initRes.get()
    check $ma == addressStr
