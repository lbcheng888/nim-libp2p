...
when defined(libp2p_run_autonat_tests):
  proc registerAutonatV2Suite() =
    suite "AutonatV2":
      teardown:
        checkTrackers()
      ...
      asyncTest "Successful DialRequest with amplification attack prevention":
        let primaryIp = checkedGetIPAddress()
        if primaryIp.len == 0 or primaryIp == "127.0.0.1":
          skip()
          return
        ...
        let
          listenAddrs =
            @[
              MultiAddress.init("/ip4/" & primaryIp & "/tcp/4040").get(),
              MultiAddress.init("/ip4/127.0.0.1/tcp/4040").get(),
            ]
  ...
  proc registerAutonatV2Suite() =
    suite "AutonatV2":
      test "AutonatV2 tests disabled":
        skip()

registerAutonatV2Suite()
