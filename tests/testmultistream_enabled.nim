...
asyncTest "test handle `ls`":
  ...
  proc testLsHandler(
    ...
    var consumed: int
    var fieldLen: uint64
    check PB.getUVarint(proto.toOpenArray(0, proto.high), consumed, fieldLen).isOk()
    check consumed + int(fieldLen) <= proto.len
    let payload = proto[consumed ..< consumed + int(fieldLen)]
    let listing = decodeProtocolListing(payload)
    check listing == @["/test/proto1/1.0.0", "/test/proto2/1.0.0"]
    await conn.close()
    done.complete()
