...
proc runTest(filename: string, moreoptions: string = "") =
  ...
  excstr.add(" -d:libp2p_skip_switch_tests ")
  let combinedParams = moreoptions & " " & flags
  if "-d:chronicles_enabled" notin combinedParams:
    excstr.add(" -d:chronicles_enabled=true ")
  if getEnv("CICOV").len > 0:
    excstr &= " --nimcache:nimcache/" & filename & "-" & $excstr.hash
  echo "[nimble] Compiling tests/" & filename & " ..."
  exec excstr &
...
task buildffi, "Build shared library for UniMaker Nim FFI":
  let repoRoot = getCurrentDir().parentDir()
  var outDir = getEnv("OUT_DIR", repoRoot / "build" / "nim-libp2p")
  let ensureDirCmd =
    when defined(windows):
      "cmd /C if not exist " & quoteShell(outDir) & " mkdir " & quoteShell(outDir)
    else:
      "mkdir -p " & quoteShell(outDir)
  exec ensureDirCmd
  ...
  let outPath = outDir / libName
  var cmd = nimc & " c --app:lib " & cfg & " " & flags &
    " --threads:on --define:chronicles_enabled=false --path:." &
    " --out:" & quoteShell(outPath)
  when defined(posix):
    cmd &= " --passC:\"-fPIC\""
  cmd &= " ../mobile_ffi/libnimlibp2p.nim"
  exec cmd
