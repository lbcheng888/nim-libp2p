...
import ./gossipsub/[types, scoring, behavior, preamblestore, sharding],
  ../../utils/heartbeat
...
proc init*(
    ...
    enableShardEncoding = false,
    shardChunkSize = 32 * 1024,
    shardRedundancy = 2,
    shardMinMessageSize = 64 * 1024,
    shardSessionTimeout = 30.seconds,
    maxMessageSize = DefaultGossipMaxMessageSize,
    bandwidthPreferenceWeight = 0.0,
    bandwidthAnnouncementMaxAge = 5.minutes,
  ...
  GossipSubParams(
    ...
    enableShardEncoding: enableShardEncoding,
    shardChunkSize: shardChunkSize,
    shardRedundancy: shardRedundancy,
    shardMinMessageSize: shardMinMessageSize,
    shardSessionTimeout: shardSessionTimeout,
    maxMessageSize: maxMessageSize,
    bandwidthPreferenceWeight: bandwidthPreferenceWeight,
    bandwidthAnnouncementMaxAge: bandwidthAnnouncementMaxAge,
...
proc validateParameters*(parameters: GossipSubParams): Result[void, cstring] =
  ...
  elif parameters.maxMessageSize <= 0:
    err("gossipsub: maxMessageSize parameter error, Must be > 0")
  elif parameters.bandwidthPreferenceWeight < 0:
    err("gossipsub: bandwidthPreferenceWeight parameter error, Must be >= 0")
  elif parameters.bandwidthAnnouncementMaxAge < chronos.ZeroDuration:
    err("gossipsub: bandwidthAnnouncementMaxAge parameter error, Must be >= 0")
  elif parameters.enableShardEncoding and parameters.shardChunkSize <= 0:
    err("gossipsub: shardChunkSize must be positive when sharding is enabled")
  elif parameters.enableShardEncoding and parameters.shardRedundancy < 0:
    err("gossipsub: shardRedundancy must be >= 0 when sharding is enabled")
  elif parameters.enableShardEncoding and parameters.shardMinMessageSize <= 0:
    err("gossipsub: shardMinMessageSize must be positive when sharding is enabled")
  elif parameters.enableShardEncoding and
      parameters.shardMinMessageSize < parameters.shardChunkSize:
    err("gossipsub: shardMinMessageSize must be >= shardChunkSize")
  elif parameters.enableShardEncoding and
      parameters.shardChunkSize > parameters.maxMessageSize:
    err("gossipsub: shardChunkSize must be <= maxMessageSize when sharding is enabled")
  elif parameters.enableShardEncoding and
      parameters.shardMinMessageSize > parameters.maxMessageSize:
    err("gossipsub: shardMinMessageSize must be <= maxMessageSize when sharding is enabled")
  elif parameters.enableShardEncoding and parameters.shardSessionTimeout <= 0.seconds:
    err("gossipsub: shardSessionTimeout must be > 0 when sharding is enabled")
  elif parameters.pxPromiseTTL < chronos.ZeroDuration:
    err("gossipsub: pxPromiseTTL parameter error, Must be >= 0")
  elif parameters.pxSuccessReward < 0:
...
when defined(libp2p_privacy_gossip):
  func privacyEnabled(g: GossipSub): bool {.inline.} =
    g.parameters.privacyDandelion

  proc privacyCleanupStemRoutes(g: GossipSub) =
    if g.privacyStemRoutes.len == 0:
      return
    let now = Moment.now()
    var expired: seq[SaltedId]
    for msgId, route in g.privacyStemRoutes.pairs:
      if route.expires <= now or route.peer.isNil or not route.peer.connected:
        expired.add(msgId)
    for msgId in expired:
      g.privacyStemRoutes.del(msgId)

  func privacyPeerInSet(peers: HashSet[PubSubPeer], peer: PubSubPeer): bool =
    if peer.isNil:
      return false
  ...
  proc privacyMaybeLimitStem(
      g: GossipSub, saltedId: SaltedId, peers: var HashSet[PubSubPeer]
  ) =
    if not g.privacyEnabled() or g.privacyPhase != privacyStem or peers.len == 0:
      return

    g.privacyCleanupStemRoutes()

    if g.privacyStemRoutes.hasKey(saltedId):
      let route = g.privacyStemRoutes[saltedId]
      if privacyPeerInSet(peers, route.peer):
        var limited = initHashSet[PubSubPeer]()
        limited.incl(route.peer)
        peers = limited
        return
      else:
        g.privacyStemRoutes.del(saltedId)

    var ordered = newSeqOfCap[PubSubPeer](peers.len)
    for peer in peers:
      ordered.add(peer)
    if ordered.len == 0:
      return

    g.rng.shuffle(ordered)
    var limited = initHashSet[PubSubPeer]()
    var fanout = g.parameters.privacyStemFanout
    if fanout <= 0:
      fanout = 1
    let limit = if ordered.len < fanout: ordered.len else: fanout
    for i in 0 ..< limit:
      limited.incl(ordered[i])
    peers = limited

    let ttl =
      if g.parameters.privacyRouteTTL > chronos.ZeroDuration:
        g.parameters.privacyRouteTTL
      else:
        g.parameters.seenTTL
    g.privacyStemRoutes[saltedId] =
      StemRoute(peer: ordered[0], expires: Moment.fromNow(ttl))
...
type ShardProcessKind = enum
  shardNotApplicable,
  shardIncomplete,
  shardComplete,
  shardInvalid

proc cleanupShardSessions(g: GossipSub) =
  let now = Moment.now()
  var expired: seq[ShardKey] = @[]
  for key, expiry in g.shardExpiry:
    if expiry <= now:
      expired.add(key)
  for key in expired:
    g.shardExpiry.del(key)
    g.shardSessions.del(key)

proc nextShardSessionId(g: GossipSub): SessionId =
  inc g.shardSessionSeq
  if g.shardSessionSeq == 0:
    inc g.shardSessionSeq
  g.shardSessionSeq

proc processShardMessage(
    g: GossipSub, topic: string, msg: Message, msgId: MessageId
): tuple[kind: ShardProcessKind, data: seq[byte]] =
  if not g.parameters.enableShardEncoding:
    return (shardNotApplicable, msg.data)

  let decoded = decodeShardPayload(msg.data)
  if decoded.isNone():
    return (shardNotApplicable, msg.data)

  var shard = decoded.get()
  if shard.originalSize < 0 or shard.chunkSize <= 0 or shard.shardCount <= 0:
    return (shardInvalid, @[])

  g.cleanupShardSessions()
  let key = shardKey(topic, shard.sessionId)

  var session = g.shardSessions.getOrDefault(key)
  if session.isNil:
    session = newShardSession(shard)
    g.shardSessions[key] = session
  elif session.completed:
    g.shardExpiry[key] = Moment.fromNow(g.parameters.shardSessionTimeout)
    return (shardIncomplete, @[])
  ...
  g.shardExpiry[key] = Moment.fromNow(g.parameters.shardSessionTimeout)
  if not session.acceptShard(shard):
    return (shardIncomplete, @[])
  ...
  session.coefficients.setLen(0)
  session.encoded.setLen(0)
  session.seenIndices.clear()
  session.coefficientHashes.clear()
  g.shardExpiry[key] = Moment.fromNow(g.parameters.shardSessionTimeout)
  (shardComplete, assembled.get())
...
proc publishSharded(
  ...
  g.cleanupShardSessions()
  let sessionId = g.nextShardSessionId()
  let cfg = ShardConfig(
    chunkSize: g.parameters.shardChunkSize,
    redundancy: g.parameters.shardRedundancy,
  )
  if cfg.chunkSize > g.parameters.maxMessageSize:
    trace "Shard chunk size exceeds maxMessageSize",
      chunk = cfg.chunkSize, limit = g.parameters.maxMessageSize
    libp2p_gossipsub_failed_publish.inc()
    return 0
  let shards = encodeShards(data, cfg, g.rng, sessionId)
  var delivered = 0
  var shardSent = false
  ...
  for payload in shards:
    let
      msg =
        if g.anonymize:
          Message.init(none(PeerInfo), payload, topic, none(uint64), false)
        else:
          inc g.msgSeqno
          Message.init(some(g.peerInfo), payload, topic, some(g.msgSeqno), g.sign)
      msgId = g.msgIdProvider(msg).valueOr:
        trace "Error generating message id for shard, skipping publish", error = error
        libp2p_gossipsub_failed_publish.inc()
        continue
  ...
  if shardSent:
    ...
    if delivered > 0:
      if g.knownTopics.contains(topic):
        libp2p_pubsub_messages_published.inc(delivered.int64, labelValues = [topic])
      else:
        libp2p_pubsub_messages_published.inc(delivered.int64, labelValues = ["generic"])
...
proc validateAndRelay(
  ...
  try:
    ...
    if msg.data.len > g.parameters.maxMessageSize and
        (not g.parameters.enableShardEncoding or not msg.data.isShardPayload()):
      debug "Dropping message above maxMessageSize",
        msgId = shortLog(msgId), size = msg.data.len, limit = g.parameters.maxMessageSize
    ...
    var shardOutcome = shardNotApplicable
    var deliverData = some(processedMsg.data)
    if g.parameters.enableShardEncoding:
      let (kind, assembled) = g.processShardMessage(topic, processedMsg, msgId)
      shardOutcome = kind
      case kind
      of shardInvalid:
        await g.punishInvalidMessage(peer, processedMsg)
        return
      of shardComplete:
        deliverData = some(assembled)
        processedMsg.data = assembled
        g.mcache.put(msgId, processedMsg)
      of shardIncomplete:
        deliverData = none(seq[byte])
      of shardNotApplicable:
        deliverData = some(processedMsg.data)

    g.rewardDelivered(peer, topic, true)

    # trigger hooks
    peer.validatedObservers(processedMsg, msgId)
    ...
    when defined(libp2p_privacy_gossip):
      g.privacyMaybeLimitStem(saltedId, toSendPeers)
    ...
    proc isMsgInIdontWant(it: PubSubPeer): bool =
      for iDontWant in it.iDontWants:
        if saltedId in iDontWant:
          ...
          libp2p_gossipsub_saved_bytes.inc(
            processedMsg.data.len.int64, labelValues = ["idontwant"]
          )
          return true
      return false
    ...
    when defined(libp2p_gossipsub_1_4):
      ...
      proc deferSend(deferPeers: HashSet[PubSubPeer]) {.async.} =
        let receiveTimeMs = calculateReceiveTimeMs(processedMsg.data.len)
        ...
        for deferPeer in deferPeers:
          if not deferPeer.isMsgInIdontWant:
            ...
            g.broadcast(
              @[deferPeer], RPCMsg(messages: @[processedMsg]), isHighPriority = false
            )

      let allPeers = toSendPeers
      toSendPeers.exclIfIt(isMsgInIMReceiving(it))
      g.sendPreamble(processedMsg, msgId, toSendPeers)
    ...
    g.broadcast(toSendPeers, RPCMsg(messages: @[processedMsg]), isHighPriority = false)
    ...
    if deliverData.isSome():
      await handleData(g, topic, deliverData.get())
  except CancelledError as exc:
...
method publish*(
  ...
  if g.parameters.enableShardEncoding and data.len >= g.parameters.shardMinMessageSize:
    return await g.publishSharded(topic, data, pubParams, peers)

  if not g.parameters.enableShardEncoding and data.len > g.parameters.maxMessageSize:
    debug "Dropping publish above maxMessageSize", size = data.len,
      limit = g.parameters.maxMessageSize
    libp2p_gossipsub_failed_publish.inc()
    return 0
  ...
  when defined(libp2p_privacy_gossip):
    g.privacyMaybeLimitStem(saltedId, peers)
...
method initPubSub*(g: GossipSub) {.raises: [InitializationError].} =
  ...
  g.shardSessions = initTable[ShardKey, ShardSession]()
  g.shardExpiry = initTable[ShardKey, Moment]()
  g.shardSessionSeq = 0
  if g.parameters.maxMessageSize <= 0:
    g.parameters.maxMessageSize = DefaultGossipMaxMessageSize
  g.maxMessageSize = g.parameters.maxMessageSize
  g.pxRecordSeq = initTable[PeerId, uint64]()
  g.pxPromises = initTable[PeerId, Table[PeerId, Moment]]()
  g.pxPeerSources = initTable[PeerId, HashSet[PeerId]]()
  g.scoreMetricAgents = initHashSet[string]()

  when defined(libp2p_privacy_gossip):
    g.privacyStemRoutes = initTable[SaltedId, StemRoute]()
    if g.parameters.privacyDandelion:
      let interval =
        if g.parameters.privacyStemDuration > chronos.ZeroDuration:
          g.parameters.privacyStemDuration
        else:
          g.parameters.heartbeatInterval
      g.privacyPhase = privacyStem
      g.privacyPhaseUntil = Moment.fromNow(interval)
    else:
      g.privacyPhase = privacyFluff
      g.privacyPhaseUntil = Moment.now()
