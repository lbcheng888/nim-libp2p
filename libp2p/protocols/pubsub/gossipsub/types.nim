...
const
  ...
  DefaultGossipMaxMessageSize* = 1024 * 1024

when defined(libp2p_privacy_gossip):
  type
    GossipPrivacyPhase* = enum
      privacyStem,
      privacyFluff
...
type
  ...
  GossipSubParams* {.public.} = object
    ...
    bandwidthPreferenceWeight*: float64
      ## When > 0, prefer higher bandwidth peers during mesh maintenance.
      ## Acts as a multiplier over ln(1 + advertised/observed bytes per second).

    bandwidthAnnouncementMaxAge*: Duration
      ## Ignore remote bandwidth announcements older than this. Non-positive disables the check.

    pxPromiseTTL*: Duration
      ## How long PX suggestions remain pending before being discarded. Non-positive disables tracking.
    pxSuccessReward*: float64
      ## Amount to subtract from behaviourPenalty when a PX suggestion yields a successful connection.
    pxFailurePenalty*: float64
      ## Amount to add to behaviourPenalty when a PX suggestion expires without a successful connection.
    ...
    enableShardEncoding*: bool
    shardChunkSize*: int
    shardRedundancy*: int
    shardMinMessageSize*: int
    shardSessionTimeout*: Duration
    maxMessageSize*: int

    when defined(libp2p_privacy_gossip):
      privacyDandelion*: bool
      privacyStemDuration*: Duration
      privacyFluffDuration*: Duration
      privacyRouteTTL*: Duration
      privacyStemFanout*: int
  ...
  GossipSub* = ref object of FloodSub
    ...
    shardSessions*: Table[ShardKey, ShardSession]
    shardExpiry*: Table[ShardKey, Moment]
    shardSessionSeq*: SessionId
    ...
    pxRecordSeq*: Table[PeerId, uint64]
    pxPromises*: Table[PeerId, Table[PeerId, Moment]]
    pxPeerSources*: Table[PeerId, HashSet[PeerId]]
    scoreMetricAgents*: HashSet[string]
    ...
    when defined(libp2p_privacy_gossip):
      privacyPhase*: GossipPrivacyPhase
      privacyPhaseUntil*: Moment
      privacyStemRoutes*: Table[SaltedId, StemRoute]
