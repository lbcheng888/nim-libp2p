...
import std/[tables, sequtils, sets, algorithm, deques, math, options, times]
...
declareCounter(
  libp2p_gossipsub_px_events,
  "PX suggestion lifecycle counters",
  labels = ["result"],
)
...
when defined(libp2p_privacy_gossip):
  proc updatePrivacyPhase(g: GossipSub) =
    if not g.parameters.privacyDandelion:
      return

    let now = Moment.now()
    if now < g.privacyPhaseUntil:
      return

    if g.privacyPhase == privacyStem:
      let interval =
        if g.parameters.privacyFluffDuration > chronos.ZeroDuration:
          g.parameters.privacyFluffDuration
        else:
...
proc shouldApplyPeerRecord(g: GossipSub, record: PeerRecord): bool =
  if record.peerId.len == 0:
    return false
  let previous = g.pxRecordSeq.getOrDefault(record.peerId)
  if previous >= record.seqNo:
    return false
  g.pxRecordSeq[record.peerId] = record.seqNo
  true

proc mergePeerRecord(g: GossipSub, record: PeerRecord) =
  if record.addresses.len == 0:
    return
  let addrs = record.addresses.mapIt(it.address)
  g.switch.peerStore.addAddressesWithTTL(
    record.peerId, addrs, DefaultDiscoveredAddressTTL
  )

proc trackPxSuggestion(g: GossipSub, provider: PeerId, suggestion: PeerId) =
  if provider.len == 0 or suggestion.len == 0 or provider == suggestion:
    return
  if g.parameters.pxPromiseTTL <= chronos.ZeroDuration:
    return
  let expiry = Moment.fromNow(g.parameters.pxPromiseTTL)
  withMapEntry(g.pxPromises, provider, initTable[PeerId, Moment](), promises):
    promises[suggestion] = expiry
  withMapEntry(g.pxPeerSources, suggestion, initHashSet[PeerId](), sources):
    sources.incl(provider)

proc setBackoff(g: GossipSub, topic: string, peerId: PeerId, expires: Moment) =
  withMapEntry(g.backingOff, topic, initTable[PeerId, Moment](), table):
    table[peerId] = expires

proc adjustBehaviourPenalty(g: GossipSub, provider: PeerId, delta: float64) =
  if provider.len == 0 or delta == 0.0:
    return
  g.peers.withValue(provider, peer):
    peer[].behaviourPenalty = max(0.0, peer[].behaviourPenalty + delta)
  g.withPeerStats(provider) do(stats: var PeerStats):
    stats.behaviourPenalty = max(0.0, stats.behaviourPenalty + delta)

proc processPeerExchange(
    g: GossipSub, provider: PubSubPeer, suggestions: seq[RoutingRecordsPair]
) =
  if suggestions.len == 0:
    return
  libp2p_gossipsub_px_events.inc(float64(suggestions.len), labelValues = ["received"])
  var tracked = 0
  for (peerId, recordOpt) in suggestions:
    if peerId.len == 0 or peerId == g.peerInfo.peerId:
      continue
    g.trackPxSuggestion(provider.peerId, peerId)
    inc tracked
    recordOpt.withValue(record):
      if g.shouldApplyPeerRecord(record):
        g.mergePeerRecord(record)
  let ignored = suggestions.len - tracked
  if tracked > 0:
    libp2p_gossipsub_px_events.inc(float64(tracked), labelValues = ["tracked"])
  if ignored > 0:
    libp2p_gossipsub_px_events.inc(float64(ignored), labelValues = ["ignored"])

proc processPxPromises(g: GossipSub) =
  if g.pxPromises.len == 0:
    return
  if g.parameters.pxPromiseTTL <= chronos.ZeroDuration:
    g.pxPromises.clear()
    g.pxPeerSources.clear()
    return
  let now = Moment.now()
  var providersToDelete: seq[PeerId] = @[]
  var expirationsPerProvider = initTable[PeerId, int]()
  for provider, promises in g.pxPromises.mpairs:
    var expiredPeers: seq[PeerId] = @[]
    for peerId, expiry in promises.pairs:
      if expiry <= now:
        expiredPeers.add(peerId)
    for peerId in expiredPeers:
      promises.del(peerId)
      g.pxPeerSources.withValue(peerId, providers):
        providers[].excl(provider)
        if providers[].len == 0:
          g.pxPeerSources.del(peerId)
      withMapEntry(expirationsPerProvider, provider, 0, count):
        inc count
    if promises.len == 0:
      providersToDelete.add(provider)
  for provider in providersToDelete:
    g.pxPromises.del(provider)
  var totalExpired = 0
  for provider, count in expirationsPerProvider.pairs:
    if count <= 0:
      continue
    totalExpired += count
    if g.parameters.pxFailurePenalty > 0.0:
      g.adjustBehaviourPenalty(provider, g.parameters.pxFailurePenalty * float64(count))
  if totalExpired > 0:
    libp2p_gossipsub_px_events.inc(float64(totalExpired), labelValues = ["expired"])

proc handlePxSuccess*(g: GossipSub, peer: PubSubPeer) =
  if g.pxPeerSources.len == 0:
    return
  var providers: seq[PeerId] = @[]
  g.pxPeerSources.withValue(peer.peerId, providerSet):
    providers = toSeq(providerSet[])
  if providers.len == 0:
    return
  g.pxPeerSources.del(peer.peerId)
  for provider in providers:
    g.pxPromises.withValue(provider, promises):
      promises[].del(peer.peerId)
      if promises[].len == 0:
        g.pxPromises.del(provider)
    if g.parameters.pxSuccessReward > 0:
      g.adjustBehaviourPenalty(provider, -g.parameters.pxSuccessReward)
  if providers.len > 0:
    libp2p_gossipsub_px_events.inc(float64(providers.len), labelValues = ["fulfilled"])

proc bandwidthSnapshotFresh(g: GossipSub, snapshot: BandwidthSnapshot): bool =
  if snapshot.timestamp == 0'u64:
    return false
  let maxAge = g.parameters.bandwidthAnnouncementMaxAge
  if maxAge <= chronos.ZeroDuration:
    return true
  let maxAgeSec = maxAge.nanoseconds.float / 1_000_000_000.0
  if maxAgeSec <= 0.0:
    return true
  let ageSec = epochTime() - float(snapshot.timestamp)
  if ageSec < 0.0:
    return true
  ageSec <= maxAgeSec

proc estimatePeerBandwidth(g: GossipSub, peer: PubSubPeer): float64 =
  var best = 0.0

  let statsOpt = g.switch.bandwidthStats(peer.peerId)
  if statsOpt.isSome:
    let stats = statsOpt.get()
    best = max(best, stats.inbound.emaBytesPerSecond)
    best = max(best, stats.outbound.emaBytesPerSecond)

  when defined(libp2p_gossipsub_1_4):
    if not peer.bandwidthTracking.isNil:
      best = max(best, peer.bandwidthTracking.download.value())

  let bandwidthBook = g.switch.peerStore[BandwidthBook]
  if bandwidthBook.contains(peer.peerId):
    let snapshot = bandwidthBook[peer.peerId]
    if g.bandwidthSnapshotFresh(snapshot):
      best = max(best, snapshot.inboundRateBps)
      best = max(best, snapshot.outboundRateBps)

  best

proc effectivePeerScore(g: GossipSub, peer: PubSubPeer): float64 =
  var score = peer.score
  let weight = g.parameters.bandwidthPreferenceWeight
  if weight > 0 and peer.peerId.len > 0:
    let bandwidth = g.estimatePeerBandwidth(peer)
    if bandwidth > 0:
      score += weight * ln(1.0 + bandwidth)
  score
...
proc grafted*(g: GossipSub, p: PubSubPeer, topic: string) =
  g.withPeerStats(p.peerId) do(stats: var PeerStats):
    ...
    info.meshTime = chronos.ZeroDuration
    info.inMesh = true
...
proc pruned*(
  ...
  if setBackoff:
    ...
    g.setBackoff(topic, p.peerId, backoffMoment)
...
proc handleGraft*(
  ...
  for graft in grafts:
    ...
    if peer.peerId in g.parameters.directPeers:
      ...
      g.setBackoff(topic, peer.peerId, backoff)
    ...
    let slackDuration = chronos.seconds(BackoffSlackTime * 2)
    if g.backingOff.getOrDefault(topic).getOrDefault(peer.peerId) >
        Moment.now() + slackDuration:
...
proc handlePrune*(g: GossipSub, peer: PubSubPeer, prunes: seq[ControlPrune]) =
  for prune in prunes:
    ...
    let acceptPx = prune.peers.len > 0 and peer.score >= g.parameters.acceptPXThreshold
    if acceptPx:
      let routingRecords = prune.getPeers(peer)
      g.processPeerExchange(peer, routingRecords)

      if g.routingRecordsHandler.len > 0:
...
when defined(libp2p_gossipsub_1_4):
  proc bandwidthEstimate(g: GossipSub, peer: PubSubPeer): float64 =
    if peer.isNil:
      return float64(InitialRate)
    let estimate = g.estimatePeerBandwidth(peer)
    if estimate > 0.0:
      estimate
    else:
      float64(InitialRate)

  proc medianBandwidthEstimate(
      g: GossipSub, peers: var HashSet[PubSubPeer]
  ): float64 =
    if peers.len == 0:
      return 0.0
    ...
    for candidate in peers:
      if candidate.isNil:
        continue
      values.add(g.bandwidthEstimate(candidate))
  ...
  proc handlePreamble*(
    ...
    for preamble in preambles:
      ...
      if g.hasSeen(g.salt(preamble.messageID)):
        ...
        let bytesPerSecond = g.bandwidthEstimate(peer)
        let transmissionTimeMs = calculateReceiveTimeMs(
          preamble.messageLength.int64, max(bytesPerSecond, 1.0).int64
        )
        ...
        if bytesPerSecond >= g.medianBandwidthEstimate(toSendPeers):
          g.broadcast(
            peers,
            RPCMsg(
              control: some(
...
proc rebalanceMesh*(g: GossipSub, topic: string, metrics: ptr MeshMetrics = nil) =
  ...
  var
    ...
    scoreCache = initTable[PeerId, float64]()

  proc cachedScore(peer: PubSubPeer): float64 =
    if scoreCache.hasKey(peer.peerId):
      return scoreCache.getOrDefault(peer.peerId, 0.0)
    let value = g.effectivePeerScore(peer)
    scoreCache[peer.peerId] = value
    value

  proc comparePeers(a, b: PubSubPeer): int =
    system.cmp(cachedScore(a), cachedScore(b))
  ...
  if npeers < g.parameters.dLow:
    ...
    # sort peers by effective score, high score first since we graft
    candidates.sort(comparePeers, SortOrder.Descending)
    ...
    # sort peers by effective score, high score first, we are grafting
    candidates.sort(comparePeers, SortOrder.Descending)
  ...
  if npeers > g.parameters.dHigh:
    ...
    # sort peers by effective score (inverted), pruning, so low score peers are on top
    prunes.sort(comparePeers, SortOrder.Ascending)
  ...
  if g.mesh.peers(topic) > 1:
    ...
    # grafting so high effective score has priority
    peers.sort(comparePeers, SortOrder.Descending)
...
proc getGossipPeers*(g: GossipSub): Table[PubSubPeer, ControlMessage] =
  ...
  for topic in topics:
    ...
    for peer in allPeers:
      withMapEntry(control, peer, ControlMessage(), message):
        message.ihave.add(ihave)
      for msgId in ihave.messageIDs:
...
when defined(libp2p_gossipsub_1_4):
  proc preambleExpirationHeartbeat*(
    ...
    heartbeat "GossipSub: Preamble Expiration", chronos.milliseconds(200):
      trace "running preamble expiration heartbeat", instance = cast[int](g)

      while true:
        let expiredOpt = g.ongoingReceives.popExpired(Moment.now())
        if expiredOpt.isNone():
          break
        let expiredOngoingReceive = expiredOpt.get()
        ...
        if PullOperation:
          ...
          let bytesPerSecond = g.bandwidthEstimate(peer)
          let transmissionTimeMs = calculateReceiveTimeMs(
            expiredOngoingReceive.messageLength.int64, max(bytesPerSecond, 1.0).int64
          )
          let expires = starts + transmissionTimeMs.milliseconds
      ...
      block processIwants:
        while true:
          let expiredOpt = g.ongoingIWantReceives.popExpired(Moment.now())
          if expiredOpt.isNone():
            break
          let expiredOngoingIWantReceived = expiredOpt.get()
