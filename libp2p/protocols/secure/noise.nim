...
proc normalizeMuxers(muxers: openArray[string]): seq[string] =
  ## Deduplicate and clamp the advertised muxer list while preserving order.
  result = @[]
  for muxer in muxers:
    if muxer.len == 0:
      continue
    ...
    if not seen:
      result.add(muxer)
      if result.len >= MaxInlineMuxers:
        break

proc selectNegotiatedMuxer(
    localMuxers, remoteMuxers: seq[string], initiator: bool
): string =
  ## Return the first common muxer following the initiator/responder preference rules.
  if localMuxers.len == 0 or remoteMuxers.len == 0:
    return ""

  if initiator:
    for local in localMuxers:
      for remote in remoteMuxers:
        if local == remote:
          return local
  else:
    for remote in remoteMuxers:
...
method handshake*(
  ...
  if p.supportedMuxers.len > 0:
    var extensions = initProtoBuffer()
    for muxer in p.supportedMuxers:
      extensions.write(2, muxer)
    extensions.finish()
    libp2pProof.write(4, extensions)

  # data field also there but not used otherwise!
  libp2pProof.finish()
  ...
  var secure =
    try:
      ...
      if not remoteSig.verify(verifyPayload, remotePubKey):
        ...
        var extensionsPb: ProtoBuffer
        if remoteProof.getField(4, extensionsPb).valueOr(false):
          var muxerEntries: seq[string]
          let muxRes = extensionsPb.getRepeatedField(2, muxerEntries)
          if muxRes.isErr():
            trace "Noise handshake failed to decode muxer extensions", conn,
              error = $muxRes.error
          elif muxRes.valueOr(false):
            if muxerEntries.len <= MaxInlineMuxers:
              remoteMuxers = normalizeMuxers(muxerEntries)
            else:
              trace "Noise handshake ignoring oversize muxer list", conn,
                count = muxerEntries.len
      ...
      let negotiatedMuxer =
        selectNegotiatedMuxer(p.supportedMuxers, remoteMuxers, initiator)
      if negotiatedMuxer.len > 0:
        conn.setNegotiatedMuxer(negotiatedMuxer)
        trace "Noise handshake negotiated muxer inline", conn,
          negotiatedMuxer
      ...
      tmp.setNegotiatedMuxer(conn.negotiatedMuxer)
      tmp
    finally:
...
proc new*(
  ...
  var noise = Noise(
    ...
    supportedMuxers: normalizeMuxers(supportedMuxers),
  )
