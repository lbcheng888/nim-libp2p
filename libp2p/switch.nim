...
proc stop*(s: Switch) {.public, async: (raises: [CancelledError]).} =
  ...
  for service in s.services:
    let stopFuture = service.stop(s)
    if stopFuture.isNil:
      let serviceType = when compiles(service.type.name): service.type.name else: "unknown"
      warn "service.stop returned nil Future", serviceType = serviceType
    else:
      discard await stopFuture

  # close and cleanup all connections
  let closeFuture = s.connManager.close()
  if closeFuture.isNil:
    warn "connManager.close returned nil Future"
  else:
    await closeFuture

  for transp in s.transports:
    try:
      let stopFuture = transp.stop()
      if stopFuture.isNil:
        warn "transport.stop returned nil Future", transportAddr = cast[int](transp)
      else:
        await stopFuture
    except CancelledError as exc:
      raise exc
