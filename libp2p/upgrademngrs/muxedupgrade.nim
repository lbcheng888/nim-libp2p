...
method upgrade*(
  ...
  when defined(libp2p_msquic_debug):
    debug "MuxedUpgrade secure begin", conn, direction = conn.dir
  let protectedConn =
  ...
  when defined(libp2p_msquic_debug):
    debug "MuxedUpgrade secure complete", sconn, peerId = sconn.peerId
