...
const
  ...
  MdnsUnicastResponse = 0x8000'u16
...
proc sendUnicast(
  ...
  if self.sockV4.isNil or payload.len == 0:
    mdnsLog("sendUnicast skipped socketNil=" & $(self.sockV4.isNil) & " payloadLen=" & $payload.len)
    info "mdns send unicast skipped", socketNil = (self.sockV4.isNil), payloadLen = payload.len
    return
  ...
  try:
    ...
    localDesc = "err:" & exc.msg
  mdnsLog("sendUnicast attempting bytes=" & $payload.len & " target=" & $remote)
  info "mdns send unicast attempt", bytes = payload.len, target = $remote, local = localDesc
  try:
    await self.sockV4.sendTo(remote, unsafeAddr payload[0], payload.len)
    mdnsLog("sendUnicast bytes=" & $payload.len & " target=" & $remote)
    info "mdns send unicast", bytes = payload.len, target = $remote, local = localDesc
  except CatchableError as exc:
    trace "mdns: failed to send unicast datagram", error = exc.msg
    warn "mdns unicast send failed", err = exc.msg
...
proc handleQuery(
    self: MdnsInterface, questions: seq[DnsQuestion], remote: TransportAddress
  ) {.async: (raises: [CancelledError, Exception]).} =
  var shouldAnnounce = false
  var announceServices: seq[string] = @[]
  var preferUnicast = false
  var allQueriesUnicast = true
  var forceUnicast = false
  ...
  for q in questions:
    ...
    let unicastRequested =
      ((q.qclass and MdnsUnicastResponse) != 0'u16) or (remote.port != MdnsPort)
    if not unicastRequested:
      allQueriesUnicast = false
    if (unicastRequested or forceUnicast) and not preferUnicast:
      preferUnicast = true
      mdnsLog("handleQuery prefer unicast name=" & q.name & " remote=" & $remote)
    let queriedService =
      if q.name.len > 0:
        q.name
      else:
    ...
    of PtrType:
      if queryCanonical == serviceCanonical:
        ...
        try:
          ...
          response.setLen(0)
        if response.len > 0:
          if unicastRequested or forceUnicast:
            await self.sendUnicast(response, remote)
          if not unicastRequested:
  ...
  if shouldAnnounce:
    ...
    for svc in announceServices:
      ...
      if preferUnicast:
        await self.sendUnicast(payload, remote)
      if not allQueriesUnicast:
...
proc processResponse(
  ...
  if (flags and 0x8000'u16) == 0:
    asyncSpawn self.handleQuery(questions, remote)
  else:
...
proc processDatagram(
  ...
  try:
    ...
    self.processResponse(data, remote)
