...
type
  ...
  ConnectionState = ref object of QuicHandleState
    ...
    transcript: seq[byte]
    serverHandshakeFlightState: uint8
    ...
    deferredProbeDeadlineUs: array[CryptoEpoch, uint64]
    maintenanceSignal: ThreadSignalPtr
    maintenanceLoop: Future[void]
    maintenanceClosed: bool
    createdAtUs: uint64
    lastPeerActivityUs: uint64
...
const
  ...
  ServerHandshakeFlightNotStarted = 0'u8
  ServerHandshakeFlightPreparing = 1'u8
  ServerHandshakeFlightPrepared = 2'u8
...
proc quicVarIntEncodedLen(val: uint64): int {.inline.} =
  if val <= 63'u64:
    1
  elif val <= 16_383'u64:
    2
  elif val <= 1_073_741_823'u64:
    4
  else:
    8

proc connectionMaxUdpPayloadBytes(conn: ConnectionState): int {.gcsafe, inline.} =
  var resultBytes = int(DefaultCongestionDatagramBytes)
  if not conn.isNil and conn.peerTransportParams.maxUdpPayloadSize > 0'u64:
    let peerLimit = int(min(conn.peerTransportParams.maxUdpPayloadSize, high(int).uint64))
    resultBytes = min(resultBytes, peerLimit)
  resultBytes

proc oneRttPacketOverheadBytes(conn: ConnectionState): int {.gcsafe, inline.} =
  if conn.isNil:
    return 0
  1 + int(conn.peerCid.length) + 4 + 16

proc maxOneRttStreamDataLen(conn: ConnectionState; streamId, offset: uint64;
    remainingBytes: int): int {.gcsafe.} =
  if conn.isNil or remainingBytes <= 0:
    return 0
  let packetBudget = conn.connectionMaxUdpPayloadBytes() - conn.oneRttPacketOverheadBytes()
  if packetBudget <= 0:
    return 0
  let frameFixedOverhead =
    1 + quicVarIntEncodedLen(streamId) +
    (if offset > 0'u64: quicVarIntEncodedLen(offset) else: 0)
  if packetBudget <= frameFixedOverhead:
    return 0

  var candidate = min(remainingBytes, packetBudget - frameFixedOverhead)
  var previous = -1
  while candidate > 0 and candidate != previous:
    previous = candidate
    let lengthFieldBytes = quicVarIntEncodedLen(uint64(candidate))
    candidate = min(remainingBytes, packetBudget - frameFixedOverhead - lengthFieldBytes)
  max(candidate, 0)
...
proc effectiveTimeoutUs(timeoutMs: uint64): uint64 {.inline, gcsafe, raises: [].} =
  timeoutMs * 1_000'u64

proc peerActivityBaseUs(conn: ConnectionState): uint64 {.gcsafe, raises: [].} =
  if conn.isNil:
    return 0'u64
  if conn.lastPeerActivityUs > 0'u64:
    return conn.lastPeerActivityUs
  conn.createdAtUs

proc markPeerActivity(conn: ConnectionState; atUs: uint64 = 0'u64) {.gcsafe, raises: [].} =
  if conn.isNil:
    return
  let effective =
    if atUs > 0'u64: atUs
    else: nowMicros()
  conn.lastPeerActivityUs = effective
...
proc clearDeferredProbeDeadline(conn: ConnectionState; epoch: CryptoEpoch) {.gcsafe, inline.} =
  if conn.isNil:
    return
  conn.deferredProbeDeadlineUs[epoch] = 0'u64
...
proc clearConnectionRecoveryState(conn: ConnectionState) {.gcsafe.} =
  ...
  for epoch in CryptoEpoch:
    ...
    if not conn.quicConn.isNil:
      conn.quicConn.model.packetSpaces[epoch].lossTime = 0'u64
  if not conn.quicConn.isNil:
    conn.quicConn.model.timers.lossDetectionTimer = 0'u64

proc closeConnectionDueToLocalTimeout(conn: ConnectionState; note: string) {.gcsafe, raises: [].} =
  if conn.isNil:
    return
  if not conn.closeReason.isEmptyOrWhitespace():
    if conn.closeReason == note:
      return
  conn.started = false
  conn.handshakeComplete = false
  conn.closeReason = note
  conn.stopConnectionMaintenance()
  conn.clearConnectionRecoveryState()
...
proc handshakeIdleDeadlineUs(conn: ConnectionState): uint64 {.gcsafe, raises: [].} =
  if conn.isNil or conn.handshakeComplete:
    return 0'u64
  let timeoutUs = effectiveTimeoutUs(conn.settingsOverlay.handshakeIdleTimeoutMs)
  if timeoutUs == 0'u64:
    return 0'u64
  let baseUs = conn.peerActivityBaseUs()
  if baseUs == 0'u64:
    return 0'u64
  baseUs + timeoutUs

proc idleDeadlineUs(conn: ConnectionState): uint64 {.gcsafe, raises: [].} =
  if conn.isNil or not conn.handshakeComplete:
    return 0'u64
  let configuredUs = effectiveTimeoutUs(conn.settingsOverlay.idleTimeoutMs)
  if configuredUs == 0'u64:
    return 0'u64
  let baseUs = conn.peerActivityBaseUs()
  if baseUs == 0'u64:
    return 0'u64
  let minIdleUs = 3'u64 * conn.currentProbeTimeoutUs(ceOneRtt)
  baseUs + max(configuredUs, minIdleUs)

proc maybeHandleConnectionTimeout(conn: ConnectionState; nowUs: uint64): bool {.gcsafe, raises: [].} =
  if conn.isNil:
    return false
  let handshakeDeadline = conn.handshakeIdleDeadlineUs()
  if handshakeDeadline > 0'u64 and nowUs >= handshakeDeadline:
    conn.closeConnectionDueToLocalTimeout("handshake idle timeout")
    return true
  let idleDeadline = conn.idleDeadlineUs()
  if idleDeadline > 0'u64 and nowUs >= idleDeadline:
    conn.closeConnectionDueToLocalTimeout("idle timeout")
    return true
  false
...
proc selectLossDetectionSchedule(conn: ConnectionState; epoch: var CryptoEpoch;
  ...
  for candidate in CryptoEpoch:
    ...
    let deferredProbeUs = conn.deferredProbeDeadlineUs[candidate]
    if latestSentUs == 0'u64 and deferredProbeUs == 0'u64:
      continue
    let ptoUs = conn.currentProbeTimeoutUs(candidate)
    var candidateTimer =
      if latestSentUs == 0'u64: deferredProbeUs
      else: latestSentUs + ptoUs
    if deferredProbeUs > candidateTimer:
      candidateTimer = deferredProbeUs
...
proc connectionMaintenanceDeadlineUs(conn: ConnectionState): uint64 {.gcsafe, raises: [].} =
  if conn.isNil:
    return 0'u64
  let lossTimer =
  ...
  let idleDeadline = conn.idleDeadlineUs()
  var earliest = 0'u64
  for candidate in [lossTimer, ackDeadline, handshakeDeadline, idleDeadline]:
    if candidate == 0'u64:
      continue
    if earliest == 0'u64 or candidate < earliest:
      earliest = candidate
  earliest
...
proc findAcceptedConnectionByInitialTuple(state: ListenerState;
    srcCidBytes, destCidBytes: openArray[byte]): ConnectionState {.gcsafe.} =
  if state.isNil:
    return nil
  for conn in state.acceptedConnections.values:
    if conn.initialPacketMatchesConnection(srcCidBytes, destCidBytes):
      return conn
  nil
...
proc handleStatelessReset(conn: ConnectionState; remote: TransportAddress;
  ...
  conn.clearConnectionRecoveryState()
  let effectiveOwner =
    if not owner.isNil: owner
    else: conn.listenerOwner
...
proc currentLossDelayUs(conn: ConnectionState): uint64 {.gcsafe, raises: [].} =
  if conn.isNil:
...
proc persistentCongestionWindowUs(conn: ConnectionState; epoch: CryptoEpoch): uint64 {.gcsafe, raises: [].} =
  if conn.isNil:
...
proc isPersistentCongestion(conn: ConnectionState; epoch: CryptoEpoch; earliestSentUs,
    latestSentUs: uint64): bool {.gcsafe, raises: [].} =
...
proc detectTimedOutLosses(conn: ConnectionState) {.gcsafe.} =
  ...
  defer:
    conn.inLossDetectionTick = false
  let nowUs = nowMicros()
  if conn.maybeHandleConnectionTimeout(nowUs):
    return
  ...
  if not dueToLossTime:
    ...
    if probeCountSent == 0'u8:
      conn.deferredProbeDeadlineUs[selectedEpoch] = nowUs + probeTimeoutUs
    else:
      conn.clearDeferredProbeDeadline(selectedEpoch)
    {.cast(gcsafe).}:
      emitDiagnostics(DiagnosticsEvent(
        kind: diagConnectionEvent,
        handle: cast[pointer](conn),
...
proc releaseHandle(handle: HQUIC) =
  ...
  withApiStateLock:
    ...
    if registered:
      ...
      releaseRoot = true
  if releaseRoot:
    GC_unref(cast[QuicHandleState](handle))

proc generateConnectionIds(): (ConnectionId, ConnectionId) =
  let base = withApiStateLock:
    let current = gNextConnectionId
...
proc safeEmitConnectionDiagnostic(conn: ConnectionState; note: string) {.gcsafe, raises: [].}
...
proc sendOneRttPacket(conn: ConnectionState; payload: seq[byte];
  ...
  if not conn.inLossDetectionTick:
    conn.detectTimedOutLosses()
  let projectedPacketBytes = conn.oneRttPacketOverheadBytes() + payload.len
  if projectedPacketBytes > conn.connectionMaxUdpPayloadBytes():
    {.cast(gcsafe).}:
      emitDiagnostics(DiagnosticsEvent(
        kind: diagConnectionEvent,
        handle: cast[pointer](conn),
        note: "TX exceeds max_udp_payload_size=" &
          $conn.connectionMaxUdpPayloadBytes() &
          " projected=" & $projectedPacketBytes
      ))
    return 0'u64
  ...
  if ackEliciting:
    ...
    if allowance > 0 and uint64(projectedPacketBytes) > allowance:
      {.cast(gcsafe).}:
        emitDiagnostics(DiagnosticsEvent(
          kind: diagConnectionEvent,
          handle: cast[pointer](conn),
...
proc sendInitialPacket(conn: ConnectionState; payload: seq[byte];
  ...
  let sendRes = sendPacketImmediate(conn.transport, sendRemote, packet)
  if not sendRes.ok:
    {.cast(gcsafe).}:
      emitDiagnostics(DiagnosticsEvent(
        kind: diagConnectionEvent,
        handle: cast[pointer](conn),
        note: "TX Initial failed kind=" & $frameKind &
          " remote=" & $sendRemote &
          " err=" & sendRes.err
      ))
    return 0'u64
  ...
  if not sendRes.ok:
    {.cast(gcsafe).}:
      emitDiagnostics(DiagnosticsEvent(
        kind: diagConnectionEvent,
        handle: cast[pointer](conn),
        note: "TX Handshake failed kind=" & $frameKind &
          " remote=" & $sendRemote &
          " err=" & sendRes.err
      ))
    return 0'u64
...
proc applyAckFrame(conn: ConnectionState; ack: proto.AckFrame; epoch: CryptoEpoch) {.gcsafe.} =
  ...
  conn.clearDeferredProbeDeadline(epoch)
  if preserveEpochProbeBackoff:
...
proc sendProbeForEpoch(conn: ConnectionState; epoch: CryptoEpoch): uint8 {.gcsafe.} =
  ...
  let probeClientContext =
    if reuseOriginalFrame and probeMeta.frameKind in {sfkStream, sfkDatagram}:
      ...
      nil
  template probeTargetPtr(): untyped =
    (if haveProbeMeta and probeMeta.targetRemote.family != AddressFamily.None:
      addr probeMeta.targetRemote
    else:
      nil)
  proc sendProbePayload(payload: seq[byte]; frameKind: SentFrameKind): uint64 {.gcsafe.} =
    case epoch
    of ceInitial:
      ...
      if not conn.handshakeComplete:
        {.cast(gcsafe).}:
          emitDiagnostics(DiagnosticsEvent(
            kind: diagConnectionEvent,
            handle: cast[pointer](conn),
            note: "pto probe suppressed because ceOneRtt selected before handshake complete"
          ))
        return 0'u64
  ...
  for _ in 0 ..< 2:
    var packetNumber = sendProbePayload(probePayload, probeFrameKind)
    if packetNumber == 0'u64 and reuseOriginalFrame:
      {.cast(gcsafe).}:
        emitDiagnostics(DiagnosticsEvent(
          kind: diagConnectionEvent,
          handle: cast[pointer](conn),
          note: "pto retransmit failed; falling back to ping epoch=" & $epoch &
            " frameKind=" & $probeFrameKind
        ))
      packetNumber = sendProbePayload(genericProbePayload, sfkPing)
...
proc newAcceptedConnection(state: ListenerState; remote, local: TransportAddress;
  ...
  conn.settingsOverlay = state.settingsOverlay
  ...
  conn.transcript = @[]
  conn.serverHandshakeFlightState = ServerHandshakeFlightNotStarted
  conn.handshakeComplete = false
  ...
  conn.createdAtUs = createdAtUs
  conn.lastPeerActivityUs = createdAtUs
...
proc handleHandshakePacket(conn: ConnectionState; remote: TransportAddress; data: seq[byte]) =
  ...
  if plaintext.len == 0 and ciphertext.len > 0:
    {.cast(gcsafe).}:
      emitDiagnostics(DiagnosticsEvent(
        kind: diagConnectionEvent,
        handle: cast[pointer](conn),
        note: "RX Handshake decrypt failed pn=" & $pnVal &
          " bytes=" & $data.len &
          " cipher=" & $ciphertext.len
      ))
    return
  ...
  var ackEliciting = false
  var sentImmediateHandshakeAck = false
  var pos = 0
  while pos < plaintext.len:
    let frameType = plaintext[pos]
    if frameType == 0x06'u8:
      ...
      inc pos
  if ackEliciting and not sentImmediateHandshakeAck:
    conn.markConnectionPacketForAck(ceHandshake, uint64(pnVal), nowUs, ackTypeAckImmediate)
    discard conn.sendAckFrame(uint64(pnVal), ceHandshake, unsafeAddr remote)
...
proc handleInitialPacket(state: ListenerState; remote, local: TransportAddress; data: seq[byte]) =
  ...
  while pos < plaintext.len:
    ...
    if frameType == 0x06'u8:
      ...
      let ackConn =
        block:
          let byTuple = state.findAcceptedConnectionByInitialTuple(
            header.srcConnectionId,
            header.destConnectionId
          )
          if not byTuple.isNil: byTuple
          else:
  ...
  if not isNewConnection and
      conn.serverHandshakeFlightState != ServerHandshakeFlightNotStarted:
    emitDiagnostics(DiagnosticsEvent(
      kind: diagConnectionEvent,
      handle: cast[pointer](conn),
      note: "RX Initial on existing connection; server flight state=" &
        $conn.serverHandshakeFlightState & " so only ACK is sent."
    ))
    if ackEliciting:
      discard conn.sendAckFrame(uint64(pnVal), ceInitial, unsafeAddr remote)
    return
  ...
  if not conn.handshakeComplete:
    conn.serverHandshakeFlightState = ServerHandshakeFlightPreparing
    ...
    conn.serverHandshakeFlightState = ServerHandshakeFlightPrepared
    ...
    var initialPayload: seq[byte] = @[]
    let initialAckPackets = qack.packetNumbersToAck(conn.ackTracker, ceInitial)
    if initialAckPackets.len > 0:
      var initialAckRanges: seq[proto.AckRange] = @[]
      for packetRange in initialAckPackets:
        initialAckRanges.add(proto.AckRange(
          smallest: packetRange.first,
          largest: packetRange.last
        ))
      initialPayload.add(proto.encodeAckFrame(initialAckRanges, 0'u64))
      qack.consumePendingAck(conn.ackTracker, ceInitial)
    initialPayload.add(buildCryptoFrame(serverHello))
        ...
        if not conn.isNil:
          ...
          {.cast(gcsafe).}:
            emitDiagnostics(DiagnosticsEvent(
              kind: diagConnectionEvent,
              handle: cast[pointer](state),
              note: "RX Handshake without matching connection len=" & $packetData.len &
                " remote=" & $remote
            ))
      else:
...
proc msquicConnectionOpen(registration: HQUIC; handler: QuicConnectionCallback;
  ...
  state.createdAtUs = nowMicros()
  state.lastPeerActivityUs = 0'u64
  let raw = storeHandle(state)
...
proc msquicConnectionSetConfiguration(connection: HQUIC;
  ...
  state.settingsOverlay = config.settingsOverlay
  state.datagramReceiveEnabled = state.settingsOverlay.datagramReceiveEnabled
  QUIC_STATUS_SUCCESS
...
proc attachConfiguration(state: ConnectionState; configuration: ConfigurationState): QUIC_STATUS =
  ...
  state.settingsOverlay = configuration.settingsOverlay
  state.datagramReceiveEnabled = state.settingsOverlay.datagramReceiveEnabled
  QUIC_STATUS_SUCCESS
...
proc msquicConnectionStart(connection: HQUIC; configuration: HQUIC;
  ...
  discard family
  state.started = true
  state.settingsOverlay = state.configuration.settingsOverlay
  state.datagramReceiveEnabled = state.settingsOverlay.datagramReceiveEnabled
  
  # Initialize Transport (Client)
  var transportEnabled = false
  try:
    # Resolve remote address. When the caller preconfigures QUIC_PARAM_CONN_REMOTE_ADDRESS
    # before ConnectionStart, keep that transport target and use serverName only for TLS
    # identity/resumption.
    var dialRemote = state.remoteAddress
    if dialRemote.family == AddressFamily.None or dialRemote.port == Port(0):
      # For Phase 1, we assume serverName is an IP string. DNS resolution is skipped for now/handled by caller if needed.
      dialRemote = initTAddress(state.serverName, Port(state.serverPort))
    try:
      state.remoteAddress = dialRemote
        ...
        (proc (transp: DatagramTransport, remote: TransportAddress) {.async.} =
          ...
          try:
            for packetData in proto.splitCoalescedPackets(data):
              ...
              if not state.connectionMatchesDestCid(packetData):
                let packetKind =
                  if (header.firstByte and 0x80'u8) != 0'u8:
                    let pType = (header.firstByte and 0x30'u8) shr 4
                    case pType
                    of 0'u8: "Initial"
                    of 1'u8: "0-RTT"
                    of 2'u8: "Handshake"
                ...
                {.cast(gcsafe).}:
                  emitDiagnostics(DiagnosticsEvent(
                    ...
                    note: "RX " & packetKind &
                      " dropped due to CID mismatch len=" & $packetData.len &
                      " remote=" & $remote
                  ))
                continue
    ...
    # CRYPTO Frame + padding. Initial packets must be >= 1200 bytes.
    var payload = buildCryptoFrame(clientHello)
    let minSize = 1200
    ...
    if transportEnabled and not state.transport.isNil:
      let initialPn = state.sendInitialPacket(payload, sfkCrypto, true)
      if initialPn != 0'u64:
        emitDiagnostics(DiagnosticsEvent(
          kind: diagConnectionEvent,
          handle: connection,
          note: "TX Initial Encrypted"
        ))
      else:
        emitDiagnostics(DiagnosticsEvent(
          kind: diagConnectionEvent,
          handle: connection,
          note: "TX Initial Failed"
        ))
    else:
      emitDiagnostics(DiagnosticsEvent(
        kind: diagConnectionEvent,
  ...
  emitDiagnostics(DiagnosticsEvent(
    kind: diagConnectionStarted,
    handle: connection,
    note: state.serverName & ":" & $state.serverPort & " remote=" & $state.remoteAddress))
  QUIC_STATUS_SUCCESS
...
proc flushStream(state: StreamState) {.gcsafe.} =
  ...
  if conn.transport.isNil:
    ...
    return
  while state.pendingChunks.len > 0:
    var chunk = state.pendingChunks[0]
    let sendLen =
      if chunk.payload.len == 0:
        0
      else:
        conn.maxOneRttStreamDataLen(state.streamId, chunk.offset, chunk.payload.len)
    if chunk.payload.len > 0 and sendLen <= 0:
      when defined(ohos):
        let sendMat = connectionSendMaterial(conn)
        warn "MsQuic flushStream deferred stream_segment_budget_exhausted",
          stream = cast[uint64](state),
          streamId = state.streamId,
          pending = state.pendingChunks.len,
          attempts = state.flushRetryAttempts,
      ...
      scheduleFlushStreamRetry(state, "stream_segment_budget_exhausted")
      return

    let isFinalSegment = chunk.payload.len == 0 or sendLen >= chunk.payload.len
    let segmentPayload =
      if chunk.payload.len == 0:
        @[]
      elif isFinalSegment:
        ...
        chunk.payload[0 ..< sendLen]
    let segmentFin = chunk.fin and isFinalSegment
    ...
    let pn = conn.sendOneRttPacket(
      ...
      streamPayload = segmentPayload,
      streamFin = segmentFin,
    ...
    if isFinalSegment:
      ...
      state.pendingChunks[0].offset = chunk.offset + uint64(segmentPayload.len)
      state.pendingChunks[0].payload = chunk.payload[sendLen .. ^1]
      state.pendingChunks[0].fin = chunk.fin
      state.pendingChunks[0].clientContext = chunk.clientContext
      state.pendingChunks[0].targetRemote = chunk.targetRemote
      state.pendingChunks[0].zeroRttDispatched = false
...
proc applyConfigurationSettingsOverlay*(configuration: HQUIC;
    overlay: QuicSettingsOverlay): QUIC_STATUS {.exportc.} =
  let state = toConfiguration(configuration)
  if state.isNil:
    return QUIC_STATUS_INVALID_PARAMETER
  state.settingsOverlay = overlay
...
proc getConfigurationSettingsOverlay*(configuration: HQUIC;
    overlay: var QuicSettingsOverlay): bool {.exportc.} =
  let state = toConfiguration(configuration)
  if state.isNil:
    return false
...
proc applyListenerSettingsOverlay*(listener: HQUIC;
    overlay: QuicSettingsOverlay): QUIC_STATUS {.exportc.} =
  let state = toListener(listener)
  if state.isNil:
    return QUIC_STATUS_INVALID_PARAMETER
...
proc getListenerSettingsOverlay*(listener: HQUIC;
    overlay: var QuicSettingsOverlay): bool {.exportc.} =
  let state = toListener(listener)
  if state.isNil:
    return false
...
proc setConnectionTimeoutsForTest*(connection: HQUIC;
    handshakeIdleTimeoutMs: uint64; idleTimeoutMs: uint64): bool =
  let state = toConnection(connection)
  if state.isNil:
    return false
  state.settingsOverlay.handshakeIdleTimeoutMs = handshakeIdleTimeoutMs
  state.settingsOverlay.idleTimeoutMs = idleTimeoutMs
  true

proc setConnectionPeerActivityForTest*(connection: HQUIC;
    createdAtUs: uint64; lastPeerActivityUs: uint64): bool =
  let state = toConnection(connection)
  if state.isNil:
    return false
  state.createdAtUs = createdAtUs
  state.lastPeerActivityUs = lastPeerActivityUs
  true

proc runConnectionMaintenanceForTest*(connection: HQUIC): bool =
  let state = toConnection(connection)
  if state.isNil:
    return false
