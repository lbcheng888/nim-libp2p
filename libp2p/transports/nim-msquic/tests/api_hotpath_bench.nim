...
proc runBench(name: string; iterations: int; body: proc ()) =
...
when isMainModule:
  proc main() =
    ...
    try:
      ...
      runBench("SetContext(表函数)", setContextIters, proc () =
        ctx.table.SetContext(ctx.connection, addr contextSlot)
      )

      runBench("SetContext(C Shim)", setContextIters, proc () =
        MsQuicSetContextShim(ctx.connection, addr contextSlot)
      )

      var toggleValue = BOOLEAN(0)
      runBench("SetParam(接收数据报)", datagramIters, proc () =
      ...
      runBench("C Shim(接收数据报)", datagramIters, proc () =
        toggleValue = (if (toggleValue == 0'u8): BOOLEAN(1) else: BOOLEAN(0))
      ...
      runBench("SetParam(关闭原因)", closeReasonIters, proc () =
