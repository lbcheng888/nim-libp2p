...
proc parseOnion3(
  ...
  if addressStr.len > 255:
    raise newException(LPError, "Onion address too long")
  let port =
    try:
      parseInt(portStr)
    except ValueError:
      raise newException(LPError, "Invalid onion3 port")
  if port <= 0 or port > high(uint16).int:
    raise newException(LPError, "Invalid onion3 port")
  let dstAddr = @[uint8(addressStr.len)] & addressStr.toBytes()
  let dstPort = @[
    byte((port shr 8) and 0xFF),
    byte(port and 0xFF),
  ]
...
proc parseDnsTcp(
  ...
  if dnsAddress.len > 255:
    raise newException(LPError, "DNS address too long for SOCKS5 request")
  let dstAddr = @(uint8(dnsAddress.len).toBytes()) & dnsAddress
  let dstPort = address[multiCodec("tcp")].tryGet().protoArgument().tryGet()
  (Socks5AddressType.FQDN.byte, dstAddr, dstPort)

proc socks5RequestParts*(address: MultiAddress): (byte, seq[byte], seq[byte]) {.raises: [LPError].} =
...
.} =
  let (atyp, dstAddr, dstPort) = socks5RequestParts(address)
...
proc new*(
  ...
  let torSwitch = T(
    ...
    dialer: Dialer.new(
      ...
      nil,
      switch.connectionGater,
    ),
    nameResolver: nil,
    connectionGater: switch.connectionGater,
  )
