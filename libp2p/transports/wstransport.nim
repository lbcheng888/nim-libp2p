...
method readOnce*(
  ...
  if res > 0 and not s.bandwidthManager.isNil and s.peerId.len > 0:
    await s.bandwidthManager.awaitLimit(
      s.peerId, Direction.In, res, s.protocol
    )
    s.bandwidthManager.record(s.peerId, Direction.In, res, s.protocol)
  return res
...
method write*(
  ...
  s.activity = true # reset activity flag
  if not s.bandwidthManager.isNil and s.peerId.len > 0 and msg.len > 0:
    s.bandwidthManager.record(s.peerId, Direction.Out, msg.len, s.protocol)
