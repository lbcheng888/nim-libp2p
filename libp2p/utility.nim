...
import std/[sets, options, macros, tables]
...
template withMapEntry*[K, V](
    table: var Table[K, V], key: K, defaultValue: V, entryName, body: untyped
) =
  block:
    var entryName {.inject.} = table.getOrDefault(key, defaultValue)
    body
    table[key] = entryName

template updateMapEntry*[K, V](
    table: var Table[K, V],
    key: K,
    defaultValue: V,
    entryName,
