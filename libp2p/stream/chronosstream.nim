...
import connection, ../bandwidthmanager
import ../utility
...
method readOnce*(
  ...
  if result > 0 and not s.bandwidthManager.isNil and s.peerId.len > 0:
    await s.bandwidthManager.awaitLimit(
      s.peerId, Direction.In, result, s.protocol
    )
    s.bandwidthManager.record(s.peerId, Direction.In, result, s.protocol)
...
proc completeWrite(
  ...
  withExceptions:
    ...
    if not s.bandwidthManager.isNil and s.peerId.len > 0 and msgLen > 0:
      s.bandwidthManager.record(s.peerId, Direction.Out, msgLen, s.protocol)
    when defined(libp2p_agents_metrics):
      s.trackPeerIdentity()
      if s.tracked:
        libp2p_peers_traffic_write.inc(msgLen.int64, labelValues = [s.shortAgent])

proc writeWithBandwidth(
    s: ChronosStream, msg: seq[byte]
): Future[void] {.async: (raises: [CancelledError, LPStreamError]).} =
  if not s.bandwidthManager.isNil and s.peerId.len > 0 and msg.len > 0:
    await s.bandwidthManager.awaitLimit(
      s.peerId, Direction.Out, msg.len, s.protocol
    )
  await s.completeWrite(s.client.write(msg), msg.len)
...
proc transport*(s: ChronosStream): StreamTransport =
  ## Expose underlying chronos stream transport.
  s.client
