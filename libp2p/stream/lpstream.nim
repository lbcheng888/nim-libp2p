...
type
  ...
  LPStreamError* = object of LPError
    ...
    hasQuicState*: bool
    quicAppError*: uint64
    hasQuicAppError*: bool
    quicStreamId*: uint64
    hasQuicStreamId*: bool
...
proc setQuicInfo*(
  ...
  if err.isNil:
    return
  err.hasQuicState = hasState
  if hasState:
    err.quicState = state
  err.hasQuicAppError = hasAppError
  if hasAppError:
    err.quicAppError = appError
  err.hasQuicStreamId = hasStreamId
  if hasStreamId:
    err.quicStreamId = streamId

proc quicDetails*(err: ref LPStreamError): string =
  if err.isNil:
    return ""
  var parts: seq[string] = @[]
  if err.hasQuicState:
    parts.add("state=" & $err.quicState)
  if err.hasQuicAppError:
    parts.add("app_error=" & $err.quicAppError)
  if err.hasQuicStreamId:
    parts.add("stream_id=" & $err.quicStreamId)
  if parts.len > 0:
    result = parts.join(" ")
  else:
    result = ""
