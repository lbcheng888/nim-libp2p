...
type
  ...
  ProtoBuffer* = object ## Protobuf's message representation object
    ...
    offset*: int
    length*: int
    pendingHeader: ProtoHeader
    hasPendingHeader: bool
...
proc init*(
    _: type[ProtoBuffer], cap: Natural = 0, options: set[ProtoFlags] = {}
): ProtoBuffer {.inline.} =
  ## Convenience constructor mirroring ``initProtoBuffer`` with optional capacity hint.
  result = initProtoBuffer(options)
  if cap > 0:
    let baseOffset = result.offset
    result.buffer = newSeqOfCap[byte](cap + baseOffset)
    result.buffer.setLen(baseOffset)

template varintField*(pb: var ProtoBuffer, field: uint, value: untyped) =
  pb.write(int(field), value)

template stringField*(pb: var ProtoBuffer, field: uint, value: string) =
  pb.write(int(field), value)

template bytesField*(pb: var ProtoBuffer, field: uint, value: openArray[byte]) =
  pb.write(int(field), value)

template bytesField*(pb: var ProtoBuffer, field: uint, value: seq[byte]) =
  pb.write(int(field), value)
...
proc write*[T: ProtoScalar](pb: var ProtoBuffer, field: int, value: T) =
  ...
  var length = 0
  var flength: int
  var header: ProtoFieldKind
  template varintEncodedLen(val: T): int =
    when T is uint64:
      vsizeof(val)
    elif T is uint32:
      vsizeof(val)
      ...
      vsizeof(val)
    elif T is uint16 or T is uint8:
      vsizeof(uint(val))
    elif T is zint64 or T is zint32 or T is zint:
      vsizeof(val)
    elif T is hint64 or T is hint32 or T is hint:
      vsizeof(val)
    elif T is int64:
      vsizeof(hint64(val))
    elif T is int32:
      ...
      {.error: "Unsupported varint subtype: " & $T.}
  when (T is uint64) or (T is uint32) or (T is uint) or (T is uint16) or (T is uint8) or
      (T is zint64) or (T is zint32) or (T is zint) or (T is hint64) or (T is hint32) or
      (T is hint) or (T is int64) or (T is int32) or (T is int):
    flength =
      vsizeof(getProtoHeader(field, ProtoFieldKind.Varint)) + varintEncodedLen(value)
    header = ProtoFieldKind.Varint
  elif T is float32:
    flength = vsizeof(getProtoHeader(field, ProtoFieldKind.Fixed32)) + sizeof(T)
    header = ProtoFieldKind.Fixed32
  elif T is float64:
    flength = vsizeof(getProtoHeader(field, ProtoFieldKind.Fixed64)) + sizeof(T)
    header = ProtoFieldKind.Fixed64
  else:
    {.error: "Unsupported ProtoScalar type: " & $T.}
...
proc toBytes*(pb: var ProtoBuffer): seq[byte] =
  ## Finalize ``pb`` and return a copy of the written bytes.
  pb.finish()
  let hasLengthPrefix =
    pb.options * {WithVarintLength, WithUint32LeLength, WithUint32BeLength} != {}
  let start =
    if hasLengthPrefix:
      pb.offset
    else:
      0
  if start <= len(pb.buffer):
    result = pb.buffer[start ..< len(pb.buffer)]
  else:
...
proc readFieldNumber*(pb: var ProtoBuffer): uint =
  ## Read next field number; returns 0 when the buffer is exhausted or invalid.
  if pb.hasPendingHeader:
    return uint(pb.pendingHeader.index)
  if pb.isEmpty():
    return 0'u
  var header: ProtoHeader
  if pb.getHeader(header).isOk():
    pb.pendingHeader = header
    pb.hasPendingHeader = true
    return uint(header.index)
  0'u

proc takePending(pb: var ProtoBuffer): (bool, ProtoHeader) =
  if pb.hasPendingHeader:
    pb.hasPendingHeader = false
    (true, pb.pendingHeader)
  else:
    (false, ProtoHeader())

proc readVarint*(pb: var ProtoBuffer): uint64 =
  let (okPending, header) = pb.takePending()
  if not okPending:
    return 0
  var value = uint64(0)
  if pb.getValue(header, value).isOk():
    value
  else:
    0

proc readString*(pb: var ProtoBuffer): string =
  let (okPending, header) = pb.takePending()
  if not okPending:
    return ""
  var value = ""
...
proc readBytes*(pb: var ProtoBuffer): seq[byte] =
  let (okPending, header) = pb.takePending()
  if not okPending:
    return @[]
  var value: seq[byte] = @[]
...
proc skipValue*(pb: var ProtoBuffer) =
  let (okPending, header) = pb.takePending()
  if not okPending:
    return
  if pb.skipValue(header).isErr():
    pb.offset = len(pb.buffer)
