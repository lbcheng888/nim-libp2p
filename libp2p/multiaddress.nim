...
proc certhashStB(s: string, vb: var VBuffer): bool =
  ## certhash stringToBuffer() implementation.
  if s.len == 0:
    return false
  let decoded = MultiBase.decode(s).valueOr:
    return false
  if not MultiHash.validate(decoded):
    return false
  vb.writeSeq(decoded)
  true

proc certhashBtS(vb: var VBuffer, s: var string): bool =
  ## certhash bufferToString() implementation.
  var payload = newSeqUninit[byte](0)
  if vb.readSeq(payload) <= 0:
    return false
  if not MultiHash.validate(payload):
    return false
  let encoded = MultiBase.encode("base64url", payload).valueOr:
    return false
  s = encoded
  true

proc certhashVB(vb: var VBuffer): bool =
  ## certhash validateBuffer() implementation.
  var payload = newSeqUninit[byte](0)
  if vb.readSeq(payload) <= 0:
    return false
  result = MultiHash.validate(payload)
...
const
  ...
  TranscoderCertHash* = Transcoder(
    stringToBuffer: certhashStB, bufferToString: certhashBtS, validateBuffer: certhashVB
  )
  ...
  ProtocolsList = [
    ...
    MAProtocol(
      mcodec: multiCodec("certhash"), kind: Length, size: 0, coder: TranscoderCertHash
    ),
    MAProtocol(
  ...
  WebRTCDirect* = mapAnd(UDP, mapEq("webrtc-direct"))
...
proc validate*(ma: MultiAddress): bool =
  ...
  var vb = ma
  const
    QuicCodec = multiCodec("quic")
    QuicV1Codec = multiCodec("quic-v1")
  ...
  var lastCodec = InvalidMultiCodec
  var allowCerthash = false
  while true:
    if vb.data.isEmpty():
      break
    if vb.data.readVarint(header) == -1:
    ...
    if proto.kind in {Fixed, Length, Path}:
      ...
      if not proto.coder.validateBuffer(vb.data):
        return false
    if proto.mcodec == WebtransportCodec:
      if (lastCodec != QuicCodec) and (lastCodec != QuicV1Codec):
        return false
      allowCerthash = true # allow optional certhash segments right after /webtransport
    elif proto.mcodec == WebrtcDirectCodec:
      if lastCodec != UdpCodec:
        return false
      allowCerthash = true # allow optional certhash segments right after /webrtc-direct
    elif proto.mcodec == CerthashCodec:
      if not allowCerthash:
        return false
    else:
      allowCerthash = false
    lastCodec = proto.mcodec
  result = true
