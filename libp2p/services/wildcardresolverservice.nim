...
proc isWildcardIpv4(bytes: seq[byte]): bool =
  bytes.len == 4 and bytes.allIt(it == 0'u8)

proc isWildcardIpv6(bytes: seq[byte]): bool =
  bytes.len == 16 and bytes.allIt(it == 0'u8)
...
proc isPreferredInterface(iface: NetworkInterface): bool =
  if iface.name.len == 0:
    return false
  let lower = iface.name.toLowerAscii()
  if iface.ifType == IfIeee80211:
    return true
  if lower.startsWith("wlan") or lower.contains("wifi"):
    return true
  if lower.startsWith("eth") or lower.startsWith("en") or lower.startsWith("lan"):
    return true
  if lower.startsWith("ap") or lower.startsWith("softap") or lower.contains("hotspot"):
    return true
  false

proc getAddresses(addrFamily: AddressFamily): seq[InterfaceAddress] =
  let interfaces = getInterfaces()
  var preferred: seq[NetworkInterface]
  var fallback: seq[NetworkInterface]
  for iface in interfaces:
    if iface.state != StatusUp:
      continue
    if iface.ifType == IfSoftwareLoopback:
      continue
    if isPreferredInterface(iface):
      preferred.add(iface)
    else:
      fallback.add(iface)

  var selected = if preferred.len > 0: preferred else: fallback
  if selected.len == 0:
    selected = interfaces.filterIt(it.isLoopbackOrUp())

  for iface in selected:
    for ifaddr in iface.addresses:
      if ifaddr.host.family != addrFamily:
        continue
      if ifaddr.host.isAnyLocal() or ifaddr.host.isLoopback() or ifaddr.host.isLinkLocal():
        continue
      result.add(ifaddr)
