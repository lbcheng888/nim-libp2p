...
import std/[atomics, base64, json, locks, math, options, sequtils, sets, strformat,
...
proc nim_thread_attach*() {.exportc, cdecl, dynlib.} =
  ## 注册从 Kotlin/Java 进入 Nim 运行时时所在线程的 GC 上下文。
  ## Nim 会针对每个线程做一次幂等初始化，避免 GC 扫描未注册线程导致崩溃。
  setupForeignThreadGc()

proc nim_thread_detach*() {.exportc, cdecl, dynlib.} =
  ## 在不再需要访问 Nim 的线程上拆卸 GC 上下文。
  ## 当前 Android 端暂未调用，仅作为调试/单元测试辅助接口。
...
type
  ...
  NimNode* = ref object
    ...
    connectedPeersSnapshot: HashSet[string]
    connectedPeersLock: Lock
    connectedPeersUpdatedMs: int64
    connectedPeersInfoCache: string
    connectedPeersInfoLock: Lock
    eventLogLock: Lock
    ...
    feedItemsLock: Lock
    peerMultiaddrsCache: Table[string, seq[string]]
    peerMultiaddrsLock: Lock
    ...
    mdnsReady: bool
    mdnsRestarting: bool
    mdnsDebugCache: string
    mdnsDebugLock: Lock

const CommandPoolLimit = 128

var commandPoolLock: Lock
var commandPool: seq[Command] = @[]

when not defined(nimscript):
  initLock(commandPoolLock)
...
proc acquireCommand(kind: CommandKind): Command =
  ...
  if commandPool.len > 0:
    cmd = commandPool.pop()
  commandPoolLock.release()
  var fresh = false
  if cmd.isNil:
    fresh = true
    new(cmd)
...
proc updateConnectedPeersSnapshot(n: NimNode, peerId: PeerId, connected: bool) =
  let peerStr = $peerId
  if peerStr.len == 0:
    return
  n.connectedPeersLock.acquire()
  if connected:
    n.connectedPeersSnapshot.incl(peerStr)
  else:
    n.connectedPeersSnapshot.excl(peerStr)
  n.connectedPeersUpdatedMs = nowMillis()
  n.connectedPeersLock.release()

proc refreshConnectedPeersSnapshot(n: NimNode) =
  if n.switchInstance.isNil:
    return
  var snapshot = initHashSet[string]()
  ...
  for pid in outbound:
    snapshot.incl($pid)
  n.connectedPeersLock.acquire()
  n.connectedPeersSnapshot = snapshot
  n.connectedPeersUpdatedMs = nowMillis()
  n.connectedPeersLock.release()

proc snapshotConnectedPeers(n: NimNode): seq[string] =
  n.connectedPeersLock.acquire()
  let snapshot = n.connectedPeersSnapshot.toSeq()
  n.connectedPeersLock.release()
  snapshot

proc snapshotConnectedPeersNonBlocking(n: NimNode): seq[string] =
  if n.isNil:
    return @[]
  if not n.connectedPeersLock.tryAcquire():
    return @[]
  let snapshot = n.connectedPeersSnapshot.toSeq()
  n.connectedPeersLock.release()
  snapshot

proc updatePeerMultiaddrsCache(n: NimNode, peerId: string, addrs: seq[string]) =
  if n.isNil or peerId.len == 0:
    return
  n.peerMultiaddrsLock.acquire()
  n.peerMultiaddrsCache[peerId] = addrs
  n.peerMultiaddrsLock.release()

proc readPeerMultiaddrsCache(n: NimNode, peerId: string): seq[string] =
  if n.isNil or peerId.len == 0:
    return @[]
  n.peerMultiaddrsLock.acquire()
  let cached = n.peerMultiaddrsCache.getOrDefault(peerId)
  n.peerMultiaddrsLock.release()
  cached

proc updateConnectedPeersInfoCache(n: NimNode, payload: string) =
  if n.isNil:
    return
  n.connectedPeersInfoLock.acquire()
  n.connectedPeersInfoCache = payload
  n.connectedPeersInfoLock.release()

proc readConnectedPeersInfoCache(n: NimNode): string =
  if n.isNil:
    return "[]"
  n.connectedPeersInfoLock.acquire()
  let cached = if n.connectedPeersInfoCache.len > 0: n.connectedPeersInfoCache else: "[]"
  n.connectedPeersInfoLock.release()
  cached

proc updateMdnsDebugCache(n: NimNode, payload: string) =
  if n.isNil:
    return
  n.mdnsDebugLock.acquire()
  n.mdnsDebugCache = payload
  n.mdnsDebugLock.release()

proc readMdnsDebugCache(n: NimNode): string =
  if n.isNil:
    return "{}"
  n.mdnsDebugLock.acquire()
  let cached = if n.mdnsDebugCache.len > 0: n.mdnsDebugCache else: "{}"
  n.mdnsDebugLock.release()
  cached
...
when defined(libp2p_msquic_experimental):
  ...
  proc rejectionStatsToJson(
    ...
    result["missing_datagram"] = %stats.missingDatagram
    result["missing_session_accept"] = %stats.missingSessionAccept
    result["connect_rejected"] = %stats.connectRejected
    result["invalid_request"] = %stats.invalidRequest
  ...
  proc connectionSnapshotToJson(
    ...
    if snapshot.webtransportPath.len > 0:
      result["webtransport_path"] = %snapshot.webtransportPath
    if snapshot.webtransportSessionId != 0'u64:
  ...
  proc emitMsQuicStats(n: NimNode; reason: string) =
    ...
    if transportStats.len == 0:
      return
    var aggregate = newJObject()
    var totalListeners = 0
    var totalConnections = 0
    var totalDatagramEnabled = 0
    var totalWebtransportReady = 0
    var totalWebtransportPending = 0
    var transportsArr = newJArray()
    for idx, stats in transportStats:
      ...
      entry["datagram_disabled"] = %stats.datagramDisabled
      entry["webtransport_ready"] = %stats.webtransportReady
      entry["webtransport_pending"] = %stats.webtransportPending
      entry["webtransport_active_slots"] = %stats.webtransportActiveSlots.int
      entry["webtransport_session_limit"] = %stats.webtransportSlotLimit.int
      entry["ready_handshake_avg_seconds"] = %stats.readyHandshakeAverage
      entry["pending_handshake_avg_seconds"] = %stats.pendingHandshakeAverage
      if stats.currentCerthash.len > 0:
        entry["current_certhash"] = %stats.currentCerthash
      if stats.certhashHistory.len > 0:
        var historyArr = newJArray()
        let limit = min(stats.certhashHistory.len, MsQuicCerthashSampleLimit)
        for i in 0 ..< limit:
          historyArr.add(%stats.certhashHistory[i])
        entry["certhash_history"] = historyArr
      entry["rejections"] = rejectionStatsToJson(stats.rejection)
      let totalConnectionsForSample = stats.connections.len
      var connectionsArr = newJArray()
      let sampleLimit = min(totalConnectionsForSample, MsQuicConnectionSampleLimit)
      for i in 0 ..< sampleLimit:
        connectionsArr.add(connectionSnapshotToJson(stats.connections[i]))
      entry["connections"] = connectionsArr
      if totalConnectionsForSample > sampleLimit:
        entry["connection_sample_limit"] = %sampleLimit
        entry["connection_total"] = %totalConnectionsForSample
      transportsArr.add(entry)
      totalListeners += stats.listenerCount
      totalConnections += stats.connectionCount
      totalDatagramEnabled += stats.datagramEnabled
      totalWebtransportReady += stats.webtransportReady
      totalWebtransportPending += stats.webtransportPending
    aggregate["listener_count"] = %totalListeners
    aggregate["connection_count"] = %totalConnections
    aggregate["datagram_enabled"] = %totalDatagramEnabled
    aggregate["webtransport_ready"] = %totalWebtransportReady
    aggregate["webtransport_pending"] = %totalWebtransportPending
    var event = newJObject()
    event["type"] = %"MsQuicStats"
...
proc collectLanIpv4Addrs(preferred: string = ""): seq[string] =
  ...
  debugLog("[nimlibp2p] collectLanIpv4Addrs -> " & summary)
  result = filtered
...
proc collectLanIpv6Addrs(): seq[string] =
  ...
  for ip in seen.items:
    result.add(ip)
  if result.len == 0:
    info "collectLanIpv6Addrs", addrs = "<empty>"
  else:
    info "collectLanIpv6Addrs", addrs = result.join(",")
...
proc refreshLanAwareAddrs(n: NimNode): seq[MultiAddress] =
  ...
  if filtered > 0 and dedup.len == 0:
    debugLog("[nimlibp2p] refreshLanAwareAddrs dropped unspecified addresses; keep original listeners")
    return @[]
  elif filtered > 0:
...
proc canonicalizeMultiaddrText(text: string): string =
  ...
  if maRes.isOk():
    return $maRes.get()
  try:
    var buf = newString(text.len)
    var idx = 0
    var slash = false
    for ch in text.items:
      if ch == '/':
        if slash:
          continue
        slash = true
...
proc gatherLanEndpoints(n: NimNode): JsonNode =
  ...
  let lanIpv6Probe = collectLanIpv6Addrs()
  if lanIpv6Probe.len == 0:
    info "lan ipv6 probe", addrs = "<empty>"
  else:
    info "lan ipv6 probe", addrs = lanIpv6Probe.join(",")
  ...
  proc normalizeExportAddr(peerId, addrText: string): Option[string] =
    ...
    while sanitized.count(p2pMarker) > 1:
      ...
      if idx > 0:
        sanitized.delete(idx, idx + p2pMarker.len)
  ...
  proc addEndpoint(target: JsonNode, peerId: string, addrs: seq[string], isLocal: bool) =
    ...
    for maText in addrs:
      ...
      if firstAddr.len == 0:
        firstAddr = sanitized
      if preferIpv4.len == 0 and sanitized.contains("/ip4/"):
        preferIpv4 = sanitized
      if preferIpv6.len == 0 and sanitized.contains("/ip6/"):
        preferIpv6 = sanitized
    if arr.len == 0:
      return
    entry["multiaddrs"] = arr
    entry["is_local"] = %isLocal
    # maintain compatibility for older clients expecting single multiaddr
    if preferIpv4.len > 0:
      entry["multiaddr"] = %preferIpv4
    elif preferIpv6.len > 0:
      entry["multiaddr"] = %preferIpv6
    elif firstAddr.len > 0:
...
proc updateMdnsAdvertisements(n: NimNode) =
  ...
  try:
    ...
    let safeAdvertised = advertised.filterIt(not isUnspecifiedMultiaddrText($it))
    let safeFull = fullAddrs.filterIt(not isUnspecifiedMultiaddrText($it))
    debugLog(
    ...
    debugLog("[nimlibp2p] mdns advertise service=" & service &
      " addrCount=" & $fullAddrs.len)
    if safeAdvertised.len == 0:
      debugLog("[nimlibp2p] updateMdnsAdvertisements no concrete addrs to advertise")
    for addr in safeAdvertised:
      n.discovery.advertise(addr)
    for addr in safeFull:
      n.discovery.advertise(addr)
  except CatchableError as exc:
...
proc filterDialable(addrs: seq[MultiAddress]): (seq[MultiAddress], seq[string]) =
  ...
  for ma in addrs:
    ...
    if mdnsDialable(ma, reason):
      ...
      let desc = $ma
      rejected.add((if desc.len > 0: desc else: "<empty>") & ":" & reason)
  (accepted, rejected)

proc reportMdnsDialFailure(n: NimNode, peer: PeerId, addrs: seq[MultiAddress], reason: string) =
...
proc connectMdnsPeer(n: NimNode, peer: PeerId, addrs: seq[MultiAddress]) {.async.} =
  ...
  if dialable.len == 0:
    ...
    reportMdnsDialFailure(n, peer, addrs, "no_dialable_addresses")
    return
  ...
  try:
    ...
    warn "mdns dial failed", peer = $peer, err = exc.msg
    debugLog("[nimlibp2p] mdns dial failed peer=" & $peer & " err=" & exc.msg)
    reportMdnsDialFailure(n, peer, ordered, exc.msg)
  except Exception as exc:
    warn "mdns dial fatal", peer = $peer, err = exc.msg
    debugLog("[nimlibp2p] mdns dial fatal peer=" & $peer & " err=" & exc.msg)
    reportMdnsDialFailure(n, peer, ordered, "unexpected:" & exc.msg)
...
proc ensurePeerConnectivity(n: NimNode, peer: PeerId): Future[bool] {.async.} =
  ...
  try:
    ...
    warn "ensurePeerConnectivity connect failed", peer = $peer, err = exc.msg
  except Exception as exc:
...
proc watchMdns(n: NimNode, query: DiscoveryQuery, service: string): Future[void] {.async.} =
  ...
  while n.running and n.mdnsEnabled:
    try:
...
proc doStartMdns(n: NimNode): Future[void] {.async.} =
  ...
  var candidates = serviceQueryVariants(n.mdnsService) & serviceQueryVariants("_unimaker._udp.")
  candidates.add(MdnsMetaQuery)
...
proc scheduleStartMdns(n: NimNode) =
  ...
  n.mdnsStarting = true
  n.stateLock.release()
  asyncSpawn(doStartMdns(n))

proc restartMdns(n: NimNode): Future[void] {.async.} =
...
proc kickMdnsQuery(n: NimNode): Future[void] {.async.} =
  ...
  var starting, restarting, ready: bool
  n.stateLock.acquire()
  starting = n.mdnsStarting
  restarting = n.mdnsRestarting
  ready = n.mdnsReady
  n.stateLock.release()
...
proc setupConnectionEvents(n: NimNode) =
  ...
  let connectedHandler =
    proc(peerId: PeerId, event: ConnEvent): Future[void] {.closure, async: (raises: [CancelledError]).} =
      updateConnectedPeersSnapshot(n, peerId, true)
      var ev = newJObject()
      ...
      when defined(libp2p_msquic_experimental):
        emitMsQuicStats(n, "connection-established")
  let disconnectedHandler =
    proc(peerId: PeerId, event: ConnEvent): Future[void] {.closure, async: (raises: [CancelledError]).} =
      updateConnectedPeersSnapshot(n, peerId, false)
      var ev = newJObject()
      ...
      recordEvent(n, "network_event", $ev)
      when defined(libp2p_msquic_experimental):
        emitMsQuicStats(n, "connection-closed")
...
proc buildSwitch(cfg: NodeConfig): Result[Switch, string] =
  ...
  for addr in cfg.listenAddresses:
    ...
    info "buildSwitch parsed listen address", address = $parsed
  if addresses.len > 0:
...
proc runCommand(
  ...
  try:
    ...
    of cmdStart:
      if n.started:
        ...
        try:
          ...
          n.started = true
          refreshConnectedPeersSnapshot(n)
    ...
    of cmdConnectPeer:
      if not n.started:
        ...
        let peer = PeerId.init(command.peerId).valueOr:
          ...
          n.lastDirectError = command.errorMsg
          return
        var addrs = n.switchInstance.peerStore[AddressBook][peer]
        if addrs.len == 0:
          command.resultCode = NimResultInvalidArgument
          command.errorMsg = "peer address not known"
          n.lastDirectError = command.errorMsg
        else:
          try:
            await n.switchInstance.connect(peer, addrs, forceDial = true)
            ...
            command.errorMsg = ""
            n.lastDirectError = ""
            info "cmdConnectPeer success", peer = command.peerId, addrCount = addrs.len
          except CatchableError as exc:
    ...
    of cmdConnectedPeers:
      let arr = snapshotConnectedPeers(n)
    ...
    of cmdPeerMultiaddrs:
      ...
      for addr in addrs:
        strAddrs.add($addr)
      updatePeerMultiaddrsCache(n, command.peerId, strAddrs)
    ...
    of cmdConnectedPeersInfo:
      ...
      for peerId, muxers in connections.pairs:
        ...
        arr.add(entry)
      command.stringResult = $arr
      updateConnectedPeersInfoCache(n, command.stringResult)
      command.resultCode = NimResultOk
    ...
    of cmdInitializeConnEvents:
      ...
      refreshConnectedPeersSnapshot(n)
      when defined(libp2p_msquic_experimental):
        emitMsQuicStats(n, "init-connection-events")
      command.resultCode = NimResultOk
    ...
    dec(n.pendingCommands)
    info "command completed", kind = $command.kind, result = command.resultCode

proc processCommandQueue(n: NimNode) =
  while true:
    var command: Command = nil
    n.commandLock.acquire()
    let beforeLen = n.commandQueue.len
    if n.commandQueue.len > 0:
      command = n.commandQueue[0]
      n.commandQueue.delete(0)
    n.commandLock.release()
    if beforeLen > 0:
      debugLog(
        "[nimlibp2p] processCommandQueue node=" & $cast[int](n) &
        " popped kind=" &
        (if command.isNil: "nil" else: $command.kind) &
        " remaining=" & $n.commandQueue.len
      )
    if command.isNil:
      break
    debugLog(
    ...
    inc(n.pendingCommands)
    asyncSpawn(runCommand(n, command))

proc signalLoop(n: NimNode) {.async.} =
  while not n.stopRequested:
    try:
      await n.signal.wait()
      var queued = 0
      n.commandLock.acquire()
      queued = n.commandQueue.len
      n.commandLock.release()
      debugLog(
...
proc eventLoop(handlePtr: pointer) {.thread.} =
  ...
  while true:
    ...
    if node.pendingCommands > 0:
      debugLog(
        "[nimlibp2p] event loop processed queue node=" & $nodeAddr &
        " pending=" & $node.pendingCommands
      )
  processCommandQueue(node)
  info "event loop exiting", pendingCommands = node.pendingCommands
  debugLog("[nimlibp2p] event loop exiting node=" & $nodeAddr)
  node.stateLock.acquire()
  node.shutdownComplete = true
  node.stateCond.signal()
  node.stateLock.release()
...
proc ensureReady(node: NimNode) =
  ...
  while true:
    node.stateLock.acquire()
    let readyNow = node.ready
    node.stateLock.release()
    if readyNow:
      debugLog("[nimlibp2p] ensureReady completed")
      break
    if waited >= maxWait:
      debugLog("[nimlibp2p] ensureReady timeout waiting for event loop")
      break
    sleep(50)
    waited += 50
...
proc waitShutdown(node: NimNode) =
  node.stateLock.acquire()
  while not node.shutdownComplete:
    node.stateCond.wait(node.stateLock)
  node.stateLock.release()
...
proc submitCommand(node: NimNode, cmd: Command): Command =
  ...
  if not active:
    ...
    return cmd
  info "submitCommand", kind = $cmd.kind, pending = node.pendingCommands
...
proc destroyCommand(cmd: Command) =
  ...
  resetCommandFields(cmd)
  commandPoolLock.acquire()
  if commandPool.len < CommandPoolLimit:
    commandPool.add(cmd)
  commandPoolLock.release()
  # If the pool is full, allow GC to reclaim the command instance.
...
proc libp2p_node_init*(configJson: cstring): pointer {.exportc, cdecl, dynlib.} =
  ...
  try:
    ...
    var node = NimNode(
      ...
      feedItems: @[],
      peerMultiaddrsCache: initTable[string, seq[string]](),
      livestreams: initTable[string, LivestreamSession](),
      lanGroups: initTable[string, LanGroupState](),
      connectedPeersSnapshot: initHashSet[string](),
      connectedPeersUpdatedMs: 0,
      connectedPeersInfoCache: "[]",
    ...
    initLock(node.connectedPeersLock)
    initLock(node.connectedPeersInfoLock)
    ...
    try:
      debugLog(
        "[nimlibp2p] before createThread node=" & $cast[int](node) &
        " stateLock=" & lockAddr(node.stateLock) &
        " commandLock=" & lockAddr(node.commandLock)
      )
      ...
      debugLog("[nimlibp2p] failure deinit stateLock ptr=" & lockAddr(node.stateLock))
      deinitLock(node.stateLock)
      debugLog("[nimlibp2p] failure deinit commandLock ptr=" & lockAddr(node.commandLock))
...
proc libp2p_node_start*(handle: pointer): cint {.exportc, cdecl, dynlib.} =
  ...
  if rc != NimResultOk and err == "WARN_DISCOVERY_ADVERTISE":
    return NimResultOk
  if rc != NimResultOk and err.len > 0:
    recordInitError(err)
  rc
...
proc libp2p_get_lan_endpoints_json_safe*(handle: pointer, outJson: ptr cstring): bool {.exportc, cdecl, dynlib.} =
  ## Safe variant that never raises; returns false when the call cannot be satisfied.
  if outJson.isNil:
    return false
  var text = "[]"
  var success = false
  try:
    ...
    warn "get_lan_endpoints_json_safe: exception", err = exc.msg
  except Defect as exc:
    warn "get_lan_endpoints_json_safe: defect", err = exc.msg
  outJson[] = allocCString(text)
  success

proc libp2p_fetch_feed_snapshot*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  let node = nodeFromHandle(handle)
  if node.isNil:
    return allocCString("{\"items\":[],\"timestamp_ms\":0}")
  var arr = newJArray()
  node.feedItemsLock.acquire()
  for item in node.feedItems:
    ...
    obj["payload"] = item.payload
    arr.add(obj)
  node.feedItemsLock.release()
  var snapshot = newJObject()
  snapshot["items"] = arr
  snapshot["timestamp_ms"] = %nowMillis()
  allocCString($snapshot)
...
proc libp2p_poll_events*(handle: pointer, maxEvents: cint): cstring {.exportc, cdecl, dynlib.} =
  ...
  if node.isNil:
    return allocCString("[]")
  var limit = int(maxEvents)
  if limit <= 0 or limit > node.eventLogLimit:
    limit = node.eventLogLimit
  node.eventLogLock.acquire()
  let available = node.eventLog.len
  if available == 0:
    node.eventLogLock.release()
    return allocCString("[]")
  var takeCount = available
  if limit < available:
    takeCount = limit
  var arr = newJArray()
  for i in 0 ..< takeCount:
    arr.add(node.eventLog[i])
  if takeCount == available:
    node.eventLog.setLen(0)
  else:
    node.eventLog = node.eventLog[takeCount ..< available]
  node.eventLogLock.release()
  allocCString($arr)
proc libp2p_get_connected_peers_json*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  ...
  if node.isNil:
    return allocCString("[]")
  let peers = snapshotConnectedPeersNonBlocking(node)
  var arr = newJArray()
  for peer in peers:
    if peer.len > 0:
      arr.add(%peer)
...
proc libp2p_connected_peers_info*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  ...
  if node.isNil:
    return allocCString("[]")
  let cached = readConnectedPeersInfoCache(node)
  if cached.len == 0 or cached == "[]":
    let cmd = makeCommand(cmdConnectedPeersInfo)
    discard submitCommandAsync(node, cmd)
  allocCString(cached)
...
proc libp2p_get_peer_multiaddrs_json*(
  ...
  if node.isNil:
    return allocCString("[]")
  let peerStr = $peerId
  let cached = readPeerMultiaddrsCache(node, peerStr)
  if cached.len == 0:
...
proc libp2p_register_peer_hints*(
  ...
  if source != nil:
    cmd.source = $source
  let rc = submitCommandAsync(node, cmd)
  if rc == NimResultOk:
    return true
  destroyCommand(cmd)
  false
...
proc libp2p_mdns_set_enabled*(handle: pointer, enabled: bool): bool {.exportc, cdecl, dynlib.} =
  ...
  if node.isNil:
    ...
    return false
  node.stateLock.acquire()
  node.mdnsEnabled = enabled
  debugLog(
    "[nimlibp2p] mdns_set_enabled flag updated node=" & $cast[int](node) &
    " enabled=" & $enabled
  )
  node.stateLock.release()
  info "mdns set enabled", enabled = enabled
  if enabled:
    scheduleStartMdns(node)
  else:
    asyncSpawn(stopMdns(node))
  true

proc libp2p_identity_from_seed*(seed: ptr uint8, seedLen: csize_t): cstring {.exportc, cdecl, dynlib.} =
  ## Derive a deterministic Ed25519 identity from an arbitrary seed buffer.
  try:
...
proc libp2p_mdns_set_interface*(handle: pointer, ipv4: cstring): bool {.exportc, cdecl, dynlib.} =
  ...
  if ip == "0.0.0.0":
    ip = ""
  node.stateLock.acquire()
  debugLog("[nimlibp2p] mdns_set_interface existing=" & node.mdnsPreferredIpv4 & " new=" & ip)
  let sameValue = node.mdnsPreferredIpv4 == ip
  if not sameValue:
    node.mdnsPreferredIpv4 = ip
  if not node.mdnsInterface.isNil:
    node.mdnsInterface.setPreferredIpv4(node.mdnsPreferredIpv4)
  let canRestart = node.mdnsEnabled and node.started
  let hasInterface = not node.mdnsInterface.isNil
  let boundMismatch = hasInterface and ip.len > 0 and (node.mdnsInterface.boundIpv4.len == 0 or node.mdnsInterface.boundIpv4 != ip)
  let restartNeeded = ip.len > 0 and (not sameValue or boundMismatch)
  node.mdnsRestartPending = restartNeeded
  debugLog(
    "[nimlibp2p] mdns_set_interface sameValue=" & $sameValue &
    " enabled=" & $node.mdnsEnabled & " started=" & $node.started &
    " willRestart=" & $node.mdnsRestartPending
  )
  node.stateLock.release()
  info "mdns interface updated", ipv4 = ip, restart = restartNeeded
  if node.mdnsRestartPending:
    if canRestart:
      info "mdns interface restart triggered", ipv4 = ip
      asyncSpawn(restartMdns(node))
    else:
      debugLog("[nimlibp2p] mdns_set_interface deferred restart (not yet started)")
  true
...
proc libp2p_mdns_debug*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  ...
  node.stateLock.acquire()
  let enabled = node.mdnsEnabled
  let started = node.started
  let preferred = node.mdnsPreferredIpv4
  let pending = node.mdnsRestartPending
  let interval = node.mdnsIntervalSeconds
  var bound = ""
  if not node.mdnsInterface.isNil:
    bound = node.mdnsInterface.boundIpv4
  node.stateLock.release()
  var obj = newJObject()
  obj["enabled"] = %enabled
  obj["started"] = %started
  obj["preferred_ipv4"] = %preferred
  obj["restart_pending"] = %pending
  obj["interval_seconds"] = %interval
  obj["interface_bound_ipv4"] = %bound
  debugLog("[nimlibp2p] mdns_debug payload=" & $obj)
  allocCString($obj)
...
proc libp2p_mdns_set_interval*(handle: pointer, seconds: cint): bool {.exportc, cdecl, dynlib.} =
  ...
  if node.isNil:
    return false
  node.stateLock.acquire()
  node.mdnsIntervalSeconds = int(seconds)
  node.stateLock.release()
  info "mdns interval updated", seconds = int(seconds)
  if node.mdnsEnabled and node.started:
    asyncSpawn(restartMdns(node))
  true
