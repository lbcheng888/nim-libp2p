...
import libp2p/[bandwidthmanager, builders, connmanager, features, memorymanager, multiaddress,
               lsmr, muxers/mplex/mplex, muxers/muxer, muxers/yamux/yamux, peerid, peerstore, protocols/connectivity/relay/client,
...
type
  DirectSessionTargetSnapshot = object
    taskKey: string
    peerId: string
    status: string
    error: string
  ...
  PeerRouteSessionSnapshot = object
    peerId: string
    status: string
    error: string
    ok: bool
...
var bootstrapSeedConnectKickoffLatestTaskKey {.global.}: Table[string, string] = initTable[string, string]()
var bootstrapSeedConnectKickoffStateLock {.global.}: Lock
var directSessionTargetState {.global.}: Table[string, string] = initTable[string, string]()
var directSessionTargetSnapshots {.global.}: Table[string, DirectSessionTargetSnapshot] =
  initTable[string, DirectSessionTargetSnapshot]()
var directSessionTargetLatestTaskKey {.global.}: Table[string, string] = initTable[string, string]()
var directSessionTargetStateLock {.global.}: Lock
...
proc bootstrapSeedConnectTaskKey(
    peerId: string,
    selectedAddrs: openArray[string]
): string {.raises: [].}

proc directSessionTargetStateJson(
    peerId: string,
    taskKey: string,
    status: string,
...
proc buildDirectSessionTargetSnapshot(
    peerId: string,
    taskKey: string,
    status: string,
    payload: JsonNode = newJNull(),
    error: string = ""
): DirectSessionTargetSnapshot {.raises: [].} =
  result = DirectSessionTargetSnapshot(
    taskKey: taskKey,
    peerId: peerId,
    status: status,
    error: error,
  ...
  if payload.kind != JObject:
    return
  let okNode = payload{"ok"}
  if not okNode.isNil and okNode.kind == JBool:
    result.ok = okNode.getBool()
  let taskRunningNode = payload{"taskRunning"}
  if not taskRunningNode.isNil and taskRunningNode.kind == JBool:
    result.taskRunning = taskRunningNode.getBool()
  let sessionWarmOkNode = payload{"sessionWarmOk"}
  if not sessionWarmOkNode.isNil and sessionWarmOkNode.kind == JBool:
    result.sessionWarmOk = sessionWarmOkNode.getBool()
  let verifiedMuxerNode = payload{"verifiedMuxer"}
  if not verifiedMuxerNode.isNil and verifiedMuxerNode.kind == JBool:
    result.verifiedMuxer = verifiedMuxerNode.getBool()
  let sessionConnKeyNode = payload{"sessionConnKey"}
  if not sessionConnKeyNode.isNil:
    case sessionConnKeyNode.kind
    of JInt:
      result.sessionConnKey = int(sessionConnKeyNode.getInt())
    of JString:
      try:
        result.sessionConnKey = parseInt(sessionConnKeyNode.getStr())
      except ValueError:
        discard
    else:
      discard
  let errorNode = payload{"error"}
  if not errorNode.isNil and errorNode.kind == JString and result.error.len == 0:
    result.error = errorNode.getStr()
  let routeReadyNode = payload{"routeReady"}
  if not routeReadyNode.isNil and routeReadyNode.kind == JBool:
    result.routeReady = routeReadyNode.getBool()
  let routePathKindNode = payload{"routePathKind"}
  if not routePathKindNode.isNil and routePathKindNode.kind == JString:
    result.routePathKind = routePathKindNode.getStr()
  let attemptCountNode = payload{"attemptCount"}
  if not attemptCountNode.isNil and attemptCountNode.kind == JInt:
    result.attemptCount = int(attemptCountNode.getInt())
  let successCountNode = payload{"successCount"}
  if not successCountNode.isNil and successCountNode.kind == JInt:
    result.successCount = int(successCountNode.getInt())
  let failureCountNode = payload{"failureCount"}
  if not failureCountNode.isNil and failureCountNode.kind == JInt:
    result.failureCount = int(failureCountNode.getInt())
  let lastAttemptAtMsNode = payload{"lastAttemptAtMs"}
  if not lastAttemptAtMsNode.isNil and lastAttemptAtMsNode.kind == JInt:
    result.lastAttemptAtMs = lastAttemptAtMsNode.getInt()
  let lastReadyAtMsNode = payload{"lastReadyAtMs"}
  if not lastReadyAtMsNode.isNil and lastReadyAtMsNode.kind == JInt:
    result.lastReadyAtMs = lastReadyAtMsNode.getInt()
  let lastFailureAtMsNode = payload{"lastFailureAtMs"}
  if not lastFailureAtMsNode.isNil and lastFailureAtMsNode.kind == JInt:
    result.lastFailureAtMs = lastFailureAtMsNode.getInt()
  let lastFailureReasonNode = payload{"lastFailureReason"}
  if not lastFailureReasonNode.isNil and lastFailureReasonNode.kind == JString:
    result.lastFailureReason = lastFailureReasonNode.getStr()

proc setDirectSessionTargetState(
    peerId: string,
    selectedAddrs: openArray[string],
    status: string,
    payload: JsonNode = newJNull(),
  ...
  let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
  if safePeerId.len == 0 or taskKey.len == 0:
    return
  let snapshot =
    buildDirectSessionTargetSnapshot(safePeerId, taskKey, status, payload, error)
  withLock(directSessionTargetStateLock):
    directSessionTargetState[taskKey] =
      directSessionTargetStateJson(safePeerId, taskKey, status, payload, error)
    directSessionTargetSnapshots[taskKey] = snapshot
    directSessionTargetLatestTaskKey[safePeerId] = taskKey

proc getDirectSessionTargetState(
    peerId: string,
    selectedAddrs: openArray[string]
): string {.raises: [].} =
  let safePeerId = peerId.strip()
  let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
  if safePeerId.len == 0 or taskKey.len == 0:
    return directSessionTargetStateJson("", "", "missing", newJNull(), "missing_peerId_or_addresses")
  withLock(directSessionTargetStateLock):
    if directSessionTargetState.hasKey(taskKey):
      return directSessionTargetState.getOrDefault(taskKey)
  directSessionTargetStateJson(safePeerId, taskKey, "missing")

proc getDirectSessionTargetSnapshot(
    peerId: string,
    selectedAddrs: openArray[string]
): DirectSessionTargetSnapshot {.raises: [].} =
  let safePeerId = peerId.strip()
  let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
  if safePeerId.len == 0 or taskKey.len == 0:
    return DirectSessionTargetSnapshot(
      taskKey: "",
      peerId: safePeerId,
      status: "missing",
      error: "missing_peerId_or_addresses",
      ok: false,
      taskRunning: false,
      sessionWarmOk: false,
      verifiedMuxer: false,
  ...
  withLock(directSessionTargetStateLock):
    if directSessionTargetSnapshots.hasKey(taskKey):
      return directSessionTargetSnapshots.getOrDefault(taskKey)
  DirectSessionTargetSnapshot(
    taskKey: taskKey,
    peerId: safePeerId,
    status: "missing",
    error: "",
...
proc getReadyDirectSessionTargetConnKey(
    peerId: string,
    selectedAddrs: openArray[string]
): int {.raises: [].} =
  let snapshot = getDirectSessionTargetSnapshot(peerId, selectedAddrs)
  if snapshot.status.toLowerAscii() != "ready" or snapshot.sessionConnKey <= 0:
    return 0
  snapshot.sessionConnKey

proc getLatestDirectSessionTargetSnapshot(
    peerId: string
): DirectSessionTargetSnapshot {.raises: [].} =
  let safePeerId = peerId.strip()
  if safePeerId.len == 0:
  ...
  withLock(directSessionTargetStateLock):
    let latestTaskKey = directSessionTargetLatestTaskKey.getOrDefault(safePeerId, "")
    if latestTaskKey.len > 0 and directSessionTargetSnapshots.hasKey(latestTaskKey):
      return directSessionTargetSnapshots.getOrDefault(latestTaskKey)
  DirectSessionTargetSnapshot(
    taskKey: "",
    peerId: safePeerId,
    status: "missing",
...
proc setBootstrapSeedConnectKickoffState(
  ...
  withLock(bootstrapSeedConnectKickoffStateLock):
    bootstrapSeedConnectKickoffState[taskKey] =
      bootstrapSeedConnectKickoffStateJson(safePeerId, taskKey, status, payload, error)
    bootstrapSeedConnectKickoffLatestTaskKey[safePeerId] = taskKey

proc getBootstrapSeedConnectKickoffState(
    peerId: string,
    selectedAddrs: openArray[string]
): string {.raises: [].} =
  let safePeerId = peerId.strip()
  let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
  if safePeerId.len == 0 or taskKey.len == 0:
    return bootstrapSeedConnectKickoffStateJson("", "", "missing", newJNull(), "missing_peerId_or_addresses")
  withLock(bootstrapSeedConnectKickoffStateLock):
    if bootstrapSeedConnectKickoffState.hasKey(taskKey):
      return bootstrapSeedConnectKickoffState.getOrDefault(taskKey)
  bootstrapSeedConnectKickoffStateJson(safePeerId, taskKey, "missing")

proc getBootstrapSeedConnectKickoffState(peerId: string): string {.raises: [].} =
  let safePeerId = peerId.strip()
  if safePeerId.len == 0:
    return bootstrapSeedConnectKickoffStateJson("", "", "missing", newJNull(), "missing_peerId")
  withLock(bootstrapSeedConnectKickoffStateLock):
    let latestTaskKey = bootstrapSeedConnectKickoffLatestTaskKey.getOrDefault(safePeerId, "")
    if latestTaskKey.len > 0 and bootstrapSeedConnectKickoffState.hasKey(latestTaskKey):
      return bootstrapSeedConnectKickoffState.getOrDefault(latestTaskKey)
...
proc getBootstrapSeedConnectKickoffStateNode(peerId: string): JsonNode =
  let safePeerId = peerId.strip()
  let rawState = getBootstrapSeedConnectKickoffState(safePeerId)
  try:
    parseJson(rawState)
...
proc bootstrapSeedConnectPayloadField(
    payload: JsonNode,
    key: string
): JsonNode {.gcsafe, raises: [].} =
  if payload.kind == JObject:
    for fieldKey, fieldValue in payload.pairs:
      if fieldKey == key:
        return fieldValue
  var attemptsNode = newJNull()
  if payload.kind == JObject:
    for fieldKey, fieldValue in payload.pairs:
      if fieldKey == "attempts":
        attemptsNode = fieldValue
        break
  if attemptsNode.kind == JArray and attemptsNode.len > 0:
    let lastAttempt = attemptsNode[^1]
    if lastAttempt.kind == JObject:
      for fieldKey, fieldValue in lastAttempt.pairs:
        if fieldKey == key:
          return fieldValue
  newJNull()

proc safeJsonStrField(
    node: JsonNode,
    key: string,
    defaultValue = ""
): string {.gcsafe, raises: [].} =
  let fieldNode = bootstrapSeedConnectPayloadField(node, key)
  if fieldNode.kind == JString:
    return fieldNode.getStr()
  defaultValue

proc safeJsonBoolField(
    node: JsonNode,
    key: string,
    defaultValue = false
): bool {.gcsafe, raises: [].} =
  let fieldNode = bootstrapSeedConnectPayloadField(node, key)
  if fieldNode.kind == JBool:
    return fieldNode.getBool()
  defaultValue

proc bootstrapSeedConnectPayloadBool(
    payload: JsonNode,
    key: string,
    defaultValue = false
): bool {.gcsafe, raises: [].} =
  safeJsonBoolField(payload, key, defaultValue)

proc bootstrapSeedConnectPayloadStr(
    payload: JsonNode,
    key: string,
    defaultValue = ""
): string {.gcsafe, raises: [].} =
  safeJsonStrField(payload, key, defaultValue)
...
proc ensureMsQuicDiagnosticsHook() =
  when defined(libp2p_msquic_experimental) and defined(libp2p_msquic_diag_hook):
...
type
  ...
  CommandKind = enum
    ...
    cmdDirectSessionTargetSet,
    cmdDirectSessionTargetClear,
    cmdUdpEchoProbe,
    cmdSendDirect,
  ...
  BootstrapSeedConnectState = enum
    bscsPending
    bscsRunning
    bscsProbing
    bscsReady
  ...
  BootstrapSeedConnectTask = ref object
    taskKey: string
    peerId: string
    source: string
    intent: BootstrapSeedConnectIntent
    state: BootstrapSeedConnectState
    updatedAtMs: int64
    deadlineAtMs: int64
    ...
    err: string
    verifiedMuxer: Muxer
    verifiedMuxerWaitMs: int
    sessionConnKey: int
    sessionWarmOk: bool
    sessionWarmError: string

  BootstrapSeedConnectIntent = enum
    bsciBackgroundWarm
    bsciForegroundSend

  PeerRouteSessionState = enum
    prssPending
    prssRunning
    prssReady
    prssCancelled
  ...
  PeerRouteSessionTask = ref object
    peerId: string
    source: string
    state: PeerRouteSessionState
    updatedAtMs: int64
    deadlineAtMs: int64
    err: string
    sessionConnKey: int

  SendDirectKickoffTask = ref object
    requestKey: string
    peerId: string
    source: string
    forceRelayConnectivity: bool
    updatedAtMs: int64
    completedAtMs: int64
    running: bool

  DirectSessionTargetSpec = object
    taskKey: string
    peerId: string
    source: string
    selectedAddrs: seq[string]
  ...
  HostNetworkStatusSnapshot = object
    networkType: string
    transport: string
    ssid: string
    localIpv4: string
    ...
    publicIpv4: string
    publicIpv6: string
    publicIpv4ProbeError: string
    publicIpv6ProbeError: string
    publicIpProbeUpdatedAtMs: int64
    publicIpv4RequiresHolePunching: bool
    listenPort: int
    isConnected: bool
    isMetered: bool
    timestampMs: int64
  ...
  NimNode* = ref object
    ...
    ## Keep one long-lived command-drain future alive for the lifetime of the
    ## node. Re-creating and retiring drain futures under real DM traffic has
    ## repeatedly crashed in ORC/GC teardown paths after successful sends.
    commandDrainTask: Future[void]
    commandDrainWake: AsyncEvent
    ...
    ## Old queue tails and completed command objects are retained for the
    ...
    ## ref-header crashes under heavy GameMesh automation on Harmony. The same
    ## ownership hazard shows up again when a completed command is still
    ## transiently referenced by the lock-free queue or its drain future while
    ## async receiver callbacks wake the command queue. Keep those refs strongly
    ## owned until node teardown instead of destroying them mid-run.
    ...
    started: bool
    startupStatusLock: Lock
    startupInFlight: bool
    startupStage: string
    startupError: string
    startupQueuedAtMs: int64
    startupUpdatedAtMs: int64
    startupLastDurationMs: int64
    startupAttempts: int
    tailnetStatusLock: Lock
    tailnetInFlight: bool
    tailnetStage: string
    tailnetError: string
    tailnetQueuedAtMs: int64
    tailnetUpdatedAtMs: int64
    tailnetLastDurationMs: int64
    tailnetAttempts: int
    tailnetWarmTask: Future[void]
    bootstrapSeedConnectTasks: Table[string, BootstrapSeedConnectTask]
    bootstrapSeedConnectFutures: Table[string, Future[void]]
    directSessionTargetLock: Lock
    directSessionTargets: Table[string, DirectSessionTargetSpec]
    peerRouteSessionTasks: Table[string, PeerRouteSessionTask]
    peerRouteSessionFutures: Table[string, Future[void]]
    sendDirectKickoffTasks: Table[string, SendDirectKickoffTask]
    sendDirectKickoffFutures: Table[string, Future[void]]
    ...
    peerHandlers: seq[(PeerEventKind, PeerEventHandler)]
    peerStateLock: Lock
    peerIdentifiedAt: Table[string, int64]
    ...
    directSessionTargetReconcileLoop: Future[void]
    peerRouteSessionReconcileLoop: Future[void]
    ...
    receivedDmLog: seq[ReceivedDmSnapshot]
    receivedDmLogLock: Lock
    receivedDmLogLimit: int
    socialStateSnapshot: string
    socialStateParsedSnapshot: JsonNode
    localSystemProfile: JsonNode
    localSystemProfileUpdatedAt: int64
    hostNetworkStatus: HostNetworkStatusSnapshot
    hostNetworkStatusUpdatedAt: int64
    lastTailnetPing: JsonNode
    lastTailnetPingUpdatedAt: int64
    peerInfoListenAddrTexts: seq[string]
    peerInfoAddrTexts: seq[string]
    ...
    deferredHostNetworkStatus: HostNetworkStatusSnapshot
...
proc publishDirectSessionTargetSnapshot(
    n: NimNode,
    spec: DirectSessionTargetSpec,
    forcedStatus = "",
    forcedError = ""
) {.gcsafe, raises: [].}
proc reconcileDirectSessionTarget(
    n: NimNode,
    spec: DirectSessionTargetSpec
) {.gcsafe, raises: [].}
proc ensureDirectSessionTargetReconcileLoop(n: NimNode) {.gcsafe, raises: [].}
proc ensurePeerRouteSessionReconcileLoop(n: NimNode) {.gcsafe, raises: [].}
proc upsertDirectSessionTarget(
    n: NimNode,
    peerIdText: string,
    addresses: seq[string],
    source: string
): DirectSessionTargetSpec {.gcsafe, raises: [].}
proc latestPeerRouteSessionSnapshot(
    n: NimNode,
    peerIdText: string
): PeerRouteSessionSnapshot {.gcsafe, raises: [].}
proc peerRouteSessionReady(
    snapshot: PeerRouteSessionSnapshot
): bool {.gcsafe, raises: [].}
proc peerRouteSessionProbing(
    snapshot: PeerRouteSessionSnapshot
): bool {.gcsafe, raises: [].}
...
proc registerPeerHintsInternal(
    n: NimNode,
    peerIdText: string,
    addresses: seq[string],
    source: string
...
proc runBootstrapSeedConnectExactAsync(
    taskState: BootstrapSeedConnectTask,
    node: NimNode,
    safePeerId: string,
    selectedAddrs: seq[string],
...
proc getOrStartBootstrapSeedConnectTask(
    n: NimNode,
    safePeerId: string,
    selectedAddrs: seq[string],
    effectiveSource: string,
...
proc bootstrapSeedConnectTaskRunning(
    n: NimNode,
    task: BootstrapSeedConnectTask
): bool {.gcsafe, raises: [].}
proc sendDirectKickoffTaskRunning(
    n: NimNode,
    task: SendDirectKickoffTask
): bool {.gcsafe, raises: [].}
proc compactFinishedSendDirectKickoffTasks(
    n: NimNode,
    keepRequestKey = ""
) {.gcsafe, raises: [].}
...
proc resetCommandFields(cmd: Command) =
  ...
  cmd.completed.store(false, moRelease)
  cmd.requestLimit = 0
  cmd.boolParam = false
...
proc isHighPriorityCommand(cmd: Command): bool =
  ...
  of cmdSendChatAck:
    return true
  of cmdBootstrapSeedConnectExactKickoff:
    return true
  of cmdDirectSessionTargetSet:
    return true
  of cmdSendDirect:
...
proc parseReceivedDmPayloadText(text: string): JsonNode {.gcsafe, raises: [].} =
...
proc receivedDmSnapshotToJson(row: ReceivedDmSnapshot): JsonNode {.gcsafe, raises: [].} =
  result = newJObject()
  result["peerId"] = %row.peerId
  result["from"] = %row.peerId
  result["conversationId"] = %row.conversationId
  result["messageId"] = %row.messageId
  result["mid"] = %row.messageId
  result["content"] = %row.body
  result["body"] = %row.body
  result["timestampMs"] = %row.timestampMs
  result["timestamp_ms"] = %row.timestampMs
  result["payload"] = parseReceivedDmPayloadText(row.payloadText)
  ...
  try:
    ...
    if available > 0:
      ...
      for i in 0 ..< takeCount:
        items.add(receivedDmSnapshotToJson(n.receivedDmLog[i]))
...
proc drainReceivedDmRows(n: NimNode): seq[ReceivedDmSnapshot] =
  if n.isNil:
    return @[]
  n.receivedDmLogLock.acquire()
  try:
...
proc restoreReceivedDmRows(n: NimNode; rows: seq[ReceivedDmSnapshot]) =
...
proc prioritizeGameSeedAddrs(seedAddrs: seq[string]): seq[string] =
  var byEndpoint = initTable[string, tuple[index: int, value: string]]()
  var seenRaw = initHashSet[string]()

  for idx, addr in seedAddrs:
    ...
    if endpointKey.len == 0:
      ...
      byEndpoint[normalized] = (idx, normalized)
      continue
    let previous = byEndpoint.getOrDefault(endpointKey, (-1, ""))
    if previous.index < 0:
      byEndpoint[endpointKey] = (idx, normalized)
    elif seedAddrPrefixRank(normalized) < seedAddrPrefixRank(previous.value):
      byEndpoint[endpointKey] = (previous.index, normalized)

  var indexed: seq[tuple[index: int, value: string]] = @[]
  for _, entry in byEndpoint.pairs:
    indexed.add(entry)
  indexed.sort(proc(
      a, b: tuple[index: int, value: string]
  ): int =
    let rankA = seedAddrRank(a.value)
    let rankB = seedAddrRank(b.value)
    if rankA < rankB:
      return -1
    if rankA > rankB:
      return 1
    system.cmp(a.index, b.index)
  )
  result = newSeqOfCap[string](indexed.len)
...
proc submitCommandNoWait(node: NimNode, cmd: Command): bool {.gcsafe.}
...
proc tsnetPublicationReady(node: NimNode): bool {.gcsafe, raises: [].} =
  ...
  if transport.isNil:
    return true
  transport.providerReadySafe() and transport.proxyListenersReadySafe()
...
proc dedupeLocalMultiaddrs(addrs: openArray[MultiAddress]): seq[MultiAddress] {.gcsafe.} =
  var seen = initHashSet[string]()
  for addr in addrs:
    let key = safeMultiaddrText(addr)
    if key.len == 0 or key in seen:
      continue
    result.add(addr)
    seen.incl(key)

proc cloneMultiaddr(ma: MultiAddress): MultiAddress {.gcsafe, raises: [].} =
  try:
    let cloned = MultiAddress.init(ma.data.buffer)
    if cloned.isOk():
      return cloned.get()
  except CatchableError:
    discard
...
proc cloneMultiaddrs(addrs: openArray[MultiAddress]): seq[MultiAddress] {.gcsafe, raises: [].} =
  for addr in addrs:
    let cloned = cloneMultiaddr(addr)
    if safeMultiaddrText(cloned).len > 0:
      result.add(cloned)

proc rememberPeerInfoAddrTexts(
    node: NimNode,
  ...
  if node.isNil:
    return
  node.peerInfoListenAddrTexts = listenAddrs
    .mapIt(safeMultiaddrText(it))
    .filterIt(it.len > 0)
  node.peerInfoAddrTexts = addrs
    .mapIt(safeMultiaddrText(it))
    .filterIt(it.len > 0)

proc dedupeAddrTexts(
    addrTexts: openArray[string]
): seq[string] {.gcsafe, raises: [].} =
  for addrText in addrTexts:
    let normalized = addrText.strip()
    if normalized.len == 0:
      continue
    var text = newStringOfCap(normalized.len)
    text.add(normalized)
    if result.anyIt(it == text):
      continue
    result.add(text)
...
proc rememberPeerInfoAddrTexts(
    ...
    addrs: openArray[string],
) {.gcsafe, raises: [].} =
  if node.isNil:
    return
  node.peerInfoListenAddrTexts = dedupeAddrTexts(listenAddrs)
  node.peerInfoAddrTexts = dedupeAddrTexts(addrs)
...
proc parseMultiaddrsFromTexts(
    addrTexts: openArray[string]
): seq[MultiAddress] {.gcsafe, raises: [].} =
  try:
    appendParsedUniqueAddrs(result, addrTexts)
  except Exception:
    result = @[]

proc currentTsnetPublishedAddrs(node: NimNode): seq[MultiAddress] {.gcsafe, raises: [].} =
  if node.isNil or node.switchInstance.isNil:
    return @[]
  let transport = node.switchInstance.tsnetTransport()
  if transport.isNil:
    return @[]
  try:
    return dedupeLocalMultiaddrs(cloneMultiaddrs(transport.publishedAddrs()))
  except CatchableError:
    return @[]
  except Defect:
    return @[]

proc currentTsnetPublishedAddrTexts(node: NimNode): seq[string] {.gcsafe, raises: [].} =
  if node.isNil or node.switchInstance.isNil:
    return @[]
  let transport = node.switchInstance.tsnetTransport()
  if transport.isNil:
    return @[]
  try:
    return dedupeAddrTexts(transport.publishedAddrTexts())
  except CatchableError:
    return @[]
  except Defect:
    return @[]
...
proc syncAdvertisedPeerInfo(
  ...
  let previousListen =
    if tsnetOnly:
      node.peerInfoListenAddrTexts
    else:
      peerInfo.listenAddrs.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  let previousAddrs =
    if tsnetOnly:
      node.peerInfoAddrTexts
    else:
      peerInfo.addrs.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  ...
  var nextListen =
    if refreshed.len > 0:
      dedupeLocalMultiaddrs(cloneMultiaddrs(refreshed))
    elif tsnetOnly:
      currentTsnetPublishedAddrs(node)
    else:
      dedupeLocalMultiaddrs(peerInfo.listenAddrs)
  if nextListen.len == 0 and not tsnetOnly:
  ...
  let preFilterListen = nextListen
  let preFilterAddrs = nextAddrs
  nextListen = filterPublishableTsnetAddrs(node, nextListen)
  nextAddrs = filterPublishableTsnetAddrs(node, nextAddrs)
  if tsnetOnly and nextListen.len == 0 and nextAddrs.len == 0:
    let strippedConcreteTsnet =
      preFilterListen.anyIt(isTsnetPublishAddr(safeMultiaddrText(it))) or
      preFilterAddrs.anyIt(isTsnetPublishAddr(safeMultiaddrText(it)))
    if strippedConcreteTsnet:
      debugLog(
        "[nimlibp2p] syncAdvertisedPeerInfo skip_empty_tsnet_publish reason=" & reason &
        " previousListen=" & (if previousListen.len > 0: previousListen.join(",") else: "<empty>") &
        " previousAddrs=" & (if previousAddrs.len > 0: previousAddrs.join(",") else: "<empty>")
      )
      return false

  let nextListenText = nextListen.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  let nextAddrsText = nextAddrs.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  ...
  rememberPeerInfoAddrTexts(node, nextListen, nextAddrs)
...
proc defaultHostNetworkStatusSnapshot(): HostNetworkStatusSnapshot {.gcsafe.} =
...
proc hostNetworkStatusToJson(status: HostNetworkStatusSnapshot): JsonNode {.gcsafe.} =
  %* {
    ...
    "preferredIpv4": status.preferredIpv4,
    "localIpv6": status.localIpv6,
    "preferredIpv6": status.preferredIpv6,
    "publicIpv4": status.publicIpv4,
    "publicIpv6": status.publicIpv6,
    "publicIpv4ProbeError": status.publicIpv4ProbeError,
    "publicIpv6ProbeError": status.publicIpv6ProbeError,
    "publicIpProbeUpdatedAtMs": status.publicIpProbeUpdatedAtMs,
    "publicIpv4RequiresHolePunching": status.publicIpv4RequiresHolePunching,
...
proc defaultHostNetworkStatus(): JsonNode =
  hostNetworkStatusToJson(defaultHostNetworkStatusSnapshot())
...
proc attachPublicIpProbeFields(
    status: var HostNetworkStatusSnapshot,
    probe: PublicIpProbeState
) {.gcsafe.} =
  let preferredIpv4 =
    (if status.preferredIpv4.len > 0: status.preferredIpv4 else: status.localIpv4).strip()
  ...
  status.publicIpv4RequiresHolePunching =
    publicIpv4.len > 0 and not isProviderPublicIpv4Text(preferredIpv4)

proc shouldRefreshPublicIpProbe(
    n: NimNode,
    hostStatus: HostNetworkStatusSnapshot,
    force = false
): bool {.gcsafe.} =
  if n.isNil:
    return false
...
proc sanitizeHostNetworkStatusSnapshot(
    node: JsonNode
): HostNetworkStatusSnapshot {.gcsafe.}
...
proc deferHostNetworkStatusSideEffects(
  ...
  n.deferredHostNetworkStatus = hostStatus
...
proc mergeTsnetListenAddrsIntoPeerInfo(
  ...
  let tsnetOnly = wantsTsnetTransport(node.cfg)
  if tsnetOnly:
    let previousListenTexts = node.peerInfoListenAddrTexts
    let previousAddrsTexts = node.peerInfoAddrTexts
    let mergedListenTexts = currentTsnetPublishedAddrTexts(node)
    let mergedAddrsTexts = dedupeAddrTexts(mergedListenTexts)
    result =
      mergedListenTexts != previousListenTexts or
      mergedAddrsTexts != previousAddrsTexts
    node.switchInstance.peerInfo.listenAddrs = parseMultiaddrsFromTexts(mergedListenTexts)
    node.switchInstance.peerInfo.addrs = parseMultiaddrsFromTexts(mergedAddrsTexts)
    rememberPeerInfoAddrTexts(node, mergedListenTexts, mergedAddrsTexts)
    return
  let previousListenTexts =
    node.switchInstance.peerInfo.listenAddrs.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  let previousAddrsTexts =
    node.switchInstance.peerInfo.addrs.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  var nextListen =
  ...
  let transportAddrs = try:
      cloneMultiaddrs(transport.publishedAddrs())
    except CatchableError:
      @[]
    except Defect:
      @[]
  var listenSeen = initHashSet[string]()
  var addrsSeen = initHashSet[string]()
  for addr in nextListen:
    let key = safeMultiaddrText(addr)
    if key.len > 0:
      listenSeen.incl(key)
  for addr in nextAddrs:
    ...
    if key.len > 0:
      addrsSeen.incl(key)
  for addr in transportAddrs:
    let key = safeMultiaddrText(addr)
    if key.len == 0:
      continue
    if key notin listenSeen:
      nextListen.add(addr)
      listenSeen.incl(key)
  ...
  let mergedListenTexts = mergedListen.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  let mergedAddrsTexts = mergedAddrs.mapIt(safeMultiaddrText(it)).filterIt(it.len > 0)
  result =
    mergedListenTexts != previousListenTexts or
    mergedAddrsTexts != previousAddrsTexts
  node.switchInstance.peerInfo.listenAddrs = mergedListen
  node.switchInstance.peerInfo.addrs = mergedAddrs
  rememberPeerInfoAddrTexts(node, mergedListen, mergedAddrs)
...
proc publishTsnetLocalPeerInfo(
  ...
  if node.peerInfoListenAddrTexts.len > 0:
    for text in node.peerInfoListenAddrTexts:
...
proc reconcileTailnetRelayListeners(n: NimNode): Future[void] {.async.} =
  ...
  if transport.isNil:
    return
  if not transport.listenerNeedsRepair():
    return
  let nowMs = nowMillis()
  recordEvent(n, "system_status", $(%*{
    ...
    "stage": "restarting",
    "updatedAtMs": nowMs,
    "providerReady": transport.providerReadySafe(),
  }))
  let repaired = await reconcileProviderListeners(transport)
  if repaired.isErr():
    recordEvent(n, "system_status", $(%*{
      "entity": "tailnet_listener",
      "stage": "failed",
  ...
  recordEvent(n, "system_status", $(%*{
    "entity": "tailnet_listener",
    "stage": (if transport.providerReadySafe(): "ready" else: "repair_started"),
    "updatedAtMs": nowMillis(),
    "providerReady": transport.providerReadySafe(),
  }))
...
proc scheduleBackgroundTailnetWarm(
  ...
  if transport.isNil:
    return
  if transport.providerReadySafe():
...
proc processTailnetWarmCompletion(n: NimNode) {.gcsafe, raises: [].} =
  if n.isNil:
    return
  if not n.tailnetWarmFinalizePending.load(moAcquire):
    return
...
proc replayDeferredHostNetworkStatusSideEffects(n: NimNode, reason: string) {.gcsafe, raises: [].} =
  if n.isNil or not n.deferredHostNetworkSideEffectsPending:
    return
  let hostStatus = n.deferredHostNetworkStatus
  ...
  n.deferredHostNetworkStatus = defaultHostNetworkStatusSnapshot()
  debugLog(
...
proc sanitizeHostNetworkStatusSnapshot(
  ...
  var status = defaultHostNetworkStatusSnapshot()
  if node.kind != JObject:
...
proc sanitizeHostNetworkStatus(node: JsonNode): JsonNode =
  hostNetworkStatusToJson(sanitizeHostNetworkStatusSnapshot(node))

proc currentHostNetworkStatus(n: NimNode): JsonNode {.gcsafe.} =
  let base =
    if n.isNil:
      defaultHostNetworkStatusSnapshot()
    else:
      n.hostNetworkStatus
  var status = base
  attachPublicIpProbeFields(status, currentPublicIpProbeState(n))
  result = hostNetworkStatusToJson(status)

proc buildHostNetworkEvent(status: HostNetworkStatusSnapshot): JsonNode =
  let normalized = hostNetworkStatusToJson(status)
  %* {
    "topic": "network_event",
    "kind": "host",
    "entity": "network",
...
proc applyHostNetworkStatus(n: NimNode, status: JsonNode): JsonNode =
  let previous =
    if n.isNil:
      ...
      n.hostNetworkStatus
  let normalized = sanitizeHostNetworkStatusSnapshot(status)
  n.hostNetworkStatus = normalized
  n.hostNetworkStatusUpdatedAt = normalized.timestampMs
  let preferredIpv4 =
    (if normalized.preferredIpv4.len > 0: normalized.preferredIpv4 else: normalized.localIpv4).strip()
  ...
  let connected = normalized.isConnected
  let networkChanged =
    previous.isConnected != connected or
    previous.networkType != normalized.networkType or
    previous.transport != normalized.transport or
    previous.preferredIpv4 != normalized.preferredIpv4 or
    previous.preferredIpv6 != normalized.preferredIpv6
  if not connected:
  ...
  attachPublicIpProbeFields(current, currentPublicIpProbeState(n))
  result = hostNetworkStatusToJson(current)
...
when defined(libp2p_msquic_experimental):
  ...
  proc defaultQuicRuntimePreference(): QuicRuntimePreference {.gcsafe, raises: [].} =
    qrpBuiltinOnly
  ...
  proc effectiveQuicRuntimePreference(
      extra: JsonNode
  ): Result[QuicRuntimePreference, string] {.gcsafe, raises: [].} =
...
proc configuredTsnetTransportBuilderConfig(
  ...
  result.controlProtocol =
    jsonGetStr(
      tsnetNode,
      "controlProtocol",
      jsonGetStr(tsnetNode, "control_protocol", "auto")
    ).strip()
  result.controlEndpoint =
    jsonGetStr(
      ...
      jsonGetStr(
        ...
        jsonGetStr(
          tsnetNode,
          "customControlEndpoint",
          jsonGetStr(tsnetNode, "custom_control_endpoint", "")
        )
      )
    ).strip()
    ...
    jsonGetStr(
      ...
      jsonGetStr(
        ...
        jsonGetStr(
          tsnetNode,
          "customRelayEndpoint",
          jsonGetStr(tsnetNode, "custom_relay_endpoint", "")
        )
      )
    ).strip()
  result.authKey =
...
proc buildTransportHealthPayload(
  ...
  when defined(libp2p_msquic_experimental):
    ...
    let requestedPreference =
      block:
        if not n.isNil:
          let prefRes = effectiveQuicRuntimePreference(n.cfg.extra)
          if prefRes.isOk:
...
template ffiNodeSafe(body: untyped): untyped =
  {.cast(gcsafe).}:
    body
...
proc emptyTailnetStatusPayload(
  ...
  result = %*{
    ...
    "providerRequestedBackend": "builtin-synthetic",
    "providerRuntimeRequested": false,
    "providerReady": false,
    "providerInApp": false,
    "legacyBridgeDeprecated": false,
...
proc buildTailnetStatusPayload(n: NimNode): JsonNode {.gcsafe, raises: [].} =
  ...
  let tailnetStart = buildTailnetStartStatusPayload(n)
  if jsonGetBool(tailnetStart, "inFlight", false):
    result = emptyTailnetStatusPayload(n, enabled = true)
    result["deferred"] = %true
    result["providerStage"] = %jsonGetStr(tailnetStart, "stage", "starting")
    result["reason"] = %"provider_start_inflight"
    return
  ...
  if not result.hasKey("providerKind"):
    result["providerKind"] = %"unknown"
  if not result.hasKey("providerRequestedBackend"):
    result["providerRequestedBackend"] = %"builtin-synthetic"
  if not result.hasKey("providerRuntimeRequested"):
    result["providerRuntimeRequested"] = %false
  if not result.hasKey("providerReady"):
    result["providerReady"] = %false
  if not result.hasKey("providerInApp"):
    result["providerInApp"] = %false
  if not result.hasKey("legacyBridgeDeprecated"):
    result["legacyBridgeDeprecated"] = %false
  if not result.hasKey("providerCapabilities"):
  ...
  if not result.hasKey("providerStage"):
    result["providerStage"] =
      %(if jsonGetBool(result, "providerReady", false): "ready" else: "idle")
  let stage = jsonGetStr(tailnetStart, "stage", "idle")
  let inFlight = jsonGetBool(tailnetStart, "inFlight", false)
  let startErr = jsonGetStr(tailnetStart, "error")
  if not jsonGetBool(result, "providerReady", false):
    case stage
    of "queued", "starting":
      result["providerStage"] = %"starting"
    of "started":
      result["providerStage"] = %"ready"
    of "failed":
      result["providerStage"] = %"failed"
      if startErr.len > 0 and not result.hasKey("error"):
        result["error"] = %startErr
    else:
      if jsonGetBool(result, "deferred", false):
        result["providerStage"] = %"deferred"
      elif inFlight:
        result["providerStage"] = %"starting"
  result["providerStartStatus"] = tailnetStart
...
proc overlayTailnetFields(target: var JsonNode, tailnet: JsonNode) {.gcsafe, raises: [].} =
  ...
  for key in [
    ...
    "providerKind",
    "providerRequestedBackend",
    "providerRuntimeRequested",
    "providerReady",
    "providerInApp",
    "legacyBridgeDeprecated",
    "providerCapabilities",
    "providerError",
    "providerWarning",
  ]:
...
proc bootstrapSeedConnectTaskSessionLive(
    n: NimNode,
    task: BootstrapSeedConnectTask
): bool {.gcsafe, raises: [].}

proc copyDirectSessionTargetSpecs(
    n: NimNode
): seq[DirectSessionTargetSpec] {.gcsafe, raises: [].} =
  if n.isNil:
    return @[]
  n.directSessionTargetLock.acquire()
  try:
    for spec in n.directSessionTargets.values:
      result.add(spec)
  finally:
    n.directSessionTargetLock.release()

proc getOwnedDirectSessionTargetSpec(
    n: NimNode,
    taskKey: string
): DirectSessionTargetSpec {.gcsafe, raises: [].} =
  if n.isNil or taskKey.len == 0:
    return DirectSessionTargetSpec()
  n.directSessionTargetLock.acquire()
  try:
    result = n.directSessionTargets.getOrDefault(taskKey, DirectSessionTargetSpec())
  finally:
    n.directSessionTargetLock.release()

proc updatePublishedDirectSessionTargetSpec(
    n: NimNode,
    spec: DirectSessionTargetSpec,
  ...
  result = spec
  if n.isNil or spec.taskKey.len == 0:
    return
  let normalizedStatus = publishedStatus.strip().toLowerAscii()
  let normalizedError = publishedError.strip()
  let updatedAtMs = nowMillis()
  n.directSessionTargetLock.acquire()
  try:
    if spec.taskKey in n.directSessionTargets:
      result = n.directSessionTargets.getOrDefault(spec.taskKey, spec)
    if result.taskKey.len == 0:
      result = spec
    result.updatedAtMs = max(result.updatedAtMs, updatedAtMs)
    if normalizedStatus.len > 0 and normalizedStatus != result.lastPublishedStatus:
      case normalizedStatus
      of "running":
        inc result.attemptCount
        result.lastAttemptAtMs = updatedAtMs
      of "ready":
        inc result.successCount
        result.lastReadyAtMs = updatedAtMs
      of "failed":
        inc result.failureCount
        result.lastFailureAtMs = updatedAtMs
        result.lastFailureReason = normalizedError
      else:
        discard
    elif normalizedStatus == "failed" and normalizedError.len > 0:
      result.lastFailureReason = normalizedError
    if normalizedStatus.len > 0:
      result.lastPublishedStatus = normalizedStatus
    if spec.peerId.len > 0:
      result.peerId = spec.peerId
    if spec.source.len > 0:
      result.source = spec.source
    if spec.selectedAddrs.len > 0:
      result.selectedAddrs = spec.selectedAddrs
    if spec.taskKey in n.directSessionTargets:
      n.directSessionTargets[spec.taskKey] = result
  finally:
    n.directSessionTargetLock.release()

proc upsertDirectSessionTargetDesiredState(
    n: NimNode,
    peerIdText: string,
    addresses: seq[string],
    source: string
  ...
  let selectedAddrs =
    try:
      prioritizeGameSeedAddrs(preferStableAddrStrings(addresses))
    except CatchableError:
      @[]
    except Exception:
      @[]
  ...
  let safeSource =
    if source.strip().len > 0:
      ...
      "android_direct_target"
  n.directSessionTargetLock.acquire()
  try:
    result = n.directSessionTargets.getOrDefault(taskKey, DirectSessionTargetSpec())
    if result.taskKey.len == 0:
      result = DirectSessionTargetSpec(
        taskKey: taskKey,
        peerId: safePeerId,
        source: safeSource,
      ...
      result.peerId = safePeerId
      result.source = safeSource
      result.selectedAddrs = selectedAddrs
      result.updatedAtMs = nowMillis()
    n.directSessionTargets[taskKey] = result
  finally:
    n.directSessionTargetLock.release()

proc clearDirectSessionTargetDesiredState(
    n: NimNode,
    peerIdText: string,
    addresses: seq[string],
    source: string
  ...
  try:
    result = n.directSessionTargets.getOrDefault(taskKey, DirectSessionTargetSpec())
    if taskKey in n.directSessionTargets:
      n.directSessionTargets.del(taskKey)
  finally:
    n.directSessionTargetLock.release()
  if result.taskKey.len == 0:
    result = DirectSessionTargetSpec(
      taskKey: taskKey,
      peerId: safePeerId,
      source: if source.strip().len > 0: source.strip() else: "android_direct_target",
      selectedAddrs: selectedAddrs,
      updatedAtMs: nowMillis(),
    )

proc liveDirectSessionTargetSnapshot(
    n: NimNode,
    peerId: string,
    selectedAddrs: openArray[string]
): DirectSessionTargetSnapshot {.gcsafe, raises: [].} =
  let safePeerId = peerId.strip()
  let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
  if n.isNil or safePeerId.len == 0 or taskKey.len == 0:
    return DirectSessionTargetSnapshot(
      taskKey: taskKey,
      peerId: safePeerId,
      status: "missing",
  ...
  let spec = getOwnedDirectSessionTargetSpec(n, taskKey)
  let task = n.bootstrapSeedConnectTasks.getOrDefault(taskKey, nil)
  let ready = bootstrapSeedConnectTaskSessionLive(n, task)
  let running = bootstrapSeedConnectTaskRunning(n, task)
  let routeStatus = liveDirectRouteStatus(n, selectedAddrs)
  let effectiveStatus =
    if ready:
      "ready"
    elif running:
      "running"
    elif not task.isNil and task.state == bscsProbing:
      "probing"
    elif spec.taskKey.len == 0 and task.isNil:
      "missing"
    elif not task.isNil and task.state == bscsFailed:
      "failed"
    elif not task.isNil and task.state == bscsCancelled:
      "cleared"
    else:
      "pending"
  let effectiveError =
    if not task.isNil and task.sessionWarmError.len > 0:
      task.sessionWarmError
    elif not task.isNil:
      task.err
    else:
      ""
  let attemptCount = spec.attemptCount
  let successCount = spec.successCount
  let failureCount = spec.failureCount
  let lastAttemptAtMs = spec.lastAttemptAtMs
  let lastReadyAtMs = spec.lastReadyAtMs
  let lastFailureAtMs = spec.lastFailureAtMs
  let lastFailureReason = spec.lastFailureReason
  DirectSessionTargetSnapshot(
    taskKey: taskKey,
    peerId: safePeerId,
    ...
    ok: ready,
    taskRunning: running,
    sessionWarmOk: ready or (not task.isNil and task.sessionWarmOk),
    verifiedMuxer: not task.isNil and not task.verifiedMuxer.isNil,
    sessionConnKey: if task.isNil: 0 else: task.sessionConnKey,
    routeReady: routeStatus.ready,
    routePathKind: routeStatus.pathKind,
...
proc liveDirectSessionTargetStateJson(
    n: NimNode,
    peerId: string,
    selectedAddrs: openArray[string]
): string {.gcsafe, raises: [].} =
  let safePeerId = peerId.strip()
  let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
  let snapshot = liveDirectSessionTargetSnapshot(n, safePeerId, selectedAddrs)
  let selectedAddrsSeq = @selectedAddrs
  var payload = newJObject()
  payload["ok"] = %snapshot.ok
  payload["peerId"] = %safePeerId
  payload["selectedAddrs"] = %selectedAddrsSeq
  payload["status"] = %snapshot.status
  payload["taskKey"] = %taskKey
  payload["taskRunning"] = %snapshot.taskRunning
  payload["sessionWarmOk"] = %snapshot.sessionWarmOk
  payload["verifiedMuxer"] = %snapshot.verifiedMuxer
  payload["sessionConnKey"] = %snapshot.sessionConnKey
  payload["routeReady"] = %snapshot.routeReady
  payload["routePathKind"] = %snapshot.routePathKind
  payload["attemptCount"] = %snapshot.attemptCount
  payload["successCount"] = %snapshot.successCount
  payload["failureCount"] = %snapshot.failureCount
  payload["lastAttemptAtMs"] = %snapshot.lastAttemptAtMs
  payload["lastReadyAtMs"] = %snapshot.lastReadyAtMs
  payload["lastFailureAtMs"] = %snapshot.lastFailureAtMs
  payload["lastFailureReason"] = %snapshot.lastFailureReason
  if snapshot.error.len > 0:
    payload["error"] = %snapshot.error
  directSessionTargetStateJson(
    safePeerId,
    taskKey,
    snapshot.status,
    payload,
...
proc getLiveReadyDirectSessionTargetConnKey(
    n: NimNode,
    peerId: string,
    selectedAddrs: openArray[string]
): int {.gcsafe, raises: [].} =
  let snapshot = liveDirectSessionTargetSnapshot(n, peerId, selectedAddrs)
  if snapshot.status.toLowerAscii() != "ready" or snapshot.sessionConnKey <= 0:
    return 0
  snapshot.sessionConnKey

proc liveDirectRouteStatusForAddr(
    n: NimNode,
    addrText: string
): tuple[ready: bool, pathKind: string] {.gcsafe, raises: [].} =
  if n.isNil or n.switchInstance.isNil:
    return (false, "")
  let transport = n.switchInstance.tsnetTransport()
  if transport.isNil:
    return (false, "")
  let normalizedAddr = stripPeerIdSuffixFromMultiaddr(addrText)
  if normalizedAddr.len == 0:
    return (false, "")
  let maRes = parseDialableMultiaddr(normalizedAddr)
  if maRes.isErr():
    return (false, "")
  let directRouteRes = transport.lookupUdpDirectRouteTarget(maRes.get())
  if directRouteRes.isErr():
    return (false, "")
  (
    true,
    normalizeTailnetPath(directRouteRes.get().pathKind),
  )

proc liveDirectRouteStatus(
    n: NimNode,
    selectedAddrs: openArray[string]
): tuple[ready: bool, pathKind: string] {.gcsafe, raises: [].} =
  for addrText in selectedAddrs:
    let routeStatus = liveDirectRouteStatusForAddr(n, addrText)
    if not routeStatus.ready:
      continue
    if normalizeTailnetPath(routeStatus.pathKind) == TsnetPathDirect:
      return (true, TsnetPathDirect)
    if not result.ready:
      result = routeStatus

proc latestOwnedDirectSessionTargetSnapshot(
    n: NimNode,
    peerIdText: string
): DirectSessionTargetSnapshot {.gcsafe, raises: [].} =
  let safePeerId = peerIdText.strip()
  ...
  ffiNodeSafe:
    getLatestDirectSessionTargetSnapshot(safePeerId)

proc directTargetReady(
    snapshot: DirectSessionTargetSnapshot
): bool {.gcsafe, raises: [].} =
  snapshot.status.toLowerAscii() == "ready" and
    snapshot.routeReady and
    snapshot.sessionWarmOk and
    snapshot.sessionConnKey > 0

proc directTargetProbing(
    snapshot: DirectSessionTargetSnapshot
): bool {.gcsafe, raises: [].} =
  snapshot.taskRunning or snapshot.status.toLowerAscii() in ["pending", "running", "probing"]

proc directTargetProjectedPathStatus(
    snapshot: DirectSessionTargetSnapshot
): string {.gcsafe, raises: [].} =
  if directTargetReady(snapshot):
    return "DIRECT"
  if directTargetProbing(snapshot):
    return "PROBING"
  ""

proc buildPeerPathProjection(
    n: NimNode,
    peerIdText: string,
    peerId: PeerId
): JsonNode {.gcsafe, raises: [].} =
  let connSnapshot = actualPeerConnectionSnapshot(n, peerIdText, peerId)
  let selectedPath = jsonGetStr(connSnapshot, "selectedPath", "").toLowerAscii()
  let relayConnected = jsonGetBool(connSnapshot, "relayConnected", false)
  let directConnected = jsonGetBool(connSnapshot, "directConnected", false)
  let directSnapshot = latestOwnedDirectSessionTargetSnapshot(n, peerIdText)
  let relaySnapshot = latestPeerRouteSessionSnapshot(n, peerIdText)
  let directPathStatus = directTargetProjectedPathStatus(directSnapshot)
  let directReady = directTargetReady(directSnapshot)
  let directProbing = directTargetProbing(directSnapshot)
  let relayReady = peerRouteSessionReady(relaySnapshot)
  let relayProbing = peerRouteSessionProbing(relaySnapshot)
  let directSuccessRatePct =
    if directSnapshot.attemptCount > 0:
      int((directSnapshot.successCount * 100) div directSnapshot.attemptCount)
    else:
      0
  let transport =
    if selectedPath == "direct" or directConnected:
      "QUIC"
    elif selectedPath == "relay" or relayConnected or relayReady:
      "RELAY"
    else:
      ""
  let pathStatus =
    if selectedPath == "direct" or directConnected or directReady:
      "DIRECT"
    elif selectedPath == "relay" or relayConnected or relayReady:
      if directProbing: "PROBING" else: "RELAY"
    else:
      if directPathStatus.len > 0:
        directPathStatus
      elif relayProbing:
        "PROBING"
      else:
  ...
  result = %*{
    ...
    "directTargetStatus": directSnapshot.status,
    "directTargetReady": directReady,
    "directTargetTaskRunning": directProbing,
    "directTargetSessionConnKey": directSnapshot.sessionConnKey,
    "directTargetRouteReady": directSnapshot.routeReady,
    "directTargetRoutePathKind": directSnapshot.routePathKind,
    "directTargetAttemptCount": directSnapshot.attemptCount,
    "directTargetSuccessCount": directSnapshot.successCount,
    "directTargetFailureCount": directSnapshot.failureCount,
    "directTargetSuccessRatePct": directSuccessRatePct,
    "directTargetLastAttemptAtMs": directSnapshot.lastAttemptAtMs,
    "directTargetLastReadyAtMs": directSnapshot.lastReadyAtMs,
    "directTargetLastFailureAtMs": directSnapshot.lastFailureAtMs,
    "relayTargetStatus": relaySnapshot.status,
    "relayTargetReady": relayReady,
    "relayTargetTaskRunning": relayProbing,
    "relayTargetSessionConnKey": relaySnapshot.sessionConnKey,
  }
  if directSnapshot.error.len > 0:
    result["directTargetError"] = %directSnapshot.error
  if directSnapshot.lastFailureReason.len > 0:
    result["directTargetLastFailureReason"] = %directSnapshot.lastFailureReason
  if relaySnapshot.error.len > 0:
    result["relayTargetError"] = %relaySnapshot.error
...
proc sendProbeDirectMessage(
  ...
  try:
    return await n.dmService.send(remotePid, payload, ackRequested, messageId, timeout)
  except CatchableError as exc:
...
proc markPeerIdentified(n: NimNode, peerIdText: string) {.gcsafe, raises: [].} =
  if n.isNil:
    return
  let safePeerId = peerIdText.strip()
  if safePeerId.len == 0:
    return
  let nowMs = nowMillis()
  n.peerStateLock.acquire()
  try:
    n.peerIdentifiedAt[safePeerId] = nowMs
  finally:
    n.peerStateLock.release()
  debugLog(
    "[nimlibp2p] peer identified peer=" & safePeerId &
    " atMs=" & $nowMs
  )

proc clearPeerIdentified(n: NimNode, peerIdText: string) {.gcsafe, raises: [].} =
  if n.isNil:
    return
  let safePeerId = peerIdText.strip()
  if safePeerId.len == 0:
  ...
  n.peerStateLock.acquire()
  try:
    if n.peerIdentifiedAt.hasKey(safePeerId):
      n.peerIdentifiedAt.del(safePeerId)
      removed = true
  finally:
    n.peerStateLock.release()
  if removed:
    debugLog("[nimlibp2p] peer identify cleared peer=" & safePeerId)

proc peerCurrentlyIdentified(n: NimNode, peer: PeerId): bool {.gcsafe, raises: [].} =
  if n.isNil or peer.len == 0:
    return false
  let peerText = $peer
  n.peerStateLock.acquire()
  try:
    result = n.peerIdentifiedAt.hasKey(peerText)
  finally:
    n.peerStateLock.release()
  if result:
    return true
  try:
    ...
    if not store.isNil:
      let protoBook = store[ProtoBook]
      if not protoBook.isNil and protoBook[peer].len > 0:
        markPeerIdentified(n, peerText)
        return true
      let agentBook = store[AgentBook]
      if not agentBook.isNil and agentBook[peer].len > 0:
        markPeerIdentified(n, peerText)
        return true
      let protoVersionBook = store[ProtoVersionBook]
      if not protoVersionBook.isNil and protoVersionBook[peer].len > 0:
        markPeerIdentified(n, peerText)
        return true
  except CatchableError:
    discard

proc waitForPeerIdentifiedWithinDeadline(
    n: NimNode,
    peer: PeerId,
    timeoutMs: int,
    pollMs = 25,
  ...
  if peer.len == 0:
    return false
  if peerCurrentlyIdentified(n, peer):
    return true
...
proc waitForPeerIdentifiedWithinDeadlineSync(
    n: NimNode,
    peer: PeerId,
    timeoutMs: int,
    pollMs = 100,
...
proc ensurePeerIdentified(
  ...
  if peerCurrentlyIdentified(n, peer):
    return true
  if n.switchInstance.isNil or n.switchInstance.connManager.isNil:
    return false

  let identifyWaitMs = max(timeoutMs, 0)
  let peerText = $peer
  let muxer = n.switchInstance.connManager.selectMuxer(peer)
  if muxer.isNil:
    debugLog(
      "[nimlibp2p] ensurePeerIdentified missing muxer peer=" & peerText &
      " timeoutMs=" & $identifyWaitMs
    )
    return await waitForPeerIdentifiedWithinDeadline(n, peer, identifyWaitMs)

  if n.switchInstance.peerStore.isNil:
    debugLog(
      "[nimlibp2p] ensurePeerIdentified missing peerStore peer=" & peerText &
      " timeoutMs=" & $identifyWaitMs
    )
    return await waitForPeerIdentifiedWithinDeadline(n, peer, identifyWaitMs)

  when defined(libp2p_msquic_experimental):
    if not muxer.connection.isNil and muxer.connection of msquicconnection.MsQuicConnection:
      # Native QUIC has already authenticated the remote peer at secure-channel
      # setup. For DM hot paths, forcing a second eager /ipfs/id stream here
      # just races the first application stream and tears the fresh connection
      # down. Treat the live secure muxer as peer-ready and defer identify
      # metadata exchange out of the send critical path.
      markPeerIdentified(n, peerText)
      debugLog(
        "[nimlibp2p] ensurePeerIdentified native_quic skip explicit identify peer=" &
        peerText
      )
      return true
  ...
  try:
    ...
    debugLog(
      "[nimlibp2p] ensurePeerIdentified explicit identify begin peer=" & peerText &
      " previousPeer=" & $previousPeerId
    )
    await n.switchInstance.peerStore.identify(muxer)
    n.switchInstance.connManager.reindexMuxerPeerId(
      muxer,
      previousPeerId,
      muxer.connection.peerId,
    )
    await n.switchInstance.connManager.triggerPeerEvents(
      muxer.connection.peerId,
      PeerEvent(
        kind: PeerEventKind.Identified,
        initiator: muxer.connection.dir == Direction.Out,
      ),
    )
    debugLog(
      "[nimlibp2p] ensurePeerIdentified explicit identify done peer=" & peerText &
      " resolvedPeer=" & $muxer.connection.peerId
    )
  except CancelledError as exc:
    debugLog(
      "[nimlibp2p] ensurePeerIdentified cancelled peer=" & peerText &
      " err=" & exc.msg
    )
    return false
  except CatchableError as exc:
...
proc updateStartupStatus(
  ...
  let nowMs = nowMillis()
  var changed = false
  n.startupStatusLock.acquire()
  try:
    if stage == "queued":
      n.startupQueuedAtMs = nowMs
      inc n.startupAttempts
      changed = true
    elif n.startupStage != stage or n.startupInFlight != inFlight or n.startupError != errorMsg:
      changed = true
    n.startupStage = stage
    n.startupInFlight = inFlight
    n.startupError = errorMsg
    n.startupUpdatedAtMs = nowMs
    if not inFlight and n.startupQueuedAtMs > 0:
      n.startupLastDurationMs = max(0'i64, nowMs - n.startupQueuedAtMs)
  finally:
    n.startupStatusLock.release()
  if changed:
    debugLog(
      "[nimlibp2p] startup status stage=" & stage &
      " inFlight=" & $inFlight &
...
proc buildStartupStatusPayload(n: NimNode): JsonNode {.gcsafe, raises: [].} =
  ...
  n.startupStatusLock.acquire()
  try:
    result["started"] = %n.started
    result["inFlight"] = %n.startupInFlight
    result["stage"] = %n.startupStage
    result["error"] = %n.startupError
    result["queuedAtMs"] = %n.startupQueuedAtMs
    result["updatedAtMs"] = %n.startupUpdatedAtMs
    result["lastDurationMs"] = %n.startupLastDurationMs
    result["attemptCount"] = %n.startupAttempts
  finally:
    n.startupStatusLock.release()
  result["pendingCommands"] = %n.pendingCommands.load(moAcquire)
  result["queuedCommands"] = %n.queuedCommands.load(moAcquire)
  result["peerId"] = %localPeerId(n)
...
proc updateTailnetStartStatus(
  ...
  try:
    if stage == "queued":
      n.tailnetQueuedAtMs = nowMs
      inc n.tailnetAttempts
      changed = true
    elif n.tailnetStage != stage or n.tailnetInFlight != inFlight or n.tailnetError != errorMsg:
      changed = true
    n.tailnetStage = stage
    n.tailnetInFlight = inFlight
    n.tailnetError = errorMsg
    n.tailnetUpdatedAtMs = nowMs
    if not inFlight and n.tailnetQueuedAtMs > 0:
      n.tailnetLastDurationMs = max(0'i64, nowMs - n.tailnetQueuedAtMs)
  finally:
    n.tailnetStatusLock.release()
  if changed:
    debugLog(
      "[nimlibp2p] tailnet status stage=" & stage &
      " inFlight=" & $inFlight &
      (if errorMsg.len > 0: " error=" & errorMsg else: "")
    )
    let payload = %*{
...
proc buildTailnetStartStatusPayload(n: NimNode): JsonNode {.gcsafe, raises: [].} =
  ...
  n.tailnetStatusLock.acquire()
  try:
    result["started"] = %(n.tailnetStage == "started")
    result["inFlight"] = %n.tailnetInFlight
    result["stage"] = %n.tailnetStage
    result["error"] = %n.tailnetError
    result["queuedAtMs"] = %n.tailnetQueuedAtMs
    result["updatedAtMs"] = %n.tailnetUpdatedAtMs
    result["lastDurationMs"] = %n.tailnetLastDurationMs
    result["attemptCount"] = %n.tailnetAttempts
  finally:
    n.tailnetStatusLock.release()
  result["pendingCommands"] = %n.pendingCommands.load(moAcquire)
  result["queuedCommands"] = %n.queuedCommands.load(moAcquire)
...
proc joinTailnetWarmThreadIfNeeded(n: NimNode) {.gcsafe, raises: [].} =
  if n.isNil:
    return
  if n.tailnetWarmThreadJoined.load(moAcquire):
    return
...
proc tailnetWarmWorker(handle: pointer) {.thread.} =
  ...
  let node = ffiNodeSafe:
    nodeFromHandle(handle)
  if node.isNil:
    return
  debugLog("[nimlibp2p] tailnet warm worker start")
  let transport = ffiNodeSafe:
    activeTsnetTransport(node)
  ...
  debugLog(
    "[nimlibp2p] tailnet warm worker finalize pending=" &
    $node.tailnetWarmFinalizePending.load(moAcquire) &
    " success=" & $warmSucceeded
  )
  if not node.signal.isNil:
    discard node.signal.fireSync(chronos.milliseconds(5))
...
proc newDirectWaiter(timeoutMs: int): DirectWaiter {.gcsafe, raises: [].}
...
proc awaitDirectAckWaiter(
  ...
  if n.isNil or messageId.len == 0:
    return (false, "missing_direct_ack_waiter")
  if not n.directWaiters.hasKey(messageId):
    return (false, "missing_direct_ack_waiter")
  let waiter = n.directWaiters.getOrDefault(messageId)
  if waiter.isNil or waiter.future.isNil:
    if n.directWaiters.hasKey(messageId):
      n.directWaiters.del(messageId)
    return (false, "invalid_direct_ack_waiter")
  let waitBudget = timeout + chronos.milliseconds(1_500)
  let timeoutFuture = sleepAsync(waitBudget)
  discard await race(waiter.future, timeoutFuture)
  let timedOut = not waiter.future.finished()
  ...
  if timedOut:
    ...
    let errMsg =
      if waiter.error.len > 0: waiter.error
      else: "ack timeout"
    return (false, errMsg)
  let success =
    try:
      ...
      if n.directWaiters.hasKey(messageId):
        n.directWaiters.del(messageId)
      return (false, if exc.msg.len > 0: exc.msg else: "ack wait failed")
  if n.directWaiters.hasKey(messageId):
    n.directWaiters.del(messageId)
  if not success:
    let errMsg =
...
proc sendDirectWithAckWaiter(
  ...
  ## Authoritative direct-message ack is modeled as an `op=ack` control frame
  ## on the same long-lived DM session. Do not wait for a reply on a short-lived
  ## one-shot message stream.
  try:
    return await n.dmService.send(
      remotePid,
      payload,
...
proc finalizeReceivedDirectMessageAsync(
    n: NimNode,
    fromPeer: string,
    convId: string,
    mid: string,
  ...
  try:
    await sleepAsync(0)
    warn "handleDirectMessageNode postprocess begin",
      peer = fromPeer,
      mid = mid,
      convId = convId
    if fromPeer.len > 0 and convId.len > 0:
      {.cast(gcsafe).}:
        warn "handleDirectMessageNode socialState lock wait", peer = fromPeer, mid = mid, convId = convId
        socialStateMemLock.acquire()
        try:
          warn "handleDirectMessageNode socialState lock acquired", peer = fromPeer, mid = mid, convId = convId
          warn "handleDirectMessageNode socialState load begin", peer = fromPeer, mid = mid, convId = convId
          let state = loadSocialStateUnsafe(n)
          warn "handleDirectMessageNode socialState load done", peer = fromPeer, mid = mid, convId = convId
          let localPeer = localPeerId(n)
          if fromPeer != localPeer:
            let persistToSocial = shouldPersistDirectMessageToSocial(payloadNode)
            warn "handleDirectMessageNode social persist decision",
              peer = fromPeer,
              mid = mid,
              convId = convId,
              persist = persistToSocial
            if persistToSocial:
              ...
              warn "handleDirectMessageNode appendConversation begin", peer = fromPeer, mid = mid, convId = convId
              appendConversationMessageNoLock(state, convId, fromPeer, message)
              warn "handleDirectMessageNode appendConversation done", peer = fromPeer, mid = mid, convId = convId
              ...
              warn "handleDirectMessageNode appendNotification begin", peer = fromPeer, mid = mid, convId = convId
              appendEarlyNotificationNoLock(state, "dm_received", "新消息", body, notificationPayload, "priority")
              warn "handleDirectMessageNode appendNotification done", peer = fromPeer, mid = mid, convId = convId
              warn "handleDirectMessageNode persistSocialState begin", peer = fromPeer, mid = mid, convId = convId
              persistSocialStateUnsafe(n, state)
              warn "handleDirectMessageNode persistSocialState done", peer = fromPeer, mid = mid, convId = convId
            else:
              warn "handleDirectMessageNode social persist skipped", peer = fromPeer, mid = mid, convId = convId
          else:
            warn "handleDirectMessageNode social persist skipped self", peer = fromPeer, mid = mid, convId = convId
        finally:
          warn "handleDirectMessageNode socialState lock release", peer = fromPeer, mid = mid, convId = convId
          socialStateMemLock.release()
    var event = newJObject()
    ...
    event["transport"] = %"nim-direct"
    warn "handleDirectMessageNode networkEvent begin", peer = fromPeer, mid = mid, convId = convId
    recordEvent(n, "network_event", $event)
    warn "handleDirectMessageNode networkEvent done", peer = fromPeer, mid = mid, convId = convId
    if fromPeer.len > 0 and convId.len > 0:
      ...
      warn "handleDirectMessageNode socialEvent begin", peer = fromPeer, mid = mid, convId = convId
      recordEvent(n, "social_event", $socialEvt)
      warn "handleDirectMessageNode socialEvent done", peer = fromPeer, mid = mid, convId = convId
    warn "handleDirectMessageNode postprocess done",
      peer = fromPeer,
      mid = mid,
      convId = convId
  except Exception as exc:
    warn "handleDirectMessageNode postprocess failed", peer = fromPeer, mid = mid, convId = convId, err = exc.msg
...
proc handleDirectMessageNode(
  ...
  try:
    ...
    warn "handleDirectMessageNode begin",
      peer = fromPeer,
      mid = mid,
      convId = convId,
      ackRequested = ackRequested,
      bodyLen = body.len
    if fromPeer.len > 0 and convId.len > 0:
      warn "handleDirectMessageNode recordReceivedDm begin", peer = fromPeer, mid = mid, convId = convId
      recordReceivedDm(n, fromPeer, convId, mid, body, ts, payloadNode)
      warn "handleDirectMessageNode recordReceivedDm done", peer = fromPeer, mid = mid, convId = convId
      asyncSpawn(finalizeReceivedDirectMessageAsync(
        n,
        fromPeer,
        convId,
        mid,
        body,
        ts,
        cloneJson(payloadNode)
      ))
    if ackRequested and fromPeer.len > 0 and originalMid.len > 0:
      ## DirectMessageService already sends the authoritative same-session
      ## direct ack control frame. Do not emit a second app-level `op=ack` here.
      debugLog(
        "[nimlibp2p] dm ack satisfied via same-session direct ack peer=" & fromPeer &
          " mid=" & originalMid &
          " senderPeer=" & (if senderPeer.len > 0: senderPeer else: "<empty>")
      )
    warn "handleDirectMessageNode done",
      peer = fromPeer,
      mid = mid,
      convId = convId,
      ackRequested = ackRequested
...
proc refreshLanAwareAddrs(n: NimNode): seq[MultiAddress] {.gcsafe.} =
  ...
  if wantsTsnetTransport(n.cfg):
    let publishedTexts = currentTsnetPublishedAddrTexts(n)
    if publishedTexts.len > 0:
      let published = parseMultiaddrsFromTexts(publishedTexts)
      debugLog(
        "[nimlibp2p] refreshLanAwareAddrs tsnet_only published=" &
        publishedTexts.join(",")
      )
      n.switchInstance.peerInfo.listenAddrs = published
      n.switchInstance.peerInfo.addrs = published
      rememberPeerInfoAddrTexts(n, publishedTexts, publishedTexts)
      return published
    if n.peerInfoListenAddrTexts.len > 0:
      let cached = parseMultiaddrsFromTexts(n.peerInfoListenAddrTexts)
      if cached.len > 0:
        debugLog(
          "[nimlibp2p] refreshLanAwareAddrs tsnet_only cached=" &
          n.peerInfoListenAddrTexts.join(",")
        )
        n.switchInstance.peerInfo.listenAddrs = cached
        n.switchInstance.peerInfo.addrs = cached
        rememberPeerInfoAddrTexts(n, n.peerInfoListenAddrTexts, n.peerInfoListenAddrTexts)
        return cached
    return @[]
  ...
  rememberPeerInfoAddrTexts(n, dedup, dedup)
  dedup
  ...
  if cfg.extra.kind == JObject:
    return parseRoutingPlaneMode(
      jsonGetStr(
        cfg.extra,
        "routingMode",
        jsonGetStr(cfg.extra, "routing_mode", "")
      )
    )
  RoutingPlaneMode.legacyOnly
...
proc configuredLsmrNetworkId(cfg: NodeConfig): string =
  if cfg.extra.kind == JObject:
    let raw = jsonGetStr(
      cfg.extra,
      "lsmrNetworkId",
      jsonGetStr(cfg.extra, "lsmr_network_id", "")
    ).strip()
    if raw.len > 0:
      return raw
  configuredWanNetworkId(cfg)
...
proc configuredLsmrAnchors(cfg: NodeConfig): seq[LsmrAnchor] =
  ...
  for item in node:
    ...
    let regionRaw = max(
      0'i64,
      jsonGetInt64(item, "regionDigit", jsonGetInt64(item, "region_digit", 5))
    )
    let serveDepthRaw = max(
      0'i64,
      jsonGetInt64(item, "serveDepth", jsonGetInt64(item, "serve_depth", 0))
    )
    let directionMaskRaw = max(
      0'i64,
      jsonGetInt64(item, "directionMask", jsonGetInt64(item, "direction_mask", 0x1ff))
    )
    result.add(
      LsmrAnchor(
        peerId: peerId,
        ...
        regionDigit: uint8(min(9'i64, max(1'i64, regionRaw))),
        attestedPrefix:
          parseLsmrPath(item.getOrDefault("attestedPrefix")),
...
proc parseNearFieldObservation(payload: string): Result[NearFieldObservation, string] =
  let node =
    try:
      ...
      return err("invalid near field observation json: " & exc.msg)
  if node.kind != JObject:
    return err("near field observation must be an object")
  let providerText = jsonGetStr(
    node,
    "provider",
    jsonGetStr(node, "nearFieldProvider", jsonGetStr(node, "near_field_provider", ""))
  )
  let provider = parseNearFieldBootstrapProvider(providerText)
  if provider.isNone():
    return err("invalid near field provider")
  let peerText = jsonGetStr(node, "peerId", jsonGetStr(node, "peer_id", "")).strip()
  if peerText.len == 0:
    return err("missing peerId")
  let peerId = PeerId.init(peerText).valueOr:
    return err("invalid peerId")
  var addrs: seq[MultiAddress] = @[]
  for key in ["addrs", "multiaddrs", "addresses"]:
    for raw in multiaddrsFromExtra(node, key):
      let parsed = MultiAddress.init(raw)
      if parsed.isOk():
        addrs.add(parsed.get())
  if addrs.len == 0:
    return err("near field observation missing addresses")
  let regionRaw = max(
    0'i64,
    jsonGetInt64(node, "regionDigit", jsonGetInt64(node, "region_digit", 0))
  )
  let serveDepthRaw = max(
    0'i64,
    jsonGetInt64(node, "serveDepth", jsonGetInt64(node, "serve_depth", 0))
  )
  let directionMaskRaw = max(
    0'i64,
    jsonGetInt64(node, "directionMask", jsonGetInt64(node, "direction_mask", 0x1ff))
  )
  ok(
    NearFieldObservation(
      provider: provider.get(),
      networkId: jsonGetStr(node, "networkId", jsonGetStr(node, "network_id", "")),
      peerId: peerId,
      addrs: addrs,
      operatorId: jsonGetStr(node, "operatorId", jsonGetStr(node, "operator_id", "")),
      regionDigit: uint8(min(9'i64, max(0'i64, regionRaw))),
      attestedPrefix:
        parseLsmrPath(
          if not node.getOrDefault("attestedPrefix").isNil:
            node.getOrDefault("attestedPrefix")
          else:
            node.getOrDefault("attested_prefix")
        ),
      serveDepth: uint8(min(255'i64, serveDepthRaw)),
      directionMask: uint32(directionMaskRaw),
      canIssueRootCert:
        jsonGetBool(
          node,
          "canIssueRootCert",
          jsonGetBool(node, "can_issue_root_cert", false)
        ),
      rssi: int32(
        jsonGetInt64(node, "rssi", jsonGetInt64(node, "signal", 0))
      ),
      observedAtMs: max(
        0'i64,
        jsonGetInt64(node, "observedAtMs", jsonGetInt64(node, "observed_at_ms", nowMillis()))
      ),
      ttlMs: max(0'i64, jsonGetInt64(node, "ttlMs", jsonGetInt64(node, "ttl_ms", 0))),
    )
  )
...
proc peerSecureChannelReady(n: NimNode, peer: PeerId): bool =
  if n.isNil or n.switchInstance.isNil:
    return false
  try:
...
proc cancelFutureAndDrainDetached[T](
    fut: Future[T],
    context: string
) {.async: (raises: []).} =
  if fut.isNil or fut.finished():
  ...
  try:
    await fut.cancelAndWait()
    debugLog("[nimlibp2p] future cancel detached complete context=" & context)
  except CancelledError as exc:
    debugLog(
      "[nimlibp2p] future cancel detached cancelled context=" & context &
      " err=" & exc.msg
    )
  except CatchableError as exc:
    debugLog(
      "[nimlibp2p] future cancel detached failed context=" & context &
      " err=" & exc.msg
    )
...
proc awaitFutureWithinDeadlineOrPeerConnected[T](
  ...
  if fut.finished():
    return (true, false)
  if peer.len > 0 and peerSecureChannelReady(n, peer):
    return (false, true)
  let safeTimeoutMs = max(timeoutMs, 0)
  if safeTimeoutMs == 0:
    return (fut.finished(), peer.len > 0 and peerSecureChannelReady(n, peer))
  let deadlineMs = nowMillis() + int64(safeTimeoutMs)
  let safePollMs = max(pollMs, 5)
  while not fut.finished():
    if peer.len > 0 and peerSecureChannelReady(n, peer):
...
proc awaitCommandCompletionAsync(
    cmd: Command,
    timeoutMs: int,
    pollMs = 25
): Future[bool] {.async.} =
  if cmd.isNil:
    return false
  if cmd.completed.load(moAcquire):
    return true
  let safeTimeoutMs = max(timeoutMs, 0)
  if safeTimeoutMs == 0:
    return cmd.completed.load(moAcquire)
  let deadlineMs = nowMillis() + int64(safeTimeoutMs)
  let safePollMs = max(pollMs, 5)
  while not cmd.completed.load(moAcquire):
    let remainingMs = int(deadlineMs - nowMillis())
    if remainingMs <= 0:
      return cmd.completed.load(moAcquire)
    await sleepAsync(chronos.milliseconds(min(safePollMs, remainingMs)))
...
proc drainCommandDetached(
  ...
  if cmd.isNil:
    return
  try:
    let completed = await awaitCommandCompletionAsync(cmd, timeoutMs)
    debugLog(
      "[nimlibp2p] command detached drain context=" & context &
      " completed=" & $completed &
      " rc=" & $int(cmd.resultCode) &
      (if cmd.errorMsg.len > 0: " err=" & cmd.errorMsg else: "")
    )
  except CatchableError as exc:
    debugLog(
      "[nimlibp2p] command detached drain failed context=" & context &
      " err=" & exc.msg
    )
  finally:
    destroyCommand(cmd)
...
proc connectMultiaddrDeferredMuxer(
    n: NimNode,
    ma: MultiAddress
): Future[Muxer] {.async.} =
  await sleepAsync(chronos.milliseconds(0))
  return await n.switchInstance.connectMuxer(
    ma,
    allowUnknownPeerId = true,
...
type WarmSeededDirectSessionResult = object
  verifiedMuxer: Muxer
  verifiedMuxerWaitMs: int
  sessionConnKey: int
  sessionWarmOk: bool
...
proc muxerUsable(
  ...
  not muxer.isNil and
    ...
    not muxer.connection.closed

proc selectVerifiedMuxer(
    n: NimNode,
    peerId: PeerId
): Muxer {.gcsafe, raises: [].} =
  if n.isNil or peerId.len == 0 or n.switchInstance.isNil or n.switchInstance.connManager.isNil:
    return nil
  let muxer = n.switchInstance.connManager.selectMuxer(peerId)
  if muxerUsable(muxer):
    return muxer
  nil

proc waitForVerifiedMuxerWithinDeadline(
    n: NimNode,
    peerId: PeerId,
    timeoutMs: int,
    pollMs = 25
  ...
  let deadlineMs = nowMillis() + int64(safeTimeoutMs)
  while true:
    let muxer = selectVerifiedMuxer(n, peerId)
    if not muxer.isNil:
      return muxer
    let remainingMs = int(deadlineMs - nowMillis())
    if remainingMs <= 0:
...
proc warmSeededDirectSessionAsync(
    n: NimNode,
    peerId: PeerId,
    peerIdText: string,
    addrText: string,
    timeoutMs: int
): Future[WarmSeededDirectSessionResult] {.async.} =
  var detail = WarmSeededDirectSessionResult(
    verifiedMuxer: nil,
    verifiedMuxerWaitMs: 0,
    sessionConnKey: 0,
    sessionWarmOk: false,
    sessionWarmError: "",
  )
  if n.isNil or n.dmService.isNil or peerId.len == 0 or addrText.strip().len == 0:
    return detail
  let safeTimeoutMs = max(timeoutMs, 0)
  let warmBudgetMs = min(max(safeTimeoutMs, 0), 10_000)
  if warmBudgetMs <= 0:
    detail.sessionWarmError = "warm_budget_exhausted"
    return detail
  let warmRes = await n.dmService.warmSessionDetailed(
    peerId,
    chronos.milliseconds(warmBudgetMs),
    preferredAddrs = parsePreferredDirectMultiaddrs(@[addrText]),
    verifiedMuxer = nil,
    preferredConnKey = 0,
    reuseVerifiedLiveConnection = false,
  )
  detail.sessionConnKey = warmRes.connKey
  detail.sessionWarmOk = warmRes.ok
  if warmRes.err.len > 0:
    detail.sessionWarmError = warmRes.err
  debugLog(
    "[nimlibp2p] warmSeededDirectSessionAsync peer=" & peerIdText &
    " addr=" & addrText &
    " connKey=" & $detail.sessionConnKey &
    " warmOk=" & $warmRes.ok &
    (if warmRes.err.len > 0: " err=" & warmRes.err else: "")
  )
  detail
...
proc connectMultiaddrExactAsync(
  ...
  result = DirectMultiaddrConnectResult(
    ok: false,
    err: "",
    peerConnected: false,
    verifiedMuxer: nil,
  ...
  try:
    if fullAddressRes.isOk():
      ...
      let addrFut = connectMultiaddrDeferredMuxer(n, ma)
      ...
      try:
        ...
        if waitRes.completed:
          let muxer = await addrFut
          result.verifiedMuxer = muxer
          result.ok = not muxer.isNil
          result.peerConnected =
            result.ok and not muxer.connection.isNil and not muxer.connection.closed
          if not result.ok:
            result.err = "peer muxer not ready"
        elif waitRes.observedConnected:
          result.peerConnected = true
          let elapsedMs = int(max(0'i64, nowMillis() - connectStartedAtMs))
          let remainingMuxerWaitMs = max(timeoutMs - elapsedMs, 0)
          if remainingMuxerWaitMs > 0:
            let muxerCompleted = await awaitFutureWithinDeadline(
              addrFut,
              remainingMuxerWaitMs,
            )
            if muxerCompleted:
              let muxer = await addrFut
              result.verifiedMuxer = muxer
              result.ok = not muxer.isNil
              result.peerConnected = result.peerConnected or
                (result.ok and not muxer.connection.isNil and not muxer.connection.closed)
              if not result.ok:
                result.err = "peer muxer not ready"
            else:
            ...
            result.err = "peer muxer not ready"
          if not result.ok and not addrFut.finished():
            if allowLateDetach and quicDial:
              asyncSpawn cancelFutureAndDrainDetached(
                addrFut,
                "connectMultiaddrExactAsync_observed_connected_muxer_timeout:" & addrText,
              )
            else:
              asyncSpawn cancelFutureAndDrainDetached(
                addrFut,
          ...
          if not result.ok and quicDial and peerCurrentlyConnected(n, peerId):
            discard await disconnectPeerWithTimeout(
              n,
              peerId,
              min(timeoutMs, 750),
              "connectMultiaddrExactAsync_quic_muxer_timeout_cleanup",
            )
        else:
      ...
      try:
        ...
        if completed:
          ...
          result.verifiedMuxer = muxer
          result.ok = not muxer.isNil
          if not muxer.isNil and not muxer.connection.isNil:
            result.peerConnected = not muxer.connection.closed
          if not result.ok:
            result.err = "peer muxer not ready"
        else:
          result.err = "connect timeout"
...
proc setupConnectionEvents(n: NimNode) =
  ...
  let identifiedHandler =
    proc(peerId: PeerId, event: PeerEvent): Future[void] {.closure, async: (raises: [CancelledError]).} =
      let peerText = $peerId
      n.bootstrapLastSeen[peerText] = nowMillis()
      markPeerIdentified(n, peerText)
      var ev = newJObject()
      ev["type"] = %"PeerIdentified"
      ev["peer_id"] = %peerText
      ev["initiator"] = %(event.initiator)
      ev["timestamp_ms"] = %nowMillis()
      recordEvent(n, "network_event", $ev)
  let leftHandler =
    proc(peerId: PeerId, event: PeerEvent): Future[void] {.closure, async: (raises: [CancelledError]).} =
      let peerText = $peerId
      clearPeerIdentified(n, peerText)
      var ev = newJObject()
      ev["type"] = %"PeerLeft"
      ev["peer_id"] = %peerText
      ev["timestamp_ms"] = %nowMillis()
  ...
  n.switchInstance.connManager.addPeerEventHandler(identifiedHandler, PeerEventKind.Identified)
  n.switchInstance.connManager.addPeerEventHandler(leftHandler, PeerEventKind.Left)
  n.connectionHandlers = @[(ConnEventKind.Connected, connectedHandler),
                           (ConnEventKind.Disconnected, disconnectedHandler)]
  n.peerHandlers = @[(PeerEventKind.Identified, identifiedHandler),
                     (PeerEventKind.Left, leftHandler)]
    ...
    while n.running.load(moAcquire):
      if n.started:
        ...
        if not transport.isNil:
          debugLog(
            "[nimlibp2p] tailnetControlRefresh step=ready_begin node=" &
            $cast[int](n)
          )
          let providerReady = ffiTsnetSafe:
            transport.providerReadySafe()
          debugLog(
            "[nimlibp2p] tailnetControlRefresh step=ready_done node=" &
            $cast[int](n) &
            " providerReady=" & $providerReady
          )
          if providerReady:
            debugLog(
              "[nimlibp2p] tailnetControlRefresh step=merge_begin node=" &
              $cast[int](n)
            )
            let changed = mergeTsnetListenAddrsIntoPeerInfo(n, transport)
            debugLog(
              "[nimlibp2p] tailnetControlRefresh step=merge_done node=" &
              $cast[int](n) &
              " changed=" & $changed
            )
            debugLog(
              "[nimlibp2p] tailnetControlRefresh step=publish_begin node=" &
              $cast[int](n)
            )
            publishTsnetLocalPeerInfo(n, transport)
            debugLog(
              "[nimlibp2p] tailnetControlRefresh step=publish_done node=" &
              $cast[int](n)
            )
            if changed:
              debugLog(
                "[nimlibp2p] tailnetControlRefresh step=warm_sync_begin node=" &
                $cast[int](n)
              )
              spawnTailnetWarmAdvertisedPeerInfo(n)
              debugLog(
                "[nimlibp2p] tailnetControlRefresh step=warm_sync_done node=" &
                $cast[int](n)
              )
...
proc buildSwitch(cfg: NodeConfig): Result[Switch, string] =
  ...
  let tsnetNode =
    if cfg.extra.kind == JObject and cfg.extra.hasKey("tsnet") and cfg.extra["tsnet"].kind == JObject:
      cfg.extra["tsnet"]
    else:
      newJObject()
  let disableDefaultTsnetListen =
    jsonGetBool(cfg.extra, "disableDefaultTsnetListen", false) or
    jsonGetBool(cfg.extra, "disable_default_tsnet_listen", false) or
    jsonGetBool(tsnetNode, "disableDefaultListen", false) or
    jsonGetBool(tsnetNode, "disable_default_listen", false) or
    jsonGetBool(tsnetNode, "noListen", false) or
    jsonGetBool(tsnetNode, "no_listen", false)
  var listenAddressStrings = cfg.listenAddresses
  if tsnetRequested and listenAddressStrings.len == 0 and not disableDefaultTsnetListen:
  ...
  if wanBootstrapEnabled(cfg):
    var wanCfg = configuredWanBootstrapConfig(cfg)
    case routingMode
    of RoutingPlaneMode.legacyOnly:
      builder = builder.withWanBootstrapProfile(wanCfg)
    of RoutingPlaneMode.dualStack:
      builder =
        builder.withWanBootstrapDualStackProfile(
          wanCfg,
          lsmrConfigOpt.get(),
          primary = primaryPlane,
        )
    of RoutingPlaneMode.lsmrOnly:
      wanCfg.routingMode = routingMode
      wanCfg.primaryPlane = primaryPlane
      wanCfg.lsmrConfig = lsmrConfigOpt
      builder =
        builder.withRoutingPlanes(routingMode, primaryPlane)
          .withLsmr(lsmrConfigOpt.get())
          .withWanBootstrapProfile(wanCfg)
    debugLog(
  ...
  elif lsmrConfigOpt.isSome():
    builder =
      builder.withRoutingPlanes(routingMode, primaryPlane)
        .withLsmr(lsmrConfigOpt.get())
    debugLog(
      "[nimlibp2p] buildSwitch enabled standalone LSMR mode=" &
  ...
  if tsnetRequested:
    ...
    when defined(libp2p_msquic_experimental):
      ...
      if prefRes.isErr:
        return err(prefRes.error)
      tsnetBuilderCfg.quicConfig.setRuntimePreference(prefRes.get())
      debugLog(
  ...
  when not defined(libp2p_no_tls):
    ...
    if enableTls:
      debugLog("[nimlibp2p] buildSwitch TLS requested but libp2p_no_tls is enabled; skipping TLS")
    else:
      debugLog("[nimlibp2p] buildSwitch TLS disabled")
  if enableNoise:
  ...
  let nodeResourceConfig = NodeResourceServiceConfig.init(
    syncInterval = chronos.seconds(10),
    localRefreshInterval = chronos.seconds(10),
    syncBatchSize = 4,
  )
  builder = builder.withNodeResourceService(
    dataDir = if cfg.dataDir.isSome(): cfg.dataDir.get() else: "",
    config = nodeResourceConfig,
  )
  debugLog(
    "[nimlibp2p] buildSwitch applied node resource service syncInterval=10s batch=4"
  )
...
proc finalizeCommand(cmd: Command) =
  cmd.completed.store(true, moRelease)
...
proc buildConnectedPeersInfoSnapshotForNode(
    node: NimNode
): JsonNode {.gcsafe, raises: [].}
...
proc buildDiagnosticsSnapshotPayload(n: NimNode): JsonNode {.gcsafe, raises: [].} =
  let peerInfo =
    if n.isNil or n.switchInstance.isNil:
      nil
    else:
      n.switchInstance.peerInfo
  if peerInfo.isNil:
    return newJObject()
  ...
  if gossipAvailable(n):
    for topic in n.gossip.PubSub.topics.keys:
      topics.add(topic)
  let listenAddrs = peerInfo.listenAddrs.mapIt($it)
  let dialableAddrs = peerInfo.addrs.mapIt($it)
  let hostNetwork = currentHostNetworkStatus(n)
  ...
  try:
    ...
    of cmdStart:
      if n.started:
        ...
        updateStartupStatus(n, "switch_start", true)
        ...
        try:
          ...
          updateStartupStatus(n, "relay_loop", true)
          startRelayReservationLoop(n)
          updateStartupStatus(n, "mdns_schedule", true)
          scheduleStartMdns(n)
          ...
          if errMsg.contains("DiscoveryInterface.advertise"):
            ...
            updateStartupStatus(n, "failed", false, errMsg)
            debugLog("[nimlibp2p] cmdStart: marked failure err=" & errMsg)
    of cmdStop:
      if not n.started:
        info "cmdStop: node already stopped"
        updateStartupStatus(n, "stopped", false)
        updateTailnetStartStatus(n, "idle", false)
        command.resultCode = NimResultOk
      else:
        updateStartupStatus(n, "stopping", false)
        ...
        if n.connectionEventsEnabled:
          ...
          for entry in n.peerHandlers:
            let kind = entry[0]
            let handler = entry[1]
            n.switchInstance.connManager.removePeerEventHandler(handler, kind)
          n.connectionHandlers.setLen(0)
          n.peerHandlers.setLen(0)
          n.connectionEventsEnabled = false
        n.peerStateLock.acquire()
        try:
          n.peerIdentifiedAt.clear()
        finally:
          n.peerStateLock.release()
        ...
        if not n.directSessionTargetReconcileLoop.isNil:
          n.directSessionTargetReconcileLoop.cancel()
          n.directSessionTargetReconcileLoop = nil
        for fut in n.bootstrapSeedConnectFutures.values:
          if not fut.isNil and not fut.finished():
            fut.cancelSoon()
        n.bootstrapSeedConnectFutures.clear()
        for fut in n.sendDirectKickoffFutures.values:
          if not fut.isNil and not fut.finished():
            fut.cancelSoon()
        n.sendDirectKickoffFutures.clear()
    ...
    of cmdConnectMultiaddr:
      if not n.started:
        ...
        let tsnetDial = lowerMultiaddr.contains("/tsnet")
    ...
    of cmdDiagnostics:
      ...
      let diagnostics = buildDiagnosticsSnapshotPayload(n)
      if diagnostics.kind != JObject or diagnostics.len == 0:
    ...
    of cmdTailnetWarmFinalize:
      ...
      if n.tailnetStage == "failed":
        command.resultCode = NimResultError
        command.errorMsg = if n.tailnetError.len > 0: n.tailnetError else: "tailnet warm failed"
      else:
        command.resultCode = NimResultOk
    of cmdTailnetStatus:
    ...
    of cmdConnectedPeersInfo:
      command.stringResult = $buildConnectedPeersInfoSnapshotForNode(n)
      command.resultCode = NimResultOk
    of cmdRegisterPeerHints:
      let registerRes = registerPeerHintsInternal(n, command.peerId, command.addresses, command.source)
      if registerRes.isErr():
        command.resultCode = NimResultInvalidArgument
        command.errorMsg = registerRes.error
      else:
        command.resultCode = NimResultOk
    ...
    of cmdBootstrapSeedConnectExactKickoff:
      if not n.started:
        ...
        let safePeerId = command.peerId.strip()
        let safeAddresses = prioritizeGameSeedAddrs(preferStableAddrStrings(command.addresses))
        let safeLimit =
          if command.requestLimit > 0:
            min(max(command.requestLimit, 1), safeAddresses.len)
          else:
            safeAddresses.len
        if safeLimit <= 0:
          command.resultCode = NimResultInvalidArgument
          ...
          let selectedAddrs = safeAddresses[0 ..< safeLimit]
          let safeSource =
            if command.source.strip().len > 0:
              command.source.strip()
            else:
              "seeded_exact_connect"
          let safeSecureTimeoutMs =
            if command.timeoutMs > 0:
              max(min(command.timeoutMs, 10_000), 250)
            else:
              5_000
          ffiNodeSafe:
            setBootstrapSeedConnectKickoffState(
              ...
              %*{
                ...
                "status": "running",
                "source": safeSource,
                "addressCount": selectedAddrs.len,
              }
            )
          ffiNodeSafe:
            discard getOrStartBootstrapSeedConnectTask(
              n,
              safePeerId,
              selectedAddrs,
              safeSource,
    ...
    of cmdDirectSessionTargetSet:
      if not n.started:
        ...
        let safePeerId = command.peerId.strip()
        let selectedAddrs = prioritizeGameSeedAddrs(preferStableAddrStrings(command.addresses))
        let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
        if taskKey.len == 0:
          command.resultCode = NimResultInvalidArgument
          command.errorMsg = "no valid addresses"
        else:
          let spec = upsertDirectSessionTarget(
            n,
            safePeerId,
            selectedAddrs,
            command.source,
          )
          if spec.taskKey.len == 0:
            command.resultCode = NimResultInvalidArgument
            command.errorMsg = "no valid addresses"
          else:
            command.resultCode = NimResultOk
    of cmdDirectSessionTargetClear:
      if command.peerId.len == 0 or command.addresses.len == 0:
        ...
        if taskKey.len == 0:
          ...
          let spec =
            clearDirectSessionTargetDesiredState(
              n,
              safePeerId,
              selectedAddrs,
              command.source,
            )
          let existingTask = n.bootstrapSeedConnectTasks.getOrDefault(taskKey, nil)
          let taskHasLiveSession =
            not existingTask.isNil and
              existingTask.sessionConnKey > 0 and
              not n.dmService.isNil and
              n.dmService.hasLiveSessionConnKey(existingTask.sessionConnKey)
          let shouldCancelBackgroundWarm =
            not existingTask.isNil and
              existingTask.intent == bsciBackgroundWarm and
              not taskHasLiveSession
          if shouldCancelBackgroundWarm:
            existingTask.state = bscsCancelled
            existingTask.updatedAtMs = nowMillis()
            existingTask.ok = false
            existingTask.err = "direct_target_cleared"
            existingTask.sessionWarmOk = false
            existingTask.sessionWarmError = "direct_target_cleared"
            existingTask.sessionConnKey = 0
            existingTask.verifiedMuxer = nil
            let existingFuture = n.bootstrapSeedConnectFutures.getOrDefault(taskKey, nil)
            if not existingFuture.isNil and not existingFuture.finished():
              existingFuture.cancelSoon()
            if taskKey in n.bootstrapSeedConnectFutures:
              n.bootstrapSeedConnectFutures.del(taskKey)
            n.bootstrapSeedConnectTasks.del(taskKey)
          publishDirectSessionTargetSnapshot(
            n,
            spec,
            "cleared",
            "direct_target_cleared"
          )
          command.resultCode = NimResultOk
    of cmdUdpEchoProbe:
      if not n.started:
    ...
    of cmdSendDirectKickoff:
      if not n.started:
        ...
        compactFinishedSendDirectKickoffTasks(n, requestKey)
        let existingTask = n.sendDirectKickoffTasks.getOrDefault(requestKey, nil)
        if existingTask.isNil or not sendDirectKickoffTaskRunning(n, existingTask):
          let taskState = SendDirectKickoffTask(
            requestKey: requestKey,
            peerId: command.peerId.strip(),
            source: command.source.strip(),
            forceRelayConnectivity: command.boolParam,
            updatedAtMs: nowMillis(),
            completedAtMs: 0,
            running: true,
          )
          ...
          n.sendDirectKickoffTasks[requestKey] = taskState
          n.sendDirectKickoffFutures[requestKey] = task
          asyncSpawn task
        elif not existingTask.isNil:
          existingTask.updatedAtMs = nowMillis()
        command.resultCode = NimResultOk
    of cmdSendChatAck:
      if not n.started:
        ...
        let identifyWaitMs =
          if command.timeoutMs > 0:
            max(min(command.timeoutMs, 8_000), 1_000)
          else:
            3_000
        let identifiedOk = await ensurePeerIdentified(n, remotePid, identifyWaitMs)
        if not identifiedOk:
          let errMsg = "peer identify not ready"
          command.resultCode = NimResultError
          command.errorMsg = errMsg
          n.lastDirectError = errMsg
          warn "cmdSendChatAck identify failed", peer = command.peerId, mid = command.messageId
          return
        var envelope = newJObject()
        envelope["op"] = %"ack"
    ...
    of cmdUpdateHostNetworkStatus:
      if command.jsonString.len == 0:
        ...
        try:
          ...
          let normalized = applyHostNetworkStatus(n, payload)
          let normalizedSnapshot = n.hostNetworkStatus
          debugLog(
          ...
          let event = buildHostNetworkEvent(normalizedSnapshot)
          recordEvent(n, "network_event", $event)
    ...
    of cmdSubmitNearFieldObservation:
      if command.jsonString.len == 0:
        command.resultCode = NimResultInvalidArgument
        command.errorMsg = "missing near field observation payload"
      elif n.switchInstance.isNil:
        command.resultCode = NimResultInvalidState
        command.errorMsg = "switch not initialized"
      elif not configuredNearFieldBootstrapEnabled(n.cfg):
        command.resultCode = NimResultInvalidState
        command.errorMsg = "near field bootstrap disabled"
      else:
        let parsed = parseNearFieldObservation(command.jsonString)
        if parsed.isErr():
          command.resultCode = NimResultInvalidArgument
          command.errorMsg = parsed.error
        else:
          let observation = parsed.get()
          if observation.networkId.len > 0 and
              observation.networkId != configuredLsmrNetworkId(n.cfg):
            command.resultCode = NimResultInvalidArgument
            command.errorMsg = "near field networkId mismatch"
          else:
            if not n.switchInstance.isNil:
              discard n.switchInstance.registerBootstrapHint(
                observation.peerId,
                observation.addrs,
                source = "nearfield:" & observation.provider.label(),
                trust = WanBootstrapTrust.quarantine,
              )
            if observeNearFieldBootstrap(n.switchInstance, observation):
              discard await refreshCoordinateRecord(n.switchInstance)
              command.resultCode = NimResultOk
              command.boolResult = true
            else:
              command.resultCode = NimResultError
              command.errorMsg = "failed to install near field observation"
    ...
    of cmdNetworkDiscoverySnapshot:
      ...
      let connectedInfo = buildConnectedPeersInfoSnapshotForNode(n)
      ...
      if n.feedService.isNil:
        ...
        if entryOpt.isNone:
          ...
          try:
            ...
            command.resultCode = NimResultOk
            if delivered == 0:
              # Publishing to the local feed topic is still a valid success when
              # there are no remote subscribers yet. Higher layers can inspect
              # `intResult == 0` if they need to distinguish local-only publish.
...
proc drainCommandQueue(n: NimNode) {.async: (raises: []), gcsafe.} =
  try:
    while true:
      if n.commandDrainWake.isNil:
        break
      try:
        await n.commandDrainWake.wait()
      except CancelledError:
        break
      n.commandDrainWake.clear()
      var command = dequeueCommand(n)
      if command.isNil:
        if n.stopRequested.load(moAcquire):
          break
        continue
      while true:
        if command.kind != cmdNone:
          ...
          n.retiredCommandTails.add(command)
        let nextCommand = dequeueCommand(n)
        if nextCommand.isNil:
          break
        command = nextCommand
  finally:
...
proc processCommandQueue(n: NimNode) {.raises: [], gcsafe.} =
  if n.isNil or not n.commandDrainActive or n.commandDrainWake.isNil:
    return
  if n.queuedCommands.load(moAcquire) == 0 and not n.stopRequested.load(moAcquire):
    return
  n.commandDrainWake.fire()
  ...
  while not n.stopRequested.load(moAcquire):
    try:
      ...
      if nowMs - lastPublicIpProbeKickMs >= PublicIpProbeLoopKickMs:
        ...
        let hostStatus = n.hostNetworkStatus
        if hostStatus.isConnected and shouldRefreshPublicIpProbe(n, hostStatus):
      ...
      debugLog(
        "[nimlibp2p] idleWakeLoop fatal node=" & $cast[int](n) &
        " err=" & exc.msg &
...
proc eventLoop(handlePtr: pointer) {.thread.} =
  ...
  node.commandDrainActive = true
  let drainTask = drainCommandQueue(node)
  node.commandDrainTask = drainTask
  asyncSpawn(drainTask)
  node.idleWakeTask = idleWakeLoop(node)
  asyncSpawn(node.idleWakeTask)
  ensureDirectSessionTargetReconcileLoop(node)
  ensurePeerRouteSessionReconcileLoop(node)
  ...
  while true:
    ...
    let queued = node.queuedCommands.load(moAcquire)
    let pending = node.pendingCommands.load(moAcquire)
    let drainStopped = node.commandDrainTask.isNil or node.commandDrainTask.finished()
    let signalStopped = node.signalLoopTask.isNil or node.signalLoopTask.finished()
    let idleStopped = node.idleWakeTask.isNil or node.idleWakeTask.finished()
    if shouldStop and queued == 0 and pending == 0 and drainStopped and signalStopped and idleStopped:
      break
    try:
      if debugEventLoop:
    ...
    processTailnetWarmCompletion(node)
...
proc libp2p_node_init*(configJson: cstring): pointer {.exportc, cdecl, dynlib.} =
  ...
  try:
    ...
    var node = NimNode(
      ...
      commandDrainWake: newAsyncEvent(),
      commandDrainActive: false,
      idleWakeTask: nil,
      ...
      hostNetworkStatus: defaultHostNetworkStatusSnapshot(),
      ...
      deferredHostNetworkStatus: defaultHostNetworkStatusSnapshot(),
      ...
      startupError: "",
      startupQueuedAtMs: 0,
      startupUpdatedAtMs: 0,
      startupLastDurationMs: 0,
      startupAttempts: 0,
      tailnetInFlight: false,
      tailnetStage: "idle",
      tailnetError: "",
      tailnetQueuedAtMs: 0,
      tailnetUpdatedAtMs: 0,
      tailnetLastDurationMs: 0,
      tailnetAttempts: 0,
      bootstrapSeedConnectTasks: initTable[string, BootstrapSeedConnectTask](),
      bootstrapSeedConnectFutures: initTable[string, Future[void]](),
      directSessionTargets: initTable[string, DirectSessionTargetSpec](),
      peerRouteSessionTasks: initTable[string, PeerRouteSessionTask](),
      peerRouteSessionFutures: initTable[string, Future[void]](),
      sendDirectKickoffTasks: initTable[string, SendDirectKickoffTask](),
      sendDirectKickoffFutures: initTable[string, Future[void]](),
    )
    ...
    initLock(node.startupStatusLock)
    initLock(node.tailnetStatusLock)
    initLock(node.directSessionTargetLock)
    initLock(node.peerStateLock)
...
proc libp2p_node_start_kickoff*(handle: pointer): bool {.exportc, cdecl, dynlib.} =
  let node = nodeFromHandle(handle)
  if node.isNil:
    return false
  if node.started:
    updateStartupStatus(node, "started", false)
    rememberLastNodeIdentity(handle, node)
    return true
  node.startupStatusLock.acquire()
  let alreadyQueued = node.startupInFlight
  node.startupStatusLock.release()
  if alreadyQueued:
    return true
  let cmd = makeCommand(cmdStart)
  updateStartupStatus(node, "queued", true)
  let ok = submitCommandNoWait(node, cmd)
  if ok:
    return true
  let err =
    if cmd.errorMsg.len > 0: cmd.errorMsg
    else: "node_start_kickoff_failed"
  updateStartupStatus(node, "failed", false, err)
  destroyCommand(cmd)
  false

proc libp2p_get_start_status_json*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  let node = nodeFromHandle(handle)
  if node.isNil:
    return allocCString("{\"started\":false,\"inFlight\":false,\"stage\":\"invalid_handle\",\"error\":\"node_not_initialized\"}")
  allocCString($(buildStartupStatusPayload(node)))

proc libp2p_tailnet_start_kickoff*(handle: pointer): bool {.exportc, cdecl, dynlib.} =
  let node = nodeFromHandle(handle)
  if node.isNil:
    return false
  if not node.started:
    updateTailnetStartStatus(node, "failed", false, "node_not_started")
    return false
  let transport = activeTsnetTransport(node)
  if transport.isNil:
    updateTailnetStartStatus(node, "failed", false, "tsnet_transport_unavailable")
    return false
  let current = buildTailnetStatusPayload(node)
  if jsonGetBool(current, "providerReady", false):
    updateTailnetStartStatus(node, "started", false)
    return true
  node.tailnetStatusLock.acquire()
  let alreadyQueued = node.tailnetInFlight
  node.tailnetStatusLock.release()
  if alreadyQueued:
    return true
  let cmd = makeCommand(cmdTailnetWarm)
  updateTailnetStartStatus(node, "queued", true)
  let ok = submitCommandNoWait(node, cmd)
  if ok:
    return true
  let err =
    if cmd.errorMsg.len > 0: cmd.errorMsg
    else: "tailnet_start_kickoff_failed"
  updateTailnetStartStatus(node, "failed", false, err)
  destroyCommand(cmd)
  false

proc libp2p_get_tailnet_start_status_json*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  let node = nodeFromHandle(handle)
  if node.isNil:
    return allocCString("{\"started\":false,\"inFlight\":false,\"stage\":\"invalid_handle\",\"error\":\"node_not_initialized\"}")
  allocCString($(buildTailnetStartStatusPayload(node)))
...
proc libp2p_send_with_ack_kickoff*(
    ...
    jsonPayload: cstring,
    timeoutMs: cint,
    preferredSessionConnKey: clonglong
): bool {.exportc, cdecl, dynlib.} =
  let node = nodeFromHandle(handle)
  if node.isNil:
    return false
  ...
  cmd.jsonString = $jsonPayload
  cmd.timeoutMs = int(timeoutMs)
  cmd.deadlineAtMs = nowMillis() + int64(max(cmd.timeoutMs, 0))
  if preferredSessionConnKey > 0:
    cmd.sessionConnKey = int(preferredSessionConnKey)
  let derivedOp = parseDirectEnvelopeOp(cmd.jsonString)
  if derivedOp.len > 0:
    cmd.op = derivedOp
...
proc libp2p_submit_near_field_observation*(
  ...
  if node.isNil:
    return false
  let cmd = makeCommand(cmdSubmitNearFieldObservation)
  cmd.jsonString = $payloadJson
  let result = submitCommand(node, cmd)
  let ok = result.resultCode == NimResultOk
  destroyCommand(cmd)
...
proc libp2p_get_connected_peers_json*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  ...
  if node.isNil:
    return allocCString("[]")
  allocCString($buildConnectedPeersArrayForNode(node))
...
proc libp2p_connected_peers_info*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  ...
  if node.isNil:
    return allocCString("[]")
  let payload = buildConnectedPeersInfoSnapshotForNode(node)
  allocCString(
...
proc libp2p_get_diagnostics_json*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  ...
  var payloadText =
    block:
      let payload = buildDiagnosticsSnapshotPayload(node)
      if payload.kind == JObject and payload.len > 0:
        $payload
      else:
        "{}"
  debugLog("[nimlibp2p] libp2p_get_diagnostics_json snapshot ready len=" & $payloadText.len)
  try:
...
proc loadSocialStateUnsafe(node: NimNode): JsonNode =
  if not node.isNil and not node.socialStateParsedSnapshot.isNil and
      node.socialStateParsedSnapshot.kind == JObject:
    return node.socialStateParsedSnapshot
  var snapshotText = ""
  if not node.isNil and node.socialStateSnapshot.len > 0:
    snapshotText = node.socialStateSnapshot
  var state = defaultSocialState()
  if snapshotText.len == 0:
    let path = socialStatePath(node)
    if path.len > 0:
      try:
        if fileExists(path):
          snapshotText = readFile(path)
      except CatchableError:
        discard
  if snapshotText.len > 0:
    try:
      let parsed = parseJson(snapshotText)
  ...
  if not node.isNil:
    node.socialStateSnapshot = $state
    node.socialStateParsedSnapshot = state
  state

proc persistSocialStateUnsafe(node: NimNode, state: JsonNode) =
  if node.isNil:
    return
  var normalized = ensureSocialSchema(
    if state.isNil or state.kind != JObject: defaultSocialState()
    else: state
  )
  let snapshotText = $normalized
  node.socialStateSnapshot = snapshotText
  node.socialStateParsedSnapshot = normalized
  let path = socialStatePath(node)
  if path.len == 0:
    return
  try:
    writeFile(path, snapshotText)
  except CatchableError as exc:
...
proc buildConnectedPeersInfoSnapshotForNode(
  ...
  let recent = recentConnectedPeersInfo(node)
  if recent.kind == JArray and recent.len > 0:
    return cloneJson(recent)
  if node.isNil or node.switchInstance.isNil or node.switchInstance.connManager.isNil:
    return newJArray()
  ...
  for peerId, muxers in connections.pairs:
    ...
    for key in [
      ...
      if connSnapshot.hasKey(key):
        entry[key] = cloneJson(connSnapshot.getOrDefault(key))
    let pathProjection = buildPeerPathProjection(node, peerIdText, peerId)
    for key in [
      "transport",
      "pathStatus",
      "directTargetStatus",
      "directTargetReady",
      "directTargetTaskRunning",
      "directTargetSessionConnKey",
      "directTargetAttemptCount",
      "directTargetSuccessCount",
      "directTargetFailureCount",
      "directTargetSuccessRatePct",
      "directTargetLastAttemptAtMs",
      "directTargetLastReadyAtMs",
      "directTargetLastFailureAtMs",
      "directTargetLastFailureReason",
      "directTargetError",
    ]:
      if pathProjection.hasKey(key):
        entry[key] = cloneJson(pathProjection.getOrDefault(key))
    ...
    let transportLatency =
      if freshTransportLatency >= 0:
        freshTransportLatency
      else:
        cachedTransportLatencyMs(node, peerIdText)
...
proc collectDiscoveredPeersForNode(node: NimNode, connectedInfo: JsonNode): JsonNode {.gcsafe.} =
  ...
  for _, row in peers:
    let peerId = jsonGetStr(row, "peerId", jsonGetStr(row, "peer_id"))
    if peerId.len > 0:
      let directSnapshot = latestOwnedDirectSessionTargetSnapshot(node, peerId)
      if directTargetProbing(directSnapshot):
        row["pathStatus"] = %"PROBING"
        row["directTargetStatus"] = %directSnapshot.status
        row["directTargetTaskRunning"] = %true
        if directSnapshot.error.len > 0:
          row["directTargetError"] = %directSnapshot.error
    arr.add(row)
  arr
...
proc nodeResourceSnapshotToJson(snapshot: NodeResourceSnapshot): JsonNode {.gcsafe, raises: [].} =
  ...
  %*{
    ...
    "resource": %*{
      "os": %*{
        "name": resource.os.name,
        "version": resource.os.version,
        "kernel": resource.os.kernel,
        "arch": resource.os.arch,
      },
      "cpu": %*{
        "model": resource.cpu.model,
        "coresLogical": resource.cpu.coresLogical,
        "frequencyMHz": resource.cpu.frequencyMHz,
      },
      "memory": %*{
        "totalBytes": resource.memory.totalBytes,
        "availableBytes": resource.memory.availableBytes,
      },
      "gpu": %*{
        "model": resource.gpu.model,
        "vendor": resource.gpu.vendor,
        "vramTotalBytes": resource.gpu.vramTotalBytes,
        "vramAvailableBytes": resource.gpu.vramAvailableBytes,
        "computeClass": resource.gpu.computeClass,
        "computeScore": resource.gpu.computeScore,
        "availableKnown": resource.gpu.availableKnown,
      },
      "disk": %*{
        "mountPath": resource.disk.mountPath,
        "type": resource.disk.`type`,
        "totalBytes": resource.disk.totalBytes,
        "availableBytes": resource.disk.availableBytes,
      },
      "bandwidth": %*{
        "uplinkBps": resource.bandwidth.uplinkBps,
        "downlinkBps": resource.bandwidth.downlinkBps,
      },
...
proc buildNetworkResourcesPayload(node: NimNode, discoveredCount: int, connectedCount: int): JsonNode {.gcsafe.} =
  ...
  var memoryAvailable = jsonGetInt64(localProfile["memory"], "availableBytes", 0)
  ...
  var gpuVramAvailable = jsonGetInt64(localProfile["gpu"], "vramAvailableBytes", 0)
  var peersWithGpu = if gpuVramTotal > 0 or gpuVramAvailable > 0: 1 else: 0
  ...
  for peerId, profile in node.peerSystemProfiles.pairs:
    ...
    memoryAvailable += jsonGetInt64(profile["memory"], "availableBytes", 0)
    ...
    gpuVramAvailable += jsonGetInt64(profile["gpu"], "vramAvailableBytes", 0)
    if jsonGetInt64(profile["gpu"], "vramBytes", 0) > 0 or
        jsonGetInt64(profile["gpu"], "vramAvailableBytes", 0) > 0 or
        jsonGetStr(profile["gpu"], "model", "").len > 0:
      inc peersWithGpu
  ...
  if not node.isNil and not node.switchInstance.isNil:
    let resourceSvc = getNodeResourceService(node.switchInstance)
    if not resourceSvc.isNil:
      let totals = resourceSvc.nodeResourceTotals()
      let resourceSnapshots = resourceSvc.nodeResourceSnapshots()
      cpuTotal = totals.cpuCoresLogical
      memoryTotal = totals.memoryTotalBytes
      memoryAvailable = totals.memoryAvailableBytes
      diskTotal = totals.diskTotalBytes
      diskAvailable = totals.diskAvailableBytes
      gpuVramTotal = totals.gpuVramTotalBytes
      gpuVramAvailable = totals.gpuVramAvailableBytes
      peersWithGpu = totals.peersWithGpu
      uplink = float64(totals.bandwidthUplinkBps)
      downlink = float64(totals.bandwidthDownlinkBps)
      snapshots = newJArray()
      for snapshot in resourceSnapshots:
        snapshots.add(nodeResourceSnapshotToJson(snapshot))
  %* {
    "nodeCount": max(max(1, discoveredCount + 1), snapshots.len),
    "onlineCount": max(max(1, connectedCount + discoveredCount), snapshots.len),
    "connectedPeers": connectedCount,
    "peersWithGpu": peersWithGpu,
    ...
    "gpuVramAvailableBytes": gpuVramAvailable,
...
proc libp2p_refresh_node_resources*(handle: pointer, limit: cint): cstring {.exportc, cdecl, dynlib.} =
  ...
  let connected = buildConnectedPeersInfoSnapshotForNode(node)
  let payload = buildNetworkResourcesPayload(node, discovered.totalCount, connected.len)
  allocCString($payload)
...
proc libp2p_network_discovery_snapshot*(handle: pointer, sourceFilter: cstring, limit: cint, connectCap: cint): cstring {.exportc, cdecl, dynlib.} =
  ...
  discard connectCap
  let connectedInfo = buildConnectedPeersInfoSnapshotForNode(node)
  let payload = buildNetworkDiscoverySnapshotPayload(
    node,
    connectedInfo,
    if sourceFilter != nil: $sourceFilter else: "",
    if limit <= 0: 0 else: int(limit),
  )
  allocCString($payload)
...
proc social_dm_send*(
  ...
  let ok = libp2p_send_with_ack(handle, peer.cstring, ($payload).cstring, 8_000)
...
proc ingestPendingGameMessages(node: NimNode) =
  ...
  var retainedRows: seq[ReceivedDmSnapshot] = @[]
  for row in rows:
    let peerId = row.peerId
    let conversationId = row.conversationId
    let messageId = row.messageId
    let payloadNode = parseReceivedDmPayloadText(row.payloadText)
...
proc social_feed_snapshot*(
  ...
  try:
    ...
    let connectedPeers = connectedPeersArray(node.selfHandle)
    let hasConnectedPeers = connectedPeers.kind == JArray and connectedPeers.len > 0
...
proc social_publish_enqueue*(handle: pointer, postJson: cstring): cstring {.exportc, cdecl, dynlib.} =
  try:
    ...
    var localPublishedPayload = newJNull()
    socialStateMemLock.acquire()
    try:
      let state = loadSocialStateUnsafe(node)
      socialApplyStep0NoLock(task)
      prependBoundedNode(state["publishTasks"], task, SocialMaxPublishTasks)
      if jsonGetStr(task, "moderationDecision", "NONE") == "PASS_TO_POOL" and
          not socialTaskHasMediaAsset(task):
        localPublishedPayload = socialTaskToMomentPayload(node, task)
        socialMarkTaskPublishedNoLock(state, task, localPublishedPayload, nowTs)
      else:
      ...
      socialStateMemLock.release()
    if localPublishedPayload.kind == JObject:
      discard socialSubmitSharedContentBroadcast(node, localPublishedPayload)
      discard socialFanoutDirectContentEnvelope(node, localPublishedPayload)
...
proc libp2p_get_bootstrap_status*(handle: pointer): cstring {.exportc, cdecl, dynlib.} =
  ...
  let routingNode = buildRoutingStatusPayload(node)
  try:
    if node.switchInstance != nil:
      ...
      bootstrap["routingStatus"] = routingNode
...
proc waitForPeerConnectedWithinDeadline(
  ...
  let safeTimeoutMs = max(timeoutMs, 0)
  if safeTimeoutMs == 0:
    return peerSecureChannelReady(node, peer)
  let deadlineMs = nowMillis() + int64(safeTimeoutMs)
  let safePollMs = max(pollMs, 25)
  while true:
    if peerSecureChannelReady(node, peer):
      return true
    let remainingMs = int(deadlineMs - nowMillis())
    if remainingMs <= 0:
      return false
...
proc waitForPeerConnectedWithinDeadlineAsync(
    node: NimNode,
    peer: PeerId,
    timeoutMs: int,
    pollMs = 100
...
proc registerPeerHintsInternal(
  ...
  let safePeerId = peerIdText.strip()
  if safePeerId.len == 0 or addresses.len == 0:
    return err("missing peerId or addresses")
  let peer = PeerId.init(safePeerId).valueOr:
  ...
  for addrStr in addresses:
    ...
    if normalizedText.len == 0:
      continue
    let ma = parseDialableMultiaddr(normalizedText).valueOr:
      warn "registerPeerHints: invalid addr", peerId = safePeerId, address = normalizedText, err = error
      continue
    if normalizedText notin normalizedAddrs:
      normalizedAddrs.add(normalizedText)
    parsed.add(ma)
  if parsed.len == 0:
    return err("no valid addresses")
  let hintSource =
    if source.strip().len > 0:
      source.strip()
    else:
      "manual"
  n.peerHints[safePeerId] = normalizedAddrs
  addPeerHintSource(n, safePeerId, hintSource)
  n.bootstrapLastSeen[safePeerId] = nowMillis()
  let peerStore = n.switchInstance.peerStore
  if not peerStore.isNil:
    try:
      ...
      warn "registerPeerHints: addAddressesWithTTL failed", peerId = safePeerId, err = exc.msg
  if not n.switchInstance.isNil:
    discard n.switchInstance.registerBootstrapHint(
      peer, parsed, source = hintSource, trust = WanBootstrapTrust.quarantine
    )
...
proc bootstrapSeedConnectExactPayload(
  ...
  let bootstrap =
    try:
      buildBootstrapStatusNode(node)
    except Exception:
      newJObject()
  let bootstrapConnected =
    try:
      bootstrapStatusShowsConnectedPeer(bootstrap, seedPeerId)
    except Exception:
      false
  var payload = newJObject()
  payload["ok"] = %ok
...
proc bootstrapSeedConnectIntentLabel(
    intent: BootstrapSeedConnectIntent
): string {.gcsafe, raises: [].} =
  case intent
  of bsciBackgroundWarm:
    "background_warm"
  of bsciForegroundSend:
    "foreground_send"

proc bootstrapSeedConnectStateLabel(
    state: BootstrapSeedConnectState
): string {.gcsafe, raises: [].} =
  case state
  of bscsPending:
...
proc cancelBootstrapSeedConnectTask(
    n: NimNode,
    taskKey: string,
    reason: string
) {.gcsafe, raises: [].} =
  ...
  if task.isNil:
    return
  task.state = bscsCancelled
  task.updatedAtMs = nowMillis()
  task.ok = false
  task.err = reason
  task.sessionWarmOk = false
  task.sessionWarmError = reason
  task.sessionConnKey = 0
  task.verifiedMuxer = nil
  let fut = n.bootstrapSeedConnectFutures.getOrDefault(taskKey, nil)
  if not fut.isNil and not fut.finished():
    fut.cancelSoon()
  if taskKey in n.bootstrapSeedConnectFutures:
    n.bootstrapSeedConnectFutures.del(taskKey)
...
proc bootstrapSeedConnectTaskRunning(
  ...
  if n.isNil or task.isNil or task.taskKey.len == 0:
    return false
  let fut = n.bootstrapSeedConnectFutures.getOrDefault(task.taskKey, nil)
  task.state == bscsRunning and
    not fut.isNil and
    not fut.finished()
...
proc sendDirectKickoffTaskRunning(
  ...
  if n.isNil or task.isNil or task.requestKey.len == 0:
    return false
  let fut = n.sendDirectKickoffFutures.getOrDefault(task.requestKey, nil)
  task.running and
    not fut.isNil and
    not fut.finished()
...
proc compactFinishedSendDirectKickoffTasks(
  ...
  if n.isNil or n.sendDirectKickoffTasks.len == 0:
    return
  var staleKeys: seq[string] = @[]
  for requestKey, task in n.sendDirectKickoffTasks.pairs:
    if keepRequestKey.len > 0 and requestKey == keepRequestKey:
      continue
    if task.isNil or not sendDirectKickoffTaskRunning(n, task):
      staleKeys.add(requestKey)
  for requestKey in staleKeys:
    if requestKey in n.sendDirectKickoffFutures:
      n.sendDirectKickoffFutures.del(requestKey)
    n.sendDirectKickoffTasks.del(requestKey)

proc bootstrapSeedConnectPublishesKickoffState(
    intent: BootstrapSeedConnectIntent
): bool {.gcsafe, raises: [].} =
  intent == bsciBackgroundWarm
...
proc bootstrapSeedConnectTaskSessionLive(
  ...
  not n.isNil and
    not task.isNil and
    task.state == bscsReady and
    task.sessionWarmOk and
    task.sessionConnKey > 0 and
    not n.dmService.isNil and
    n.dmService.hasLiveSessionConnKey(task.sessionConnKey)

proc peerRouteSessionTaskRunning(
    n: NimNode,
    task: PeerRouteSessionTask
): bool {.gcsafe, raises: [].} =
  if n.isNil or task.isNil or task.peerId.len == 0:
    return false
  let fut = n.peerRouteSessionFutures.getOrDefault(task.peerId, nil)
  task.state == prssRunning and
    not fut.isNil and
    not fut.finished()

proc livePeerRouteSessionConnKey(
    n: NimNode,
    peerIdText: string
): int {.gcsafe, raises: [].} =
  if n.isNil or n.dmService.isNil:
  ...
  if peerRes.isErr():
    return 0
  for snapshot in n.dmService.peerSessionControlSnapshots(peerRes.get()):
    if snapshot.lifecycle != DmSessionLive:
      continue
    if snapshot.connKey <= 0:
      continue
    if n.dmService.hasLiveSessionConnKey(snapshot.connKey):
      return snapshot.connKey
  0
...
proc latestPeerRouteSessionSnapshot(
  ...
  let safePeerId = peerIdText.strip()
  if safePeerId.len == 0:
    return PeerRouteSessionSnapshot(
      peerId: "",
      status: "missing",
      error: "missing_peerId",
      ok: false,
  ...
  let task =
    if n.isNil:
      ...
      n.peerRouteSessionTasks.getOrDefault(safePeerId, nil)
  let liveConnKey = livePeerRouteSessionConnKey(n, safePeerId)
  let running = peerRouteSessionTaskRunning(n, task)
  let effectiveStatus =
    if liveConnKey > 0:
      "ready"
    elif running:
      ...
      "missing"
    elif task.state == prssFailed:
      "failed"
    elif task.state == prssCancelled:
      "cleared"
    else:
      "pending"
  let effectiveError =
    if task.isNil:
      ...
      task.err
  PeerRouteSessionSnapshot(
    peerId: safePeerId,
    status: effectiveStatus,
    error: effectiveError,
    ok: liveConnKey > 0,
    taskRunning: running,
    sessionWarmOk: liveConnKey > 0,
    sessionConnKey: liveConnKey,
  )

proc peerRouteSessionReady(
    snapshot: PeerRouteSessionSnapshot
): bool {.gcsafe, raises: [].} =
  snapshot.status.toLowerAscii() == "ready" and snapshot.sessionConnKey > 0

proc peerRouteSessionProbing(
    snapshot: PeerRouteSessionSnapshot
): bool {.gcsafe, raises: [].} =
  snapshot.taskRunning or snapshot.status.toLowerAscii() in ["pending", "running"]

proc runPeerRouteSessionWarmAsync(
    taskState: PeerRouteSessionTask,
    n: NimNode,
    peerIdText: string,
    source: string,
    timeoutMs: int
  ...
  let safeSource =
    if source.strip().len > 0:
      ...
      "route_session_target"
  let safeTimeoutMs = max(timeoutMs, 250)
  try:
    if n.isNil or n.dmService.isNil:
      raise (ref LPError)(msg: "direct message service unavailable")
    let peerRes = PeerId.init(safePeerId)
    if peerRes.isErr():
      raise (ref LPError)(msg: "invalid peer id")
    taskState.peerId = safePeerId
    taskState.source = safeSource
    taskState.state = prssRunning
    taskState.updatedAtMs = nowMillis()
    taskState.deadlineAtMs = taskState.updatedAtMs + int64(safeTimeoutMs)
    taskState.err = ""
    taskState.sessionConnKey = 0
    let warmRes = await n.dmService.warmSessionDetailed(
      peerRes.get(),
      chronos.milliseconds(safeTimeoutMs),
      relayOnly = true,
    ...
    taskState.updatedAtMs = nowMillis()
    if warmRes.ok and warmRes.connKey > 0 and n.dmService.hasLiveSessionConnKey(warmRes.connKey):
      taskState.state = prssReady
      taskState.err = ""
      taskState.sessionConnKey = warmRes.connKey
    else:
      taskState.state = prssFailed
      taskState.err =
        if warmRes.err.len > 0:
          warmRes.err
        else:
          "relay route session unavailable"
      taskState.sessionConnKey = 0
  except CancelledError:
    taskState.updatedAtMs = nowMillis()
    taskState.state = prssCancelled
    taskState.err = "route_session_target_cancelled"
    taskState.sessionConnKey = 0
  except CatchableError as exc:
    taskState.updatedAtMs = nowMillis()
    taskState.state = prssFailed
    taskState.err = if exc.msg.len > 0: exc.msg else: "route session warm failed"
    taskState.sessionConnKey = 0
  finally:
    let currentTask =
      if n.isNil:
        ...
        n.peerRouteSessionTasks.getOrDefault(safePeerId, nil)
    if currentTask == taskState and safePeerId in n.peerRouteSessionFutures:
      n.peerRouteSessionFutures.del(safePeerId)

proc reconcilePeerRouteSession(
    n: NimNode,
    peerIdText: string,
    source: string
) {.gcsafe, raises: [].} =
  ...
  let safeSource =
    if source.strip().len > 0:
      ...
      "route_session_target"
  let liveConnKey = livePeerRouteSessionConnKey(n, safePeerId)
  var task = n.peerRouteSessionTasks.getOrDefault(safePeerId, nil)
  if liveConnKey > 0:
    if task.isNil:
      task = PeerRouteSessionTask(
        peerId: safePeerId,
        ...
        state: prssReady,
        updatedAtMs: nowMillis(),
        deadlineAtMs: nowMillis(),
        err: "",
        sessionConnKey: liveConnKey,
      )
    else:
      task.peerId = safePeerId
      task.source = safeSource
      task.state = prssReady
      task.updatedAtMs = nowMillis()
      task.deadlineAtMs = task.updatedAtMs
      task.err = ""
      task.sessionConnKey = liveConnKey
    n.peerRouteSessionTasks[safePeerId] = task
    return
  if peerRouteSessionTaskRunning(n, task):
    if not task.isNil:
      task.source = safeSource
      task.updatedAtMs = nowMillis()
    return
  let taskState = PeerRouteSessionTask(
    peerId: safePeerId,
    source: safeSource,
    state: prssRunning,
    updatedAtMs: nowMillis(),
    deadlineAtMs: nowMillis() + 4_500,
    err: "",
    sessionConnKey: 0,
  )
  let fut = runPeerRouteSessionWarmAsync(
    taskState,
    n,
    safePeerId,
    safeSource,
  ...
  n.peerRouteSessionTasks[safePeerId] = taskState
  n.peerRouteSessionFutures[safePeerId] = fut
  asyncSpawn fut

proc ensurePeerRouteSessionReconcileLoop(n: NimNode) {.gcsafe, raises: [].} =
  if n.isNil:
    return
  if not n.peerRouteSessionReconcileLoop.isNil and not n.peerRouteSessionReconcileLoop.finished:
    return
  let loop = proc(): Future[void] {.closure, async: (raises: [CancelledError]).} =
    while n.running.load(moAcquire):
      if n.started:
        var desired = initTable[string, DirectSessionTargetSpec]()
        for spec in copyDirectSessionTargetSpecs(n):
          let peerId = spec.peerId.strip()
          if peerId.len == 0:
            continue
          let existing = desired.getOrDefault(peerId, DirectSessionTargetSpec())
          if existing.taskKey.len == 0 or spec.updatedAtMs >= existing.updatedAtMs:
            desired[peerId] = spec
        for peerId, spec in desired.pairs:
          try:
            reconcilePeerRouteSession(n, peerId, spec.source)
          except CatchableError as exc:
            debugLog(
              "[nimlibp2p] peerRouteSession reconcile failed peer=" & peerId &
              " err=" & exc.msg
            )
        var stalePeers: seq[string] = @[]
        for peerId in n.peerRouteSessionTasks.keys:
          if not desired.hasKey(peerId):
            stalePeers.add(peerId)
        for peerId in stalePeers:
          let fut = n.peerRouteSessionFutures.getOrDefault(peerId, nil)
          if not fut.isNil and not fut.finished():
            fut.cancelSoon()
          if peerId in n.peerRouteSessionFutures:
            n.peerRouteSessionFutures.del(peerId)
          n.peerRouteSessionTasks.del(peerId)
      await sleepAsync(chronos.milliseconds(750))
  let reconcileFuture = loop()
  n.peerRouteSessionReconcileLoop = reconcileFuture
  asyncSpawn(reconcileFuture)

proc waitForPeerRouteSessionReady(
    n: NimNode,
    peerIdText: string,
    timeoutMs: int,
    source: string
): Future[tuple[ok: bool, connKey: int, err: string, snapshot: PeerRouteSessionSnapshot]] {.async.} =
  let safePeerId = peerIdText.strip()
  let missingSnapshot = latestPeerRouteSessionSnapshot(n, safePeerId)
  if n.isNil or safePeerId.len == 0:
    return (false, 0, "missing_peer_id", missingSnapshot)
  reconcilePeerRouteSession(n, safePeerId, source)
  ensurePeerRouteSessionReconcileLoop(n)
  let deadlineAtMs = nowMillis() + int64(max(timeoutMs, 0))
  while true:
    let snapshot = latestPeerRouteSessionSnapshot(n, safePeerId)
    if peerRouteSessionReady(snapshot):
      return (true, snapshot.sessionConnKey, "", snapshot)
    if snapshot.status.toLowerAscii() in ["failed", "cleared"]:
      let errMsg =
        if snapshot.error.len > 0:
          snapshot.error
        else:
          "relay route session unavailable"
      return (false, 0, errMsg, snapshot)
    let remainingMs = int(deadlineAtMs - nowMillis())
    if remainingMs <= 0:
      let errMsg =
        if snapshot.error.len > 0:
          snapshot.error
        else:
          "relay route session not ready"
      return (false, 0, errMsg, snapshot)
    await sleepAsync(chronos.milliseconds(min(100, remainingMs)))

proc waitForDirectOrRelaySessionReady(
    n: NimNode,
    peerIdText: string,
    selectedAddrs: seq[string],
    timeoutMs: int,
    ...
    connKey: int,
    err: string,
    directSnapshot: DirectSessionTargetSnapshot,
    relaySnapshot: PeerRouteSessionSnapshot
]] {.async.} =
  let safePeerId = peerIdText.strip()
  var directSnapshot =
    if selectedAddrs.len > 0:
      ffiNodeSafe:
        liveDirectSessionTargetSnapshot(n, safePeerId, selectedAddrs)
    else:
      latestOwnedDirectSessionTargetSnapshot(n, safePeerId)
  var relaySnapshot = latestPeerRouteSessionSnapshot(n, safePeerId)
  if n.isNil or safePeerId.len == 0:
    return (false, false, 0, "missing_peer_id", directSnapshot, relaySnapshot)
  if selectedAddrs.len > 0:
    discard upsertDirectSessionTarget(
      n,
      safePeerId,
      selectedAddrs,
      source,
  ...
  reconcilePeerRouteSession(n, safePeerId, source)
  ensureDirectSessionTargetReconcileLoop(n)
  ensurePeerRouteSessionReconcileLoop(n)
  let deadlineAtMs = nowMillis() + int64(max(timeoutMs, 0))
  while true:
    directSnapshot =
      if selectedAddrs.len > 0:
        ...
        latestOwnedDirectSessionTargetSnapshot(n, safePeerId)
    relaySnapshot = latestPeerRouteSessionSnapshot(n, safePeerId)
    let directConnKey =
      if selectedAddrs.len > 0:
        ffiNodeSafe:
          getLiveReadyDirectSessionTargetConnKey(n, safePeerId, selectedAddrs)
      else:
        0
    if directConnKey > 0 and n.dmService.hasLiveSessionConnKey(directConnKey):
      return (true, false, directConnKey, "", directSnapshot, relaySnapshot)
    if peerRouteSessionReady(relaySnapshot) and
        relaySnapshot.sessionConnKey > 0 and
        n.dmService.hasLiveSessionConnKey(relaySnapshot.sessionConnKey):
      return (
        true,
        true,
        relaySnapshot.sessionConnKey,
        "",
        directSnapshot,
        relaySnapshot,
      )
    ...
    if remainingMs <= 0:
      let errMsg =
        if relaySnapshot.error.len > 0:
          relaySnapshot.error
        elif directSnapshot.error.len > 0:
          directSnapshot.error
        else:
          "direct and relay sessions not ready"
      return (false, false, 0, errMsg, directSnapshot, relaySnapshot)
    await sleepAsync(chronos.milliseconds(min(100, remainingMs)))

proc buildDirectSessionTargetPayload(
    n: NimNode,
    spec: DirectSessionTargetSpec,
    task: BootstrapSeedConnectTask,
    status: string,
  ...
  let ready = bootstrapSeedConnectTaskSessionLive(n, task)
  let running = bootstrapSeedConnectTaskRunning(n, task)
  let routeStatus = liveDirectRouteStatus(n, spec.selectedAddrs)
  let directSuccessRatePct =
    if spec.attemptCount > 0:
      int((spec.successCount * 100) div spec.attemptCount)
    else:
      0
  result = %*{
    "ok": ready,
    "peerId": spec.peerId,
    "source": spec.source,
    "selectedAddrs": spec.selectedAddrs,
    "status": status,
    ...
    "routeReady": routeStatus.ready,
    "routePathKind": routeStatus.pathKind,
    "updatedAtMs": spec.updatedAtMs,
    "attemptCount": spec.attemptCount,
    "successCount": spec.successCount,
    "failureCount": spec.failureCount,
    "lastAttemptAtMs": spec.lastAttemptAtMs,
    "lastReadyAtMs": spec.lastReadyAtMs,
    "lastFailureAtMs": spec.lastFailureAtMs,
    "lastFailureReason": spec.lastFailureReason,
    "successRatePct": directSuccessRatePct,
  }
  if not task.isNil:
    result["taskState"] = %bootstrapSeedConnectStateLabel(task.state)
    result["verifiedMuxerWaitMs"] = %task.verifiedMuxerWaitMs
    if task.attemptedAddr.len > 0:
      result["attemptedAddr"] = %task.attemptedAddr
    let taskError =
      if error.len > 0:
        error
      elif task.sessionWarmError.len > 0:
        task.sessionWarmError
      else:
        task.err
    if taskError.len > 0:
...
proc publishDirectSessionTargetSnapshot(
  ...
  if spec.peerId.len == 0 or spec.selectedAddrs.len == 0:
    return
  let task =
    if n.isNil:
      nil
    else:
      n.bootstrapSeedConnectTasks.getOrDefault(spec.taskKey, nil)
  let ready = bootstrapSeedConnectTaskSessionLive(n, task)
  let running = bootstrapSeedConnectTaskRunning(n, task)
  let effectiveStatus =
    if forcedStatus.len > 0:
      forcedStatus
    elif ready:
      "ready"
    elif running:
      ...
      "pending"
  let effectiveError =
    if forcedError.len > 0:
      forcedError
    elif not task.isNil and task.sessionWarmError.len > 0:
      task.sessionWarmError
    elif not task.isNil:
      task.err
    else:
      ""
  let publishedSpec =
    updatePublishedDirectSessionTargetSpec(
      n,
      spec,
      effectiveStatus,
      effectiveError,
    )
  let payload = buildDirectSessionTargetPayload(
    n,
    publishedSpec,
    task,
    effectiveStatus,
  ...
  ffiNodeSafe:
    setDirectSessionTargetState(
      publishedSpec.peerId,
      publishedSpec.selectedAddrs,
      effectiveStatus,
      payload,
      effectiveError,
    )
...
proc reconcileDirectSessionTarget(
  ...
  if n.isNil or spec.peerId.len == 0 or spec.selectedAddrs.len == 0:
    return
  let existingTask = n.bootstrapSeedConnectTasks.getOrDefault(spec.taskKey, nil)
  if bootstrapSeedConnectTaskSessionLive(n, existingTask):
    publishDirectSessionTargetSnapshot(n, spec, "ready")
    return
  if bootstrapSeedConnectTaskRunning(n, existingTask):
    publishDirectSessionTargetSnapshot(n, spec, "running")
    return
  publishDirectSessionTargetSnapshot(n, spec, "running")
  try:
    ffiNodeSafe:
    ...
    publishDirectSessionTargetSnapshot(n, spec, "failed", exc.msg)
    return
  publishDirectSessionTargetSnapshot(n, spec)

proc ensureDirectSessionTargetReconcileLoop(n: NimNode) {.gcsafe, raises: [].} =
  if n.isNil:
    return
  if not n.directSessionTargetReconcileLoop.isNil and not n.directSessionTargetReconcileLoop.finished:
    return
  let loop = proc(): Future[void] {.closure, async: (raises: [CancelledError]).} =
    while n.running.load(moAcquire):
      let targets = copyDirectSessionTargetSpecs(n)
      if n.started and targets.len > 0:
        for spec in targets:
          try:
            reconcileDirectSessionTarget(n, spec)
          except CatchableError as exc:
            debugLog(
              "[nimlibp2p] directSessionTarget reconcile failed peer=" & spec.peerId &
              " err=" & exc.msg
            )
            publishDirectSessionTargetSnapshot(n, spec, "failed", exc.msg)
      await sleepAsync(chronos.milliseconds(750))
  let reconcileFuture = loop()
  n.directSessionTargetReconcileLoop = reconcileFuture
  asyncSpawn(reconcileFuture)
...
proc upsertDirectSessionTarget(
  ...
  let spec = upsertDirectSessionTargetDesiredState(n, peerIdText, addresses, source)
  if spec.taskKey.len == 0:
    return DirectSessionTargetSpec()
  publishDirectSessionTargetSnapshot(n, spec, "pending")
  try:
    reconcileDirectSessionTarget(n, spec)
  except CatchableError as exc:
    publishDirectSessionTargetSnapshot(n, spec, "failed", exc.msg)
  ensureDirectSessionTargetReconcileLoop(n)
  try:
    reconcilePeerRouteSession(n, spec.peerId, spec.source)
  except CatchableError:
    discard
  ensurePeerRouteSessionReconcileLoop(n)
  spec

proc exactSeedConnectTotalBudgetMs(addrs: openArray[string]): int {.gcsafe, raises: [].}

proc bootstrapSeedConnectCanJoinUnfinishedTask(
    requestedIntent: BootstrapSeedConnectIntent,
    existingTask: BootstrapSeedConnectTask
): bool {.gcsafe, raises: [].} =
  not existingTask.isNil and
    requestedIntent == existingTask.intent

proc bootstrapSeedConnectRequestedDeadlineAtMs(
    selectedAddrs: openArray[string],
    safeSecureTimeoutMs: int
): int64 {.gcsafe, raises: [].} =
  let safeBudgetMs =
    if safeSecureTimeoutMs > 0:
      max(1_000, min(safeSecureTimeoutMs, exactSeedConnectTotalBudgetMs(selectedAddrs)))
    else:
      exactSeedConnectTotalBudgetMs(selectedAddrs)
  nowMillis() + int64(safeBudgetMs)
...
proc getOrStartBootstrapSeedConnectTask(
  ...
  if n.isNil or safePeerId.len == 0 or selectedAddrs.len == 0:
    return nil
  let taskKey = bootstrapSeedConnectTaskKey(safePeerId, selectedAddrs)
  if taskKey.len == 0:
    return nil
  let requestedDeadlineAtMs =
    bootstrapSeedConnectRequestedDeadlineAtMs(selectedAddrs, safeSecureTimeoutMs)
  let existingTask = n.bootstrapSeedConnectTasks.getOrDefault(taskKey, nil)
  if bootstrapSeedConnectTaskSessionLive(n, existingTask):
    debugLog(
      "[nimlibp2p] getOrStartBootstrapSeedConnectTask reuse_live peer=" & safePeerId &
      " connKey=" & $existingTask.sessionConnKey &
      " source=" & effectiveSource &
      " intent=" & bootstrapSeedConnectIntentLabel(intent) &
      " existingIntent=" & bootstrapSeedConnectIntentLabel(existingTask.intent)
    )
    return existingTask
  if bootstrapSeedConnectTaskRunning(n, existingTask):
    if intent == bsciForegroundSend and existingTask.intent == bsciBackgroundWarm:
      existingTask.intent = bsciForegroundSend
      existingTask.source = effectiveSource
      existingTask.updatedAtMs = nowMillis()
      existingTask.deadlineAtMs = max(existingTask.deadlineAtMs, requestedDeadlineAtMs)
      debugLog(
        "[nimlibp2p] getOrStartBootstrapSeedConnectTask promote_running peer=" & safePeerId &
        " source=" & effectiveSource &
        " intent=" & bootstrapSeedConnectIntentLabel(intent) &
        " existingIntent=background_warm"
      )
      return existingTask
    if bootstrapSeedConnectCanJoinUnfinishedTask(intent, existingTask):
      existingTask.updatedAtMs = nowMillis()
      existingTask.deadlineAtMs = max(existingTask.deadlineAtMs, requestedDeadlineAtMs)
      debugLog(
        "[nimlibp2p] getOrStartBootstrapSeedConnectTask reuse_running peer=" & safePeerId &
        " source=" & effectiveSource &
        " intent=" & bootstrapSeedConnectIntentLabel(intent) &
        " existingIntent=" & bootstrapSeedConnectIntentLabel(existingTask.intent)
      )
      return existingTask
    debugLog(
      "[nimlibp2p] getOrStartBootstrapSeedConnectTask supersede_running peer=" & safePeerId &
      " source=" & effectiveSource &
      " intent=" & bootstrapSeedConnectIntentLabel(intent) &
      " existingIntent=" & bootstrapSeedConnectIntentLabel(existingTask.intent)
    )
    cancelBootstrapSeedConnectTask(n, taskKey, "superseded_by_new_intent")
  let taskState = BootstrapSeedConnectTask(
    taskKey: taskKey,
    peerId: safePeerId,
    source: effectiveSource,
    intent: intent,
  ...
  n.bootstrapSeedConnectTasks[taskKey] = taskState
  n.bootstrapSeedConnectFutures[taskKey] = task
  asyncSpawn task
  taskState
...
proc exactSeedAllocatedBudgetMs(
    nominalMs: int,
    remainingBudgetMs: int,
    remainingAttempts: int,
    minimumMs: int
): int {.gcsafe.} =
  let safeRemainingBudgetMs = max(remainingBudgetMs, 0)
  if safeRemainingBudgetMs == 0:
    return 0
  let safeRemainingAttempts = max(remainingAttempts, 1)
  let fairShareMs = max(minimumMs, safeRemainingBudgetMs div safeRemainingAttempts)
  min(safeRemainingBudgetMs, min(nominalMs, fairShareMs))

proc exactSeedConnectAllocatedTimeoutMs(
    maText: string,
    remainingBudgetMs: int,
    remainingAttempts: int
): int {.gcsafe.} =
  exactSeedAllocatedBudgetMs(
    exactSeedConnectAttemptBudgetMs(maText),
    remainingBudgetMs,
    remainingAttempts,
    750
  )
...
proc ensurePeerReadyViaExplicitAddresses(
    ...
    ok: bool, err: string, phase: string, detail: JsonNode, verifiedMuxer: Muxer
]] {.async.} =
  let safePeerId = peerIdText.strip()
  if n.isNil or safePeerId.len == 0:
    return (false, "peer id missing", "invalid_peer", %*{"phase": "invalid_peer"}, nil)
  let selectedAddrs = prioritizeGameSeedAddrs(
    preferStableAddrStrings(normalizePeerTargetAddresses(safePeerId, addresses))
  )
  if selectedAddrs.len == 0:
    return (false, "no valid addresses", "invalid_addresses", %*{"phase": "invalid_addresses"}, nil)
  let hintSource =
    if source.strip().len > 0:
      source.strip()
    else:
  ...
  for idx, addr in selectedAddrs:
    ...
    let connectTimeoutMs = exactSeedConnectAllocatedTimeoutMs(
      addr,
      remainingBudgetMs,
      remainingAttempts
    )
    ...
    let allowLateDetach = remainingAttempts == 1 and
    ...
    var connectOk = connectRes.ok
    attemptDetail["connectOk"] = %connectOk
    attemptDetail["peerConnected"] = %connectRes.peerConnected
    attemptDetail["verifiedMuxer"] = %(not connectRes.verifiedMuxer.isNil)
    attemptDetail["verifiedMuxerWaitMs"] = %0
    attemptDetail["phase"] = %(
      if connectOk: "connect_done"
      elif connectRes.err == "connect timeout": "connect_timeout"
      elif connectRes.err == "peer muxer not ready": "muxer_timeout"
      else: "connect_failed"
    )
    if connectRes.err.len > 0:
      attemptDetail["error"] = %connectRes.err
    if not connectOk and allowLateDetach:
      let elapsedBeforeLateCheck = int(nowMillis() - startedAtMs)
      ...
      if lateConnectWindowMs > 0:
        debugLog(
          "[nimlibp2p] ensurePeerReadyViaExplicitAddresses attempt:late_connect_wait_start peer=" &
          safePeerId &
          " addr=" & addr &
          " waitMs=" & $lateConnectWindowMs
        )
        var latePeerConnected = false
        if peerRes.isOk():
          latePeerConnected = await waitForPeerConnectedWithinDeadlineAsync(
            n,
            peerRes.get(),
            lateConnectWindowMs,
          )
          attemptDetail["peerConnected"] = %latePeerConnected
        var lateBootstrapConnected = false
        if not latePeerConnected:
          lateBootstrapConnected = await waitForBootstrapPeerConnectedWithinDeadlineAsync(
            n,
            safePeerId,
            lateConnectWindowMs,
          )
          attemptDetail["bootstrapConnected"] = %lateBootstrapConnected
        if latePeerConnected or lateBootstrapConnected:
          connectOk = true
          attemptDetail["connectOk"] = %true
          attemptDetail["lateConnected"] = %true
          attemptDetail["phase"] = %"connect_done"
          if attemptDetail.kind == JObject and attemptDetail.hasKey("error"):
            attemptDetail.delete("error")
        debugLog(
          "[nimlibp2p] ensurePeerReadyViaExplicitAddresses attempt:late_connect_wait_done peer=" &
          safePeerId &
          " addr=" & addr &
          " peerConnected=" & $latePeerConnected &
          " bootstrapConnected=" & $lateBootstrapConnected &
          " connectOk=" & $connectOk
        )
    if not connectOk:
      lastPhase =
        if connectRes.err == "connect timeout":
          "connect_timeout"
        elif connectRes.err == "peer muxer not ready":
          "muxer_timeout"
        else:
          "connect_failed"
      if connectRes.err.len > 0:
      ...
      continue
    if not connectRes.verifiedMuxer.isNil:
      let connectedIdentifyOk =
        if peerRes.isOk():
          peerCurrentlyIdentified(n, peerRes.get())
        else:
          false
      attemptDetail["phase"] = %(if connectedIdentifyOk: "ready" else: "connected")
      attemptDetail["secureOk"] = %true
      attemptDetail["identifiedOk"] = %connectedIdentifyOk
      return (
        true,
        "",
        if connectedIdentifyOk: "ready" else: "connected",
        attemptDetail,
        connectRes.verifiedMuxer,
      )
    let elapsedBeforeMuxer = int(nowMillis() - startedAtMs)
    remainingBudgetMs = totalBudgetMs - elapsedBeforeMuxer
    if connectRes.peerConnected and peerRes.isOk():
      let connectedPeer = peerRes.get()
      let verifiedMuxerWaitMs =
        min(
          max(remainingBudgetMs, 0),
          if lowerAddr.contains("/quic-v1") or lowerAddr.contains("/quic/"):
            1_500
          else:
            500
        )
      let connectedIdentifyOk = peerCurrentlyIdentified(n, connectedPeer)
      let verifiedMuxer = await waitForVerifiedMuxerWithinDeadline(
        n,
        connectedPeer,
        verifiedMuxerWaitMs,
      )
      debugLog(
        "[nimlibp2p] ensurePeerReadyViaExplicitAddresses attempt:muxer_shortcut peer=" &
        safePeerId &
        " addr=" & addr &
        " muxer=" & $(not verifiedMuxer.isNil) &
        " identified=" & $connectedIdentifyOk &
        " waitMs=" & $verifiedMuxerWaitMs
      )
      if not verifiedMuxer.isNil:
        attemptDetail["phase"] = %(if connectedIdentifyOk: "ready" else: "connected")
        attemptDetail["secureOk"] = %true
        attemptDetail["identifiedOk"] = %connectedIdentifyOk
        attemptDetail["verifiedMuxer"] = %true
        attemptDetail["verifiedMuxerWaitMs"] = %verifiedMuxerWaitMs
        return (
          true,
          "",
          if connectedIdentifyOk: "ready" else: "connected",
    ...
    if readyRes.ok:
      var verifiedMuxer: Muxer = nil
      var verifiedMuxerWaitMs = 0
      if peerRes.isOk():
        let elapsedBeforeVerifiedMuxer = int(nowMillis() - startedAtMs)
        let remainingVerifiedBudgetMs = max(totalBudgetMs - elapsedBeforeVerifiedMuxer, 0)
        verifiedMuxerWaitMs =
          min(
            remainingVerifiedBudgetMs,
            if lowerAddr.contains("/quic-v1") or lowerAddr.contains("/quic/"):
              2_500
            else:
              750
          )
        verifiedMuxer = await waitForVerifiedMuxerWithinDeadline(
          n,
          peerRes.get(),
          verifiedMuxerWaitMs,
        )
      if attemptDetail.kind == JObject:
        attemptDetail["verifiedMuxer"] = %(not verifiedMuxer.isNil)
        attemptDetail["verifiedMuxerWaitMs"] = %verifiedMuxerWaitMs
      if verifiedMuxer.isNil:
        lastPhase = "muxer_timeout"
        lastError = "peer muxer not ready"
        if attemptDetail.kind == JObject:
          attemptDetail["phase"] = %"muxer_timeout"
          attemptDetail["error"] = %lastError
        lastDetail = attemptDetail
        continue
      return (true, "", "ready", attemptDetail, verifiedMuxer)
  ...
  (false, lastError, lastPhase, lastDetail, nil)

proc hasTsnetTargetAddress(addresses: openArray[string]): bool {.gcsafe, raises: [].} =
  for addr in addresses:
    if addr.toLowerAscii().contains("/tsnet"):
      return true
  false

proc tailnetReadySnapshot(n: NimNode): JsonNode {.gcsafe, raises: [].} =
  if n.isNil:
    return %*{
      "ok": false,
      "phase": "invalid_node",
      "providerReady": false,
      "proxyListenersReady": false,
      "error": "node unavailable",
    }
  let payload = buildTailnetStatusPayload(n)
  if payload.kind == JObject:
    return cloneJson(payload)
  %*{
    "ok": false,
    "phase": "invalid_tailnet_status",
    "providerReady": false,
    "proxyListenersReady": false,
    "error": "tailnet status unavailable",
  }

proc tsnetLocalReady(n: NimNode): bool {.gcsafe, raises: [].} =
  let payload = tailnetReadySnapshot(n)
  jsonGetBool(payload, "ok", false) and
    jsonGetBool(payload, "providerReady", false)

proc ensureLocalTsnetReady(
    n: NimNode,
    timeoutMs: int
): Future[tuple[ok: bool, err: string, detail: JsonNode]] {.async.} =
  var snapshot = tailnetReadySnapshot(n)
  if jsonGetBool(snapshot, "providerReady", false):
    snapshot["phase"] = %"local_tailnet_provider_ready"
    return (true, "", snapshot)
  let transport = activeTsnetTransport(n)
  if transport.isNil:
    snapshot["phase"] = %"local_tailnet_unavailable"
    snapshot["error"] = %"tsnet transport unavailable"
    return (false, "tsnet transport unavailable", snapshot)
  let deadlineMs = nowMillis() + int64(max(timeoutMs, 0))
  if not jsonGetBool(snapshot, "providerReady", false):
    let warmRes = transport.warmProvider()
    if warmRes.isErr():
      snapshot = tailnetReadySnapshot(n)
      snapshot["phase"] = %"local_tailnet_warm_failed"
      snapshot["error"] = %warmRes.error
      return (false, warmRes.error, snapshot)
    discard mergeTsnetListenAddrsIntoPeerInfo(n, transport)
    publishTsnetLocalPeerInfo(n, transport)
    snapshot = tailnetReadySnapshot(n)
  while true:
  ...
  let errMsg =
    if not jsonGetBool(snapshot, "providerReady", false):
      "local tailnet provider not ready"
    else:
      "local tailnet not ready"
  snapshot["phase"] = %"local_tailnet_provider_not_ready"
  snapshot["error"] = %errMsg
  (false, errMsg, snapshot)
...
proc executeSendDirectCommand(
  ...
  template finalizeKickoffFailure(defaultStage: string, errText: string, detailNode: JsonNode = newJNull()) =
    if command.streamKey.len > 0:
      ffiNodeSafe:
        var payload = getSendWithAckKickoffPayloadSnapshot(command.streamKey)
        if payload.kind != JObject:
          payload = newJObject()
        let finalStage = jsonGetStr(payload, "stage").strip()
        payload["ok"] = %false
        payload["requestKey"] = %command.streamKey
        payload["peerId"] = %command.peerId
        payload["stage"] = %(
          if finalStage.len > 0:
            finalStage
        ...
        payload["addressCount"] = %command.addresses.len
        payload["source"] = %command.source
        payload["ackRequested"] = %command.ackRequested
        if command.messageId.len > 0:
          payload["messageId"] = %command.messageId
        if command.op.len > 0:
          payload["op"] = %command.op
        if detailNode.kind == JObject:
          mergeJsonObjectFields(payload, detailNode)
        payload["error"] = %errText
        setSendWithAckKickoffState(
          command.streamKey,
          "done",
  ...
  let overallTimeoutMs =
    if command.timeoutMs > 0:
      max(command.timeoutMs, 500)
    else:
      12_000
  let deadlineMs = nowMillis() + int64(overallTimeoutMs)

  proc remainingBudgetMs(): int =
    max(0, int(deadlineMs - nowMillis()))
  ...
  debugLog("[nimlibp2p] cmdSendDirect start peer=" & command.peerId &
    ...
    " sessionConnKey=" & $command.sessionConnKey)
  ...
  if local.len == 0:
    ...
    return
  let normalizedPeerId = command.peerId.strip()
  if command.addresses.len > 0:
  ...
  let selectedTargetAddrs =
    if command.addresses.len > 0:
      prioritizeGameSeedAddrs(preferStableAddrStrings(command.addresses))
    else:
      @[]
  var envelope: JsonNode
  if command.jsonString.len > 0:
  ...
  var peerReadyErr = ""
  var reuseVerifiedLiveConnection = false
  var verifiedMuxer: Muxer = nil
  var usedRelayRouteSession = false
  var preferredSessionConnKey =
    if command.sessionConnKey > 0 and n.dmService.hasLiveSessionConnKey(command.sessionConnKey):
      command.sessionConnKey
    elif selectedTargetAddrs.len > 0:
      let snapshotConnKey = ffiNodeSafe:
        getLiveReadyDirectSessionTargetConnKey(n, normalizedPeerId, selectedTargetAddrs)
      if snapshotConnKey > 0 and n.dmService.hasLiveSessionConnKey(snapshotConnKey):
        snapshotConnKey
      else:
        0
    else:
      0
  if command.addresses.len > 0:
    if hasTsnetTargetAddress(command.addresses):
      let remainingMs = remainingBudgetMs()
      if remainingMs <= 0:
        peerReadyOk = false
        peerReadyErr = "send timeout budget exhausted before local tailnet ready"
        updateKickoffStage("local_tailnet_budget_exhausted", peerReadyErr)
      else:
        updateKickoffStage("local_tailnet_ready_wait")
        let tailnetReady = await ensureLocalTsnetReady(n, remainingMs)
        if not tailnetReady.ok:
          peerReadyOk = false
          peerReadyErr = tailnetReady.err
          updateKickoffStage("local_tailnet_not_ready", peerReadyErr, tailnetReady.detail)
        else:
          updateKickoffStage("local_tailnet_ready", "", tailnetReady.detail)
    if not peerReadyErr.isEmptyOrWhitespace():
      discard
    elif command.boolParam:
      if selectedTargetAddrs.len == 0:
        peerReadyOk = false
        peerReadyErr = "no dialable seed address"
        updateKickoffStage(
          "connect_exact_failed",
          peerReadyErr,
          %*{
            "pathSelection": "relay_only",
            "addressCount": command.addresses.len,
            "selectedAddrCount": selectedTargetAddrs.len,
          }
        )
      else:
        let routeWaitBudgetMs = remainingBudgetMs()
        let sessionReady = await waitForDirectOrRelaySessionReady(
          n,
          normalizedPeerId,
          selectedTargetAddrs,
          routeWaitBudgetMs,
        ...
        if sessionReady.ok and sessionReady.connKey > 0:
          preferredDmAddrs = @[]
          preferredSessionConnKey = sessionReady.connKey
          reuseVerifiedLiveConnection = false
          verifiedMuxer = nil
          usedRelayRouteSession = sessionReady.useRelay
          peerReadyOk = true
          peerReadyErr = ""
          updateKickoffStage(
            "peer_ready",
            "",
            %*{
              "pathSelection": "direct_ready_or_relay",
              "sendPath":
                if sessionReady.useRelay:
                  "relay_ready_session"
                else:
                  "direct_ready_session",
              "addressCount": command.addresses.len,
              "selectedAddrCount": selectedTargetAddrs.len,
              "hintAddressCount": selectedTargetAddrs.len,
              "directTargetStatus": sessionReady.directSnapshot.status,
              "directTargetTaskRunning": sessionReady.directSnapshot.taskRunning,
              "directTargetSessionConnKey": sessionReady.directSnapshot.sessionConnKey,
              "relayTargetStatus": sessionReady.relaySnapshot.status,
              "relayTargetTaskRunning": sessionReady.relaySnapshot.taskRunning,
              "relayTargetSessionConnKey": sessionReady.relaySnapshot.sessionConnKey,
              "sessionConnKey": sessionReady.connKey,
            }
          )
        else:
          peerReadyOk = false
          peerReadyErr = sessionReady.err
          updateKickoffStage(
            "connect_exact_failed",
            peerReadyErr,
            %*{
              ...
              "directTargetStatus": sessionReady.directSnapshot.status,
              "directTargetTaskRunning": sessionReady.directSnapshot.taskRunning,
              "directTargetError": sessionReady.directSnapshot.error,
              "relayTargetStatus": sessionReady.relaySnapshot.status,
              "relayTargetTaskRunning": sessionReady.relaySnapshot.taskRunning,
              "relayTargetError": sessionReady.relaySnapshot.error,
            }
          )
    elif preferredSessionConnKey > 0:
      peerReadyOk = true
      peerReadyErr = ""
      updateKickoffStage(
        "peer_ready",
        "",
        %*{
          "sessionReuse": "command_session_conn",
          "sessionConnKey": preferredSessionConnKey,
          "addressCount": command.addresses.len,
          "sendAddressCount": preferredDmAddrs.len,
        }
      )
    else:
      let remainingMs = remainingBudgetMs()
      if remainingMs <= 0:
        peerReadyOk = false
        peerReadyErr = "send timeout budget exhausted before connect"
        updateKickoffStage("connect_exact_budget_exhausted", peerReadyErr)
        debugLog(
          "[nimlibp2p] cmdSendDirect connect budget exhausted peer=" & command.peerId &
          " addrCount=" & $command.addresses.len
        )
      else:
        updateKickoffStage("connect_exact")
        let directTargetSpec = upsertDirectSessionTarget(
          n,
          normalizedPeerId,
          selectedTargetAddrs,
          if command.source.len > 0: command.source else: "cmd_send_direct",
        )
        var directTargetSnapshot = ffiNodeSafe:
          liveDirectSessionTargetSnapshot(n, normalizedPeerId, selectedTargetAddrs)
        var directTargetConnKey = ffiNodeSafe:
          getLiveReadyDirectSessionTargetConnKey(n, normalizedPeerId, selectedTargetAddrs)
        if directTargetConnKey > 0 and
            not n.dmService.hasLiveSessionConnKey(directTargetConnKey):
          directTargetConnKey = 0
        var directTargetDetail = %*{
          "joinedDirectTarget": directTargetSpec.taskKey.len > 0,
          "taskKey": directTargetSpec.taskKey,
          "status": directTargetSnapshot.status,
          "taskRunning": directTargetSnapshot.taskRunning,
          "sessionWarmOk": directTargetSnapshot.sessionWarmOk,
          "verifiedMuxer": directTargetSnapshot.verifiedMuxer,
          "sessionConnKey": directTargetSnapshot.sessionConnKey,
          "addressCount": selectedTargetAddrs.len,
        }
        if directTargetSnapshot.error.len > 0:
          directTargetDetail["error"] = %directTargetSnapshot.error
        if directTargetConnKey > 0:
          peerReadyOk = true
          peerReadyErr = ""
          reuseVerifiedLiveConnection = false
          verifiedMuxer = nil
          preferredSessionConnKey = directTargetConnKey
          directTargetDetail["sessionReuse"] = %"direct_target_ready"
          directTargetDetail["sessionConnKey"] = %directTargetConnKey
          updateKickoffStage("peer_ready", "", directTargetDetail)
          debugLog(
            "[nimlibp2p] cmdSendDirect direct target ready peer=" & command.peerId &
            " sessionConnKey=" & $directTargetConnKey &
            " status=" & directTargetSnapshot.status
          )
        else:
          let directTargetErr =
            if directTargetSnapshot.error.len > 0:
              directTargetSnapshot.error
            elif directTargetSpec.taskKey.len == 0:
              "direct session target unavailable"
            elif directTargetSnapshot.status.toLowerAscii() == "failed":
              "bootstrap direct session failed"
            else:
              "bootstrap direct session probing"
          directTargetDetail["fallbackPath"] = %"relay_route_session"
          updateKickoffStage("connect_relay_fallback", directTargetErr, directTargetDetail)
          let fallbackBudgetMs = remainingBudgetMs()
          if fallbackBudgetMs <= 0:
            peerReadyOk = false
            peerReadyErr = "send timeout budget exhausted before relay fallback"
            updateKickoffStage("connect_exact_failed", peerReadyErr, directTargetDetail)
          else:
            let routeReady = await waitForPeerRouteSessionReady(
              n,
              normalizedPeerId,
              fallbackBudgetMs,
              if command.source.len > 0: command.source else: "cmd_send_direct",
            )
            if routeReady.ok and routeReady.connKey > 0:
              preferredDmAddrs = @[]
              preferredSessionConnKey = routeReady.connKey
              reuseVerifiedLiveConnection = false
              verifiedMuxer = nil
              usedRelayRouteSession = true
              peerReadyOk = true
              peerReadyErr = ""
              directTargetDetail["sendPath"] = %"relay_ready_session"
              directTargetDetail["relayTargetStatus"] = %routeReady.snapshot.status
              directTargetDetail["relayTargetTaskRunning"] = %routeReady.snapshot.taskRunning
              directTargetDetail["sessionConnKey"] = %routeReady.connKey
              updateKickoffStage("peer_ready", "", directTargetDetail)
            else:
              peerReadyOk = false
              peerReadyErr = routeReady.err
              directTargetDetail["relayTargetStatus"] = %routeReady.snapshot.status
              directTargetDetail["relayTargetTaskRunning"] = %routeReady.snapshot.taskRunning
              directTargetDetail["relayTargetError"] = %routeReady.snapshot.error
              updateKickoffStage("connect_exact_failed", peerReadyErr, directTargetDetail)
  else:
    ...
    if connectivityOk:
      ...
      if remainingMs <= 0:
        peerReadyOk = false
        peerReadyErr = "send timeout budget exhausted before identify"
        updateKickoffStage("identify_wait_budget_exhausted", peerReadyErr)
      else:
        updateKickoffStage("identify_wait")
        let identifyWaitMs =
          if overallTimeoutMs > 0:
            max(min(remainingMs div 2, 8_000), 1_000)
          else:
            8_000
  ...
  if not peerReadyOk:
    ...
    finalizeKickoffFailure("peer_ready_failed", errMsg)
    ...
    return
  let sendPath =
    if usedRelayRouteSession:
      "relay_ready_session"
    elif preferredSessionConnKey > 0 and command.boolParam:
      "direct_ready_session"
    elif preferredSessionConnKey > 0:
      "warmed_session_conn"
    elif not verifiedMuxer.isNil:
      "verified_live_muxer"
    elif reuseVerifiedLiveConnection:
      "verified_live_connection"
    else:
      "generic_connectivity"
  let sendRelayOnly = command.boolParam and usedRelayRouteSession
  debugLog("[nimlibp2p] cmdSendDirect connectivity peer=" & command.peerId &
    " ok=true ackRequested=" & $command.ackRequested &
    " path=" & sendPath &
    " relayOnly=" & $sendRelayOnly)
  let sendBudgetMs = remainingBudgetMs()
  if sendBudgetMs <= 0:
    let errMsg = "send timeout budget exhausted before dm send"
    command.boolResult = false
    command.errorMsg = errMsg
    n.lastDirectError = errMsg
    command.resultCode = NimResultError
    updateKickoffStage("send_budget_exhausted", errMsg)
    finalizeKickoffFailure("send_budget_exhausted", errMsg)
    debugLog(
      "[nimlibp2p] cmdSendDirect send budget exhausted peer=" & command.peerId &
      " mid=" & mid
    )
    return
  let timeout = chronos.milliseconds(sendBudgetMs)
    ...
    let resultTuple =
      if command.ackRequested:
        await sendDirectWithAckWaiter(
          ...
          verifiedMuxer = verifiedMuxer,
          preferredConnKey = preferredSessionConnKey,
          reuseVerifiedLiveConnection = reuseVerifiedLiveConnection,
          relayOnly = sendRelayOnly,
...
proc runSendDirectKickoffAsync(
  ...
  let remainingBudgetMs =
    if deadlineAtMs > 0:
      max(0, int(deadlineAtMs - startedAtMs))
    else:
      max(timeoutMs, 0)
  if remainingBudgetMs <= 0:
    ffiNodeSafe:
      setSendWithAckKickoffState(
        ...
        %*{
          ...
          "peerId": safePeerId,
          "stage": "send_with_ack",
          "addressCount": safeAddresses.len,
          "source": safeSource,
          "ackRequested": ackRequested,
          "error": "send_with_ack_timeout",
          "durationMs": 0,
        },
  ...
  debugLog(
    ...
    " relayOnly=" & $(not taskState.isNil and taskState.forceRelayConnectivity) &
    " timeoutMs=" & $timeoutMs &
    " remainingBudgetMs=" & $remainingBudgetMs &
    " source=" & safeSource
  )
  ...
  cmd.source =
    if safeSource.len > 0:
      ...
      ""
  cmd.boolParam = not taskState.isNil and taskState.forceRelayConnectivity
  if payloadMid.len > 0:
  ...
  try:
    ...
    destroyCommand(cmd)
    if not taskState.isNil:
      taskState.running = false
      taskState.updatedAtMs = nowMillis()
      taskState.completedAtMs = taskState.updatedAtMs
      let currentTask = n.sendDirectKickoffTasks.getOrDefault(safeRequestKey, nil)
      if currentTask == taskState and safeRequestKey in n.sendDirectKickoffFutures:
        n.sendDirectKickoffFutures.del(safeRequestKey)
...
proc libp2p_bootstrap_seed_connect_exact*(
  ...
  var normalizedBootstrapAddrs: seq[string] = @[]
  addBootstrapAddresses(safePeerId, addresses, normalizedBootstrapAddrs)
  if normalizedBootstrapAddrs.len > 0:
    addresses = normalizedBootstrapAddrs
  ...
  for idx, addr in selectedAddrs:
    ...
    let timeoutMs = exactSeedConnectAllocatedTimeoutMs(
      addr,
      remainingBudgetMs,
      remainingAttempts
    )
    ...
    if rc == NimResultOk:
      ...
      if not peerConnected and peerRes.isOk():
        peerConnected = peerCurrentlyConnected(node, peerRes.get())
        attemptNode["peerConnected"] = %peerConnected
      if secureOk and peerRes.isOk():
        let elapsedBeforeIdentify = int(nowMillis() - exactStartedAtMs)
        remainingBudgetMs = totalBudgetMs - elapsedBeforeIdentify
        let identifyWaitMs =
          exactSeedAllocatedBudgetMs(
            if lowerAddr.contains("/quic-v1") or lowerAddr.contains("/quic/"):
              8_000
...
proc runBootstrapSeedConnectExactAsync(
  ...
  let publishKickoffState =
    not taskState.isNil and
    bootstrapSeedConnectPublishesKickoffState(taskState.intent)
  debugLog(
    "[nimlibp2p] bootstrapSeedConnectExactAsync start peer=" & safePeerId &
    " source=" & effectiveSource &
    (if not taskState.isNil:
      " intent=" & bootstrapSeedConnectIntentLabel(taskState.intent)
    else:
      "") &
  ...
  try:
    ...
    if registerRes.isErr():
      ...
      if not taskState.isNil:
        taskState.state = bscsFailed
        taskState.updatedAtMs = nowMillis()
        taskState.ok = false
        taskState.err = registerRes.error
      if publishKickoffState:
        ffiNodeSafe:
          setBootstrapSeedConnectKickoffState(
            safePeerId,
            selectedAddrs,
    ...
    for idx, addr in selectedAddrs:
      var remainingBudgetMs =
        if not taskState.isNil and taskState.deadlineAtMs > 0:
          int(taskState.deadlineAtMs - nowMillis())
        else:
          fallbackTotalBudgetMs - int(nowMillis() - exactStartedAtMs)
      if remainingBudgetMs <= 0:
        lastError = "bootstrap_seed_connect_timeout"
        break
      ...
      attemptedAddr = addr
      if not taskState.isNil:
        taskState.attemptedAddr = attemptedAddr
      let lowerAddr = addr.toLowerAscii()
      let addrPathKind =
        if "/p2p-circuit" in lowerAddr: "relay" else: "direct"
      let remainingAttempts = max(1, selectedAddrs.len - idx)
      ...
      let ma = parseDialableMultiaddr(addr).valueOr:
        attemptNode["ok"] = %false
        attemptNode["error"] = %("invalid multiaddr: " & error)
        attempts.add(attemptNode)
        lastError = "invalid multiaddr: " & error
        continue
      discard ma
      ...
      debugLog(
        "[nimlibp2p] bootstrapSeedConnectExactAsync attempt:session_warm_start peer=" & safePeerId &
        " addr=" & addr &
        " waitMs=" & $warmWaitMs
      )
      var sessionReadyOk = false
      var warmDetail = WarmSeededDirectSessionResult(
        verifiedMuxer: nil,
        verifiedMuxerWaitMs: 0,
        sessionConnKey: 0,
        sessionWarmOk: false,
      ...
      try:
        if peerRes.isOk():
          warmDetail = await warmSeededDirectSessionAsync(
            node,
            peerRes.get(),
            safePeerId,
            addr,
        ...
        warmDetail.sessionWarmError =
          if exc.msg.len > 0: exc.msg else: $exc.name
      sessionReadyOk = warmDetail.sessionWarmOk and warmDetail.sessionConnKey > 0
      if peerRes.isOk():
        peerConnected = peerCurrentlyConnected(node, peerRes.get())
      attemptNode["ok"] = %sessionReadyOk
      attemptNode["peerConnected"] = %peerConnected
      attemptNode["connectOk"] = %sessionReadyOk
      attemptNode["secureOk"] = %sessionReadyOk
      attemptNode["verifiedMuxer"] = %(not warmDetail.verifiedMuxer.isNil)
      attemptNode["verifiedMuxerWaitMs"] = %warmDetail.verifiedMuxerWaitMs
      attemptNode["sessionConnKey"] = %warmDetail.sessionConnKey
      attemptNode["sessionWarmOk"] = %warmDetail.sessionWarmOk
      attemptNode["sessionReadyOk"] = %sessionReadyOk
      attemptNode["identifiedOk"] = %sessionReadyOk
      if warmDetail.sessionWarmError.len > 0:
        attemptNode["error"] = %warmDetail.sessionWarmError
        attemptNode["sessionWarmError"] = %warmDetail.sessionWarmError
      if not taskState.isNil:
        taskState.verifiedMuxer = warmDetail.verifiedMuxer
        taskState.verifiedMuxerWaitMs = warmDetail.verifiedMuxerWaitMs
        taskState.sessionConnKey = warmDetail.sessionConnKey
        taskState.sessionWarmOk = warmDetail.sessionWarmOk
        taskState.sessionWarmError = warmDetail.sessionWarmError
      debugLog(
        "[nimlibp2p] bootstrapSeedConnectExactAsync attempt:session_warm_done peer=" & safePeerId &
        " addr=" & addr &
        " sessionWarmOk=" & $warmDetail.sessionWarmOk &
        " sessionConnKey=" & $warmDetail.sessionConnKey &
        (if warmDetail.sessionWarmError.len > 0:
          " err=" & warmDetail.sessionWarmError
        else:
          "")
      )
      ...
      let bootstrapConnected =
        try:
          bootstrapStatusShowsConnectedPeer(bootstrapNode, safePeerId)
        except Exception:
          false
      attemptNode["bootstrapConnected"] = %bootstrapConnected
      attempts.add(attemptNode)
      ...
      if sessionReadyOk:
        ...
        ffiNodeSafe:
          setDirectSessionTargetState(
            ...
            %*{
              ...
              "taskRunning": false,
              "sessionWarmOk": true,
              "verifiedMuxer": (not warmDetail.verifiedMuxer.isNil),
              "verifiedMuxerWaitMs": warmDetail.verifiedMuxerWaitMs,
              "sessionConnKey": warmDetail.sessionConnKey,
            },
          )
        debugLog(
          ...
          " addr=" & addr &
          " connectOk=" & $sessionReadyOk &
          " secureOk=" & $sessionReadyOk &
          " peerConnected=" & $peerConnected &
          " bootstrapConnected=" & $bootstrapConnected
        )
        asyncSpawn(promoteConnectedBootstrapPeerDetached(
        ...
        payload["connectOk"] = %sessionReadyOk
        payload["secureOk"] = %sessionReadyOk
        payload["identifiedOk"] = %sessionReadyOk
        payload["sessionReadyOk"] = %sessionReadyOk
        if attemptNode.kind == JObject:
          let verifiedMuxerNode = attemptNode{"verifiedMuxer"}
          if not verifiedMuxerNode.isNil:
            payload["verifiedMuxer"] = cloneJson(verifiedMuxerNode)
          let verifiedMuxerWaitMsNode = attemptNode{"verifiedMuxerWaitMs"}
          if not verifiedMuxerWaitMsNode.isNil:
            payload["verifiedMuxerWaitMs"] = cloneJson(verifiedMuxerWaitMsNode)
          let sessionConnKeyNode = attemptNode{"sessionConnKey"}
          if not sessionConnKeyNode.isNil:
            payload["sessionConnKey"] = cloneJson(sessionConnKeyNode)
          let sessionWarmOkNode = attemptNode{"sessionWarmOk"}
          if not sessionWarmOkNode.isNil:
            payload["sessionWarmOk"] = cloneJson(sessionWarmOkNode)
          let sessionWarmErrorNode = attemptNode{"sessionWarmError"}
          if not sessionWarmErrorNode.isNil:
            payload["sessionWarmError"] = cloneJson(sessionWarmErrorNode)
        if publishKickoffState:
          ffiNodeSafe:
            setBootstrapSeedConnectKickoffState(
              safePeerId,
      ...
      let sessionWarmError = jsonGetStr(attemptNode, "sessionWarmError")
      if sessionWarmError.len > 0:
        lastError = sessionWarmError
      elif jsonGetBool(attemptNode, "sessionWarmOk", false):
        lastError = "bootstrap direct session unavailable"
    ...
    if not taskState.isNil:
      taskState.state = bscsCancelled
      taskState.updatedAtMs = nowMillis()
      taskState.ok = false
      taskState.err = exc.msg
      taskState.sessionWarmOk = false
      taskState.sessionWarmError = exc.msg
      taskState.sessionConnKey = 0
      taskState.verifiedMuxer = nil
    if publishKickoffState:
      ffiNodeSafe:
        setBootstrapSeedConnectKickoffState(
          safePeerId,
    ...
    if not taskState.isNil:
      let currentTask = node.bootstrapSeedConnectTasks.getOrDefault(taskState.taskKey, nil)
      if currentTask == taskState and taskState.taskKey in node.bootstrapSeedConnectFutures:
        node.bootstrapSeedConnectFutures.del(taskState.taskKey)
...
proc bootstrapSeedConnectExactKickoffThread(
  ...
  let safeLimit =
    if kickoffArg.limit > 0:
      min(max(int(kickoffArg.limit), 1), orderedAddrs.len)
    else:
      orderedAddrs.len
  let selectedAddrs =
    if safeLimit > 0:
...
proc libp2p_bootstrap_seed_connect_exact_kickoff*(
  ...
  let cmd = makeCommand(cmdBootstrapSeedConnectExactKickoff)
  cmd.peerId = safePeerId
  cmd.addresses = selectedAddrs
  cmd.source = (if source.isNil: "" else: $source).strip()
  cmd.requestLimit = safeLimit
  cmd.timeoutMs =
    if secureTimeoutMs > 0:
      max(min(int(secureTimeoutMs), 10_000), 250)
    else:
      5_000
  let queued = submitCommandNoWait(node, cmd)
  if queued:
    debugLog(
      "[nimlibp2p] bootstrapSeedConnectExact kickoff queued peer=" & safePeerId &
      " limit=" & $safeLimit &
      " secureTimeoutMs=" & $cmd.timeoutMs
    )
    return true
  let err = block:
    let trimmed = cmd.errorMsg.strip()
    if trimmed.len > 0:
      trimmed
    else:
      "bootstrap_seed_connect_exact_kickoff_failed"
  destroyCommand(cmd)
  setBootstrapSeedConnectKickoffState(
    safePeerId,
    selectedAddrs,
...
proc libp2p_bootstrap_seed_connect_exact_kickoff_status_for_addresses*(
    peerId: cstring,
    addressesJson: cstring,
    limit: cint
): cstring {.exportc, cdecl, dynlib.} =
  ...
  if safePeerId.len == 0 or safeAddressesJson.len == 0:
    return allocCString(
      bootstrapSeedConnectKickoffStateJson(
        safePeerId,
        "",
        "missing",
        newJNull(),
...
proc libp2p_direct_session_target_set*(
    handle: pointer,
    peerId: cstring,
    addressesJson: cstring,
    source: cstring,
  ...
  let spec =
    upsertDirectSessionTargetDesiredState(
      node,
      safePeerId,
      selectedAddrs,
      (if source.isNil: "" else: $source).strip(),
    )
  if spec.taskKey.len > 0:
    return true
  let err = "direct_session_target_set_failed"
  setDirectSessionTargetState(
    safePeerId,
    selectedAddrs,
    "failed",
...
proc libp2p_direct_session_target_clear*(
    handle: pointer,
    peerId: cstring,
    addressesJson: cstring,
    limit: cint
  ...
  if spec.taskKey.len > 0:
    setDirectSessionTargetState(
      ...
      %*{
        ...
        "lastFailureAtMs": spec.lastFailureAtMs,
        "lastFailureReason": spec.lastFailureReason,
        "error": "direct_target_cleared",
      },
      "direct_target_cleared",
    )
    return true
  let err = "direct_session_target_clear_failed"
  recordRuntimeError(err)
  false

proc libp2p_direct_session_target_status_for_addresses*(
    handle: pointer,
    peerId: cstring,
    addressesJson: cstring,
    limit: cint
  ...
  allocCString(
    liveDirectSessionTargetStateJson(
      node,
      safePeerId,
      orderedAddrs[0 ..< safeLimit],
    )
