...
- **边缘混淆**：DEX/桥接阶段通过单边流动性池或聚合路由（历史方案见已移除的 `examples/dex/production_kline_design.md`）打碎大额 USDC，结合多次闪兑/批处理，减少入口端被机器学习关联的概率。
...
| P4 | CoinJoin/混币协调 | 已移除的 `examples/dex/dex_node` 曾规划增 `dex.mixers` 主题，借 `libp2p/protocols/http/http.nim` 协议交换混币参数；`examples/bridge/services/executorservice.nim` 写入匿名标签 | 已移除的 `examples/dex/scripts/run_coinjoin_demo.sh`、storage 匿名标签 | 依赖 P3 匿名标识 | ⚙️ 进行中：`dex_node` 加入 Mixer CLI/`dex.mixers` Gossip + `/dex/mixer` HTTP route，周期性广播混币请求并打印计划，尚待示例脚本与 Executor 存储匿名标签 |
| P5 | 指标与运维匿名化 | `libp2p/services/metricsservice.nim` / `libp2p/metrics_exporter.nim` 提供匿名模式（PeerId hash、刷新时间随机化），`docs/metrics.md` + 已移除的 `examples/dex/.env` 补 flag | `nimble test -d:libp2p_metrics_anonymized` 通过，Prometheus 不含明文 PeerId | MetricsService gcsafe 改造 | ⏸ 未启动（依赖 Resource/Metrics gcsafe 重构完成） |

- **推进顺序**：优先网络层（P1、P2）→ 证明入口（P3）→ 资金流混淆（P4）→ 运维匿名（P5）。
- **验证策略**：新增单元测试 + 示例脚本；在沙箱复现“桥接混淆→隐私 gossip→洋葱支付”的端到端链路。

> P4 目前落地的内容：已移除的 `examples/dex/dex_node.nim` 曾增加 `--enable-mixer`、`--mixer-interval`、`--mixer-slots` 等参数，节点会在 `dex.mixers` Gossip 主题广播混币请求，并在本地挂载 `/dex/mixer` HTTP 路径返回 slot 调度方案（使用 `lpHttp.HttpService` 承载）。Matcher/Trader 通过日志即可看到「请求 → HTTP 计划 → 响应」完整链路，为后续与桥 Executor 的匿名标签写入/脚本化演示（已移除的 `examples/dex/scripts/run_coinjoin_demo.sh`）打下基础。
...
| D3 网络与可靠性 | 将 D2 协议嵌入 gossip + HTTP，处理重试/退出/DoS，并提供模拟器 | 已移除的 `examples/dex/mixer_service.nim`、`tests/integration/testcoinjoinflow.nim`（多节点 + 故障注入） | D2 |
...
| 资金用途 | 惩罚池资金用于补贴测试/审计费用，余额由 Program Owner 定期审计，记录在已移除的 `examples/dex/mixer_service.nim` 暴露的指标中。 |
...
| 网络集成 | Gossip stem/fluff + hpservice 盲中继 + `/dex/mixer` HTTP | integration | 已移除的 `examples/dex/mixer_service.nim` + `tests/integration/testcoinjoinflow.nim` |
...
- 评审通过后进入 D2：实现 `libp2p/crypto/coinjoin.nim`、`tests/coinjoin/testprotocol.nim`，并在已移除的 `examples/dex/mixer_service.nim` 原型化网络流程。
